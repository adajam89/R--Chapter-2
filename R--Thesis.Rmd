...
##  FA:    
 ...
 a <- 
        eta_sq(aov(FA ~ stim_format, data = exp5__anova.FA__data), partial = TRUE)
...
Our results match previous results by Paivio et al (1968):

- they just added colour to words, and did not observe any increased distinctiveness. BUT, findings from Experiments 3 + 4 suggest the effects of colour are negligible.
- Ensor noted this, and that's why they also manipulated font, size, capitilisation *and* colour.
...
On the surface, my findings appear to support Dual-coding theory (DCT):
   ...
   - The distinctiveness manipulations made to the words do nothing to remove the additional code available for pictures. 
...
Explanations of inconsistent findings:
  ...
  - Number of stim (10x of each type) = same as Ensor's Exp 1.
      - Ensor's Exp 2 doubled this, which also eliminated the PSE toward colour drawings. 
  - My *N* = 168 vs. Ensor's *N* = 30 (per exp). 
