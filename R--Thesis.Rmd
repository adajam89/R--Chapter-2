...
##    Ensor vs. current d'+c table: 
      exp5__table__ensor.vs.current__d.prime.c <- 
            tribble(~" ", ~"Grey words", ~"Distinctive words", ~"Grey words", ~"Distinctive words",
                  "Experiment 1",
...
\newpage
### Results
Comparisons are made in relation to the second experiment of @ensor2019b, as the number of study (80) and test (160) items were most similar to the current study (study: 60, test: 120). 

#### Manipulation check
To establish whether the distinctive word stimuli were indeed more distinctive than the regular words, a series of one-way repeated measures ANOVAs were run on *d'* and *c* scores, the mean proportion of hits and FAs, and overall scores of recognition (hits - FAs). 
...
In the ANOVA on d' scores, there was a significant main effect of stimuli format `r exp5__d.prime.aov__main.effect`. Distinctive words (`r exp5__d.prime.means__col.word`) produced significantly better discrimination between hits and FAs than regular words (`r exp5__d.prime.means__grey.word`), `r exp5__d.prime.contrasts__grey.word.col.word`.
...
##    Ensor vs. current d'+c table: 
      exp5__table__ensor.vs.current__hit.FA <- 
...
The ANOVA on the proportion of overall hits revealed a significant main effect of stimuli format, `r exp5__prop.hit.aov__main.effect`, though planned comparisons showed there was no difference in the number or hits between regular (`r exp5__prop.hit.means__grey.word`) and distinctive words (`r exp5__prop.hit.means__col.word`), `r exp5__prop.hit.contrasts__grey.word.col.word`. This was in contrast to the findings of @ensor2019b; reanalysis of their data also showed significant main effects of stimuli format across both experiments (Experiment 1: `r ensor.exp1__prop.hit.aov__main.effect`; Experiment 2: `r ensor.exp2__prop.hit.aov__main.effect`), however, distinctive words produced significantly more hits than regular words across both experiments (see `r table_numbering("exp5__table__ensor.vs.current__hit.FA", display = "cite")`).

`r table_numbering("exp5__table__ensor.vs.current__hit.FA")`
```{r, echo=FALSE, cache=FALSE}
      exp5__table__ensor.vs.current__hit.FA %>%
          kable("latex", booktabs = T, align = "rccc", escape = F) %>%
          kable_styling(font_size = 11, full_width = T, latex_options = c("hold_position", "condensed")) %>%
          column_spec(2:3, width = "1.8cm") %>%
          column_spec(4, width = "5cm") %>%
          add_header_above(c(" " = 1, "Hits" = 3)) %>%
...
##    Create list of desired contrasts from the above reference grid (1 = row of interest, 0 = ignore): 
      ...
      col.word = c(0, 1, 0, 0)
##    Run contrasts:                                    
      ensor.exp1__prop.FA.contrasts <- contrast(ensor.exp1__prop.FA.means, method = 
                                                list("grey.word - col.word" = grey.word - col.word), 
                                                adjust = "bonf")
##    Format results for use in report:
      ...
      ensor.exp1__prop.FA.means__grey.word <- ensor.exp1__prop.FA.means__apa$estimate$greyword %>%
          str_remove("\\,.*")
      ensor.exp1__prop.FA.means__col.word <- ensor.exp1__prop.FA.means__apa$estimate$colword %>% str_remove("\\,.*")
      ensor.exp1__prop.FA.contrasts__apa <- papaja:::apa_print.emmGrid(ensor.exp1__prop.FA.contrasts) 
      ensor.exp1__prop.FA.contrasts__grey.word.col.word <- ensor.exp1__prop.FA.contrasts__apa$statistic
```



In the ANOVA on the proportion of FAs, there was again a significant main effect of stim format `r exp5__prop.FA.aov__main.effect`. The distinctive words (`r exp5__prop.FA.means__col.word`) produced significantly fewer FAs than regular words (`r exp5__prop.FA.means__grey.word`), `r exp5__prop.FA.contrasts__grey.word.col.word`. Reanalysis of @ensor2019b data showed the same patterns. 


In the ANOVA on the proportion of FAs, there was no significant main effect of stim format `r ensor.exp1__prop.FA.aov__main.effect`.
...
##    Format results for use in report:
      ...
      ensor.exp1__d.prime.means__grey.word <- ensor.exp1__d.prime.means__apa$estimate$greyword %>%
          str_remove("\\,.*")
      ensor.exp1__d.prime.means__col.word <- ensor.exp1__d.prime.means__apa$estimate$colword %>% str_remove("\\,.*")
