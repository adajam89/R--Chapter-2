...
##    Import study data (which contains demographics info):                            
      exp1__participant_info <- read_csv(
        "./resources/experiment_1/experiment_1_data/Jamie - RFG Images (1) - STUDY_January 13, 2021_08.21.csv") %>%
          ...
          mutate_all(tolower) %>%
          group_by(study_id) %>%
          gather(., "question", "resp_optiononse", -study_id) %>%
          arrange(study_id) %>%
          drop_na(resp_optiononse) %>%
...
##  Temporarily extract demographics info:
      exp1__demographics <- exp1__participant_info %>%
          filter(question == "ip" | question == "recorded_date" | question == "progress" |
                 ...
                 question == "source") %>%
          spread(question, resp_optiononse) %>%
...
##    Temporarily extract study data (so we can reshape and put RTs on the same row as resp_optiononses).
      exp1__study_data <- exp1__participant_info %>%
          filter(question != "ip" & question != "recorded_date" & question != "progress" &
                 question != "age" & question != "gender" & question != "eng_first_lang" &
                 question != "source") %>%
          mutate(temp_index = as.double(resp_optiononse)) %>%
          mutate(index = ifelse(is.na(temp_index),'resp_optiononse','rt')) %>%
          select(-temp_index) %>%
          as.data.frame()
      exp1__study_data <- reshape(exp1__study_data, idvar=c('study_id','question'), timevar='index',
                                  direction="wide") %>%
          as_tibble(exp1__study_data, .name_repair) %>%
          rename(study_rt = "resp_optiononse.rt", study_resp_optiononse = "resp_optiononse.resp_optiononse") %>%
          group_by(study_id) %>%
...
##    Split the 'stim' column up, so item name and stimuli format are seperate:
          separate(stim, c("item_name", "stim")) %>%
##    Clearly show which lists were studied as words and pics: 
          mutate(studied_words = ifelse(stim == "word", study_list, NA)) %>%
          mutate(studied_pics = ifelse(stim == "picture", study_list, NA)) %>%
          select(-study_list) %>%
##    Count total study resp_optiononses (should be 60):
          add_tally(stim == "word" | stim == "picture", sort = FALSE, 
                    name = "study_responses_total") %>%
##    Count total study resp_optiononses by stim-format (should be 30 each):
          add_tally(stim == "word", sort = FALSE, name = "study_total_word_resp_option") %>%
          add_tally(stim == "picture", sort = FALSE, name = "study_total_pic_resp_option") %>%
##    Count total *correct* word resp_optiononses (should be 30):
          add_tally(stim == "word" & study_resp_optiononse == "word", sort = FALSE, 
                    name = "study_correct_word") %>%
##    Count total *correct* pic resp_optiononses (should be 30):
          add_tally(stim == "picture" & study_resp_optiononse == "picture", sort = FALSE, 
                    name = "study_correct_pic") %>%
##    Count total *correct* resp_optiononses (should be 60):
          mutate(study_correct = study_correct_word + study_correct_pic) %>%
##    Convert total *correct* resp_optiononses to proportion:
...
##    Calculate mean word reaction time (correct resp_optiononses):
          mutate(mean_study_rt_corr_word = mean(study_rt[stim == "word" & study_resp_optiononse == "word"])) %>%
##    Calculate mean pic reaction time (correct resp_optiononses):
          mutate(mean_study_rt_corr_pic = mean(study_rt[stim == "picture" & 
                                                        study_resp_optiononse == "picture"])) %>%
##    Remove columns with multiple unique values now we've done the totals:
          select(-study_rt, -study_resp_optiononse, -item_name, -stim) %>%
...
##    Merge justification columns from across the three resp_optiononse option conditions:
...
##    Reformat from long to wide:
          gather(., "question", "resp_optiononse", -test_id) %>%
          arrange(test_id)
#------------------------------------------------------------------------------------------------------------------
      exp1__data.part2 <- 
...
##    Reformat from long to wide:
          ...
          arrange(test_id)
#------------------------------------------------------------------------------------------------------------------
      exp1__test.stats <- exp1__data.part1 %>%
          bind_rows(exp1__data.part2) %>%
          drop_na(resp_optiononse)
...
##    Tidy test data:
      exp1__test.stats <- exp1__test.stats %>%
          filter(question != "ip" & question != "start_date" & question != "progress" &
                   ...
                   question != "both_justification" & question != "guess_justification" & 
                   question != "comments") %>%
          mutate(rfr_resp_option_type = question) %>%
          mutate(rfr_resp_option_type = str_remove(rfr_resp_option_type, ".*_")) %>%
          mutate(rfr_resp_option_type = ifelse(rfr_resp_option_type == "recollection" | rfr_resp_option_type == "familiarity",
                                        rfr_resp_option_type, "")) %>%
          mutate(question = str_remove(question,
...
##    Reshape to put RTs on the same row as resp_optiononses):
          mutate(temp_index = as.double(resp_optiononse)) %>%
          mutate(index = ifelse((is.na(temp_index) & resp_option == "rfg") & 
                                  (resp_optiononse == "old" | resp_optiononse == "new"), 'rfg_old_new','rt')) %>%
          mutate(index = ifelse((is.na(temp_index) & resp_option == "rfbg") & 
                                  (resp_optiononse == "old" | resp_optiononse == "new"), 'rfbg_old_new', index)) %>%
          mutate(index = ifelse((is.na(temp_index) & resp_option == "rf_ratings") & 
                                  (resp_optiononse == "old" | resp_optiononse == "new"), 'rf_ratings_old_new', index)) %>%
          mutate(index = ifelse((is.na(temp_index) & resp_option == "rfg") & 
                                  (resp_optiononse != "old" & resp_optiononse != "new"), 'rfg', index)) %>%
          mutate(index = ifelse((is.na(temp_index) & resp_option == "rfbg") & 
                                  (resp_optiononse != "old" & resp_optiononse != "new"), 'rfbg', index)) %>%
          mutate(index = ifelse((is.na(temp_index) & 
                                   resp_option == "rf_ratings" & rfr_resp_option_type == "recollection") & 
                                  (resp_optiononse != "old" & resp_optiononse != "new"), 'rec_rating', index)) %>%
          mutate(index = ifelse((is.na(temp_index) & 
                                   resp_option == "rf_ratings" & rfr_resp_option_type == "familiarity") & 
                                  (resp_optiononse != "old" & resp_optiononse != "new"), 'fam_rating', index)) %>%
          select(-rfr_resp_option_type, -temp_index, -question_number) %>%
          as.data.frame()
#------------------------------------------------------------------------------------------------------------------
      exp1__test.stats <- reshape(exp1__test.stats, idvar=c('test_id','question'), timevar='index', 
                                 direction="wide") %>%
          as_tibble(exp1__study_data, .name_repair) %>%
          unite("old_new", resp_optiononse.rfg_old_new, resp_optiononse.rfbg_old_new, 
                resp_optiononse.rf_ratings_old_new, remove = TRUE, na.rm = TRUE) %>%
          select(-resp_option.rfg_old_new, -resp_option.rfg, -resp_option.rf_ratings_old_new, 
                 ...
                 -resp_option.rfbg) %>%
##    Recode RF-Ratings resp_optiononses to better show rec/fam:
          mutate(resp_optiononse.rec_rating = str_remove_all(resp_optiononse.rec_rating, " ")) %>%
          mutate(resp_optiononse.rec_rating = str_remove_all(resp_optiononse.rec_rating, "(?<=.{1}).+")) %>%
          mutate(resp_optiononse.rec_rating = str_c("rec_", resp_optiononse.rec_rating)) %>%
          mutate(resp_optiononse.fam_rating = str_remove_all(resp_optiononse.fam_rating, " ")) %>%
          mutate(resp_optiononse.fam_rating = str_remove_all(resp_optiononse.fam_rating, "(?<=.{1}).+")) %>%
          mutate(resp_optiononse.fam_rating = str_c("fam_", resp_optiononse.fam_rating)) %>%
##    Unite columns of the same resp_optiononse type:
          unite("recfam_1", resp_optiononse.rfg, resp_optiononse.rfbg, resp_optiononse.rec_rating, remove = TRUE, na.rm = TRUE) %>%
          mutate(recfam_1 = na_if(recfam_1, "")) %>%
          rename(recfam_2 = "resp_optiononse.fam_rating")
##    Merge with 'exp1__image_urls' to get proper item names:
      ...
      exp1__test.stats <- exp1__test.stats %>%
          ...
          rename(old_new_rt = "resp_optiononse.rt", resp_option = "resp_option.rt", 
                 belong_to_study_list = "study_list") %>%
##    Remove filler items:
          filter(!str_detect(belong_to_study_list, 'filler')) %>%
##    Split the 'stim' column up, so item name and stimuli format are seperate:
          separate(stim, c("item_name", "stim")) %>%
##    Count total test resp_optiononses (should be 120):
          add_tally(stim == "word" | stim == "picture", sort = FALSE, name = "test_total_resp_option") %>%
##    Count total test resp_optiononses by stim-format (should be 60 each):
          add_tally(stim == "word", sort = FALSE, name = "test_total_word_resp_option") %>%
          add_tally(stim == "picture", sort = FALSE, name = "test_total_pic_resp_option") %>%
##    Calculate mean overall reaction time:
          mutate(old_new_rt = as.double(old_new_rt)) %>%
          mutate(mean_old_new_rt_total = mean(old_new_rt)) %>%
##    Calculate mean word reaction time (all resp_optiononses):
          mutate(mean_test_rt_word = mean(old_new_rt[stim == "word"])) %>%
##    Calculate mean pic reaction time (all resp_optiononses):
          mutate(mean_test_rt_pic = mean(old_new_rt[stim == "picture"]))
##    Add justifications back in: 
      exp1__test.stats <- exp1__test.stats %>%
          ...
          select(test_id, ip, start_date, rec_justification, fam_justification, both_justification,
                 guess_justification, comments, resp_option, item_name, stim, belong_to_study_list, 
                 old_new, old_new_rt, recfam_1, recfam_2, test_total_resp_option, test_total_word_resp_option, 
                 test_total_pic_resp_option, mean_old_new_rt_total, mean_test_rt_word, mean_test_rt_pic) %>%
          mutate_if(is.character, list(~na_if(., ""))) %>%
          rename( test_start_time = "start_date")
##    Remove Ps with incomplete test data:
      exp1__incomplete_exp1__test.stats <- exp1__test.stats %>%
          select(test_id, test_total_resp_option) %>%
          distinct(test_id, .keep_all = TRUE) %>%
          filter(test_total_resp_option != 120) %>%
          pull(test_id)
      exp1__test.stats <- exp1__test.stats %>%
          filter(!test_id %in% exp1__incomplete_exp1__test.stats)
####################################################################################################################
...
##  Add new d'prime calculations to 'exp3__test.stats' df:  ------------------------------------------------------ 
    exp1__test.stats <- exp1__test.stats %>%
        ...
        mutate_at(vars(age, study_responses_total, study_total_word_resp_option, study_total_pic_resp_option,
...
##----------------------------------------------------------------------------------------------------------------
    ...
    exp1__n.unspecified <- nrow(exp1__final.data %>% filter(source == "0"))
