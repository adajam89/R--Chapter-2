...
exp5 <- exp5__final_data %>%
  select(prop.hit__grey.word, prop.hit__col.word, prop.hit__grey.drawing, prop.hit__col.drawing, 
         prop.hit__grey.photo, prop.hit__col.photo)
write.csv(exp5,'~/Desktop/exp5.csv', row.names = FALSE)
...
##    Get means:
      ...
      exp5__prop.hit.means__grey.word <- summary(exp5__prop.hit.means)$emmean[1]
      exp5__prop.hit.means__col.word <- summary(exp5__prop.hit.means)$emmean[2]
```

The ANOVA on the mean proportion of overall hits revealed a significant main effect of stimuli format, `r apa::anova_apa(exp5__prop.hit.aov, effect = "stim", format = "rmarkdown", print = FALSE)`. However, planned comparisons revealed 


`r apa::apa(exp5__prop.hit__ttest, format = "rmarkdown", print = FALSE)`
