...
```{r fig.width=7, fig.height=5, echo=FALSE, cache=TRUE, warning=FALSE}
      exp5__figure__rfg.hits
```
...
```{r echo=FALSE, results = FALSE, message=FALSE, warning=FALSE, cache=TRUE, fig.show='hide'}
      ...
      ensor.exp2__prop.hit__ttest2 <- ensor.exp2__prop.hit %>% filter(stim == "col.word" | 
                                                                      stim == "grey.drawing") %>%
                                      t.test(prop.hit~stim, paired=TRUE, data = .)
      exp5__prop.hit__ttest__grey.word.vs.col.word2 <- exp5__prop.hit %>% filter(stim == "col.word" | stim == "grey.drawing") %>%
                                      t.test(prop.hit~stim, paired=TRUE, data = .)
#################################################################################################################
...
```{r echo=FALSE, results = FALSE, message=FALSE, warning=FALSE, cache=TRUE, fig.show='hide'}
      ...
      ensor.exp2__prop.FA__ttest2 <- ensor.exp2__prop.FA %>% filter(stim == "col.word" | 
                                                                      stim == "grey.drawing") %>%
                                      t.test(prop.FA~stim, paired=TRUE, data = .)
      exp5__prop.FA__ttest__grey.word.vs.col.word2 <- exp5__prop.FA %>% filter(stim == "col.word" | stim == "grey.drawing") %>%
                                      t.test(prop.FA~stim, paired=TRUE, data = .)
#################################################################################################################
...
### Discussion
@ensor2019b demonstrated how picture superiority effects (PSEs) could be eliminated by increasing the distinctiveness of word stimuli (relative to shaded drawings) via manipulations to colour, font, size, and capitalisation. The current experiment aimed to replicate such findings in order to examine whether such 


effects extend to photographs 


to determine whether the 


The first hypothesis pertained to the current manipulation, whereby the word items with manipulations to colour, font, and capitalisation were expected to produce enhanced recognition relative to regular word stimuli.  

Recognition will be enhanced for  across items, compared to those consistently presented in the same black typeface. For colour words, this enhancement is expected to manifest as the following compared to greyscale words:
    + i) higher overall *d'* scores;
    + ii) higher proportion of correct hits;
    + iii) lower proportion of false alarms. 
...
The second hypothesis proposed standard picture superiority effects (PSEs) would emerge - a finding that was abundantly clear when examining the data, with *all* pictures (greyscale drawings; colour drawings; greyscale photographs; colour photographs) demonstrating superior performance compared to regular words (manifesting as significantly higher proportions of hits, lower proportions of FAs, and a higher mean *d'* scores). In a similar fashion, *photo* superiority effects (PhSEs) were demonstrated when comparing performance for photograph items against that of drawing items, both for greyscale items (replicating the finding of *Experiment 3*) and colour items (replicating the finding of *Experiment 4*). 
