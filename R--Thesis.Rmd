...
### Data prep
*d'* couldn't be calculated for grey words for 5 participants (zero hits and zero FAs, they did not respond *Old* at all), and for col words for 2 participants. Their data was not excluded entirely - just the data toward those stim types where the result was "inf".
...
##    Calculate d-prime:                                                                                       ##
#################################################################################################################
      exp5__grey.word.d.prime <- exp5__test_stats %>%
          select(id, prop.hit__grey.word, prop.FA__grey.word, prop.miss__grey.word, prop.CR__grey.word) %>%
      ...
      exp5__col.word.d.prime <- exp5__test_stats %>%
      ...
      exp5__grey.drawing.d.prime <- exp5__test_stats %>%
      ...
      exp5__col.drawing.d.prime <- exp5__test_stats %>%
      ...
      exp5__grey.photo.d.prime <- exp5__test_stats %>%
      ...
      exp5__col.photo.d.prime <- exp5__test_stats %>%
...
##    Add new d'prime calculations to 'exp5__test_stats' df: 
      exp5__test_stats <- exp5__test_stats %>%
          right_join(exp5__grey.word.d.prime) %>%
          right_join(exp5__col.word.d.prime) %>%
          right_join(exp5__grey.drawing.d.prime) %>%
          right_join(exp5__col.drawing.d.prime) %>%
          right_join(exp5__grey.photo.d.prime) %>%
          right_join(exp5__col.photo.d.prime)
...
#---------------------------------------------------------------------------------------------------------------- 
      ensor.exp1__grey.word.d.prime <- ensor.exp1.data %>%
          ...
          #summarise(mean(c__grey.word)) %>% pull() %>% round(3)
...
#---------------------------------------------------------------------------------------------------------------- 
      ensor.exp2__grey.word.d.prime <- ensor.exp2.data %>%
          ...
          mutate(dprime__grey.word = qnorm(prop.hit__grey.word) - qnorm(prop.FA__grey.word)) %>%
          select(id, dprime__grey.word)
          #summarise(mean(dprime)) %>% pull() %>% round(3)
...
##################################################################################################################
##    One-way repeated measures ANOVA:
      exp5__d.prime.analysis <- exp5__d.prime %>%
          arrange(d.prime) %>% 
          filter(id != 2413133 & id != 2517279 & id != 2726395 & id != 2523158 & id != 2419641 & id != 2447369 &
                 id != 2501529)
        
      exp5__d.prime.aov <- afex::aov_ez('id', 'd.prime', fun_aggregate = mean, exp5__d.prime.analysis, 
...
#----------------------------------------------------------------------------------------------------------------
##    Significant main effect, so perform Paired Samples t-test:
      exp5__d.prime__ttest <- exp5__d.prime.analysis %>% filter(stim == "grey.word" | stim == "col.word") %>%
...
#################################################################################################################   
      exp5__table__ensor.vs.current__d.prime <- 
            tribble(~" ", ~"Grey words", ~"Distinctive words", ~"Planned comparisons",
                  ...
                  "Trials: 60 study / 120 test:", 
                      round(exp5__d.prime.analysis %>% filter(stim == "grey.word") %>% pull(d.prime) %>% mean(),2),
                      round(exp5__d.prime.analysis %>% filter(stim == "col.word") %>% pull(d.prime) %>% mean(),2),
                      apa::apa(exp5__d.prime__ttest, format = "rmarkdown", print = FALSE))     
```

```{r echo=FALSE, results = FALSE, cache=FALSE}
      table_numbering(name = "exp5__table__ensor.vs.current__d.prime", caption = "Mean *d'* scores for regular and distinctive word stimuli, across the current experiment and @ensor2019b. Within-subjects contrasts are presented following significant main effects of stimuli format (reported in text).", display = FALSE)
```      
...
For the mean proportion of FAs, the second experiment of @ensor2019b showed an identical pattern to the current study: a significant main effect of stimuli format [`r apa::anova_apa(ensor.exp2__prop.FA.aov, effect = "stim", format = "rmarkdown", print = FALSE)`] showed that distinctive words produced significantly fewer FAs than regular words (see `r table_numbering("exp5__table__ensor.vs.current__FAs", display = "cite")`). The same pattern was not evident in their first experiment, however, where no significant main effect of stimuli format was found [`r apa::anova_apa(ensor.exp1__prop.FA.aov, effect = "stim", format = "rmarkdown", print = FALSE)`]. Visual inspection of the data shows that subjects made more FAs in the @ensor2019b experiments compared to the current study; this is especially apparent when comparisons are made toward the experiment with more trials.
...
```{r, echo=FALSE, results = FALSE, message=FALSE, warning=FALSE, cache=TRUE, fig.show='hide'}
      ...
      ensor.exp2__d.prime__ttest2 <- ensor.exp2__d.prime %>% filter(stim == "col.word" | 
                                                                      stim == "grey.drawing") %>%
                                      t.test(d.prime~stim, paired=TRUE, data = .)
      exp5__d.prime__ttest2 <- exp5__d.prime.analysis %>% filter(stim == "col.word" | stim == "grey.drawing") %>%
                                      t.test(d.prime~stim, paired=TRUE, data = .)
#################################################################################################################
...
#################################################################################################################   
      exp5__table__ensor.vs.current__d.prime2 <- 
            tribble(~" ", ~"Distinctive words", ~"Grey drawings", ~"Planned comparisons",
                  ...
                  "Trials: 60 study / 120 test:", 
                      round(exp5__d.prime.analysis %>% filter(stim == "col.word") %>% pull(d.prime) %>% mean(),2),
                      round(exp5__d.prime.analysis %>% filter(stim == "grey.drawing") %>% pull(d.prime) %>%
                              mean(),2),
                      apa::apa(exp5__d.prime__ttest2, format = "rmarkdown", print = FALSE))
```      

```{r echo=FALSE, results = FALSE, cache=FALSE}
      table_numbering(name = "exp5__table__ensor.vs.current__d.prime2", caption = "Mean *d;* scores for distinctive word stimuli and grey drawings, across the current experiment and @ensor2019b. Within-subjects contrasts are presented following significant main effects of stimuli format (reported in text).", display = FALSE)
```

A key finding in @ensor2019b data was an elimination of the PSE; distinctive word stimuli and grey drawings produced similar *d'* scores, indicative of comparable distinctiveness between the two stimuli formats. Such findings were replicated in the current reanalysis of the data (see `r table_numbering("exp5__table__ensor.vs.current__d.prime2", display = "cite")`). In the current experiment, however, this finding was not replicated, with drawings showing significantly higher *d'* than the distinctive words, suggesting the manipulation was not effective. 
...
```{r echo=FALSE, results = FALSE, cache=FALSE}
      table_numbering(name = "exp5__table__ensor.vs.current__prop.hit2", caption = "Mean proportion of hits for distinctive word stimuli and grey drawings, across the current experiment and @ensor2019b. Within-subjects contrasts are presented following significant main effects of stimuli format (reported in text).", display = FALSE)
```
...
```{r echo=FALSE, results = FALSE, cache=FALSE}
      table_numbering(name = "exp5__table__ensor.vs.current__prop.FA2", caption = "Mean proportion of FAs for distinctive word stimuli and grey drawings, across the current experiment and @ensor2019b. Within-subjects contrasts are presented following significant main effects of stimuli format (reported in text).", display = FALSE)
```
