...
```
### Participants
A total of `r nrow(exp1__full_data)` subjects completed the online experiment (see `r table_numbering("exp1--age-gender--tablecaption", display = "cite")` for a breakdown of the gender and age of the sample). This sample size is typical in the literature for a medium effect size (.25), however, due to mixed conclusions regarding the direction of R + F, it was difficult to determine an exact a-priori number of subjects. 

The current sample was primarily comprised of participants sourced from voluntary participation websites such as \href{https://www.prolific.co/}{Prolific Academic} (`r exp1__n_prolific`%)
