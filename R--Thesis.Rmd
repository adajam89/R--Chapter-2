...
The current experiment aims to establish whether colour information facilitates recognition

irrespective of the level of detail present in an image. 

The methodology of *Experiment 3* will be repeated utilising colour (rather than greyscale) versions of both types of image: i) a set of simple, line-drawn object illustrations sourced from @rossion2004; ii) a set of detailed real-world object photographs established in *Experiment 2*. 


It is acknowledged that colour may not affect the image items in a uniform manner. The highly distinctiveness nature of the photograph items may plateau in such a way that performance can no longer be noticeably enhanced; any additional distinctiveness generated by colour information might provide little benefit if performance is already sufficiently high. If this is the case, however, the effects of the colour manipulation should still be evident when examining recognition performance for drawings. 
...
\hfill\break To determine the effects of stimuli format and response option on the classification of recognition memory judgements, separate 3 (stimuli format: words, drawings, photographs) x 2 (response option condition: RFG-judgements, RFBG-judgements) mixed ANOVAs were conducted on the mean proportion of hits assigned Recollection, Familiarity, and Guessing (see `r figure_numbering("exp4__prop.hit.RFG__figure", display = "cite")`), and the mean proportion of FAs assigned RFG (see `r figure_numbering("exp4__prop.FA.RFG__figure", display = "cite")`) 
...
##  Create figure:
    exp4__prop.FA.RFG__figure <-
        ...
        theme(
            ...
            plot.margin = margin(20, 20, 24, 30))
```
```{r exp4__prop-rec-fam-guess-FAs--figurecaption, echo=FALSE, results = FALSE, cache=FALSE}
