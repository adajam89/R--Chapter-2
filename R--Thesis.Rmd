...
#### Response option:
In the ANOVA on the mean proportion of hits, there was a significant main effect of response option condition `r apa::anova_apa(exp3__prop.hit.aov, effect = "resp_option", format = "rmarkdown", print = FALSE)`. Follow up t-tests showed a higher proportion of hits in the RFG group (*M*= `r round(exp3__prop.hit__resp.option.means %>% filter(resp_option == "RFG") %>% pull(emmean),2)`) compared to the RFBG group (*M*= `r round(exp3__prop.hit__resp.option.means %>% filter(resp_option == "RFBG") %>% pull(emmean),2)`), `r str_c(papaja::apa_print(exp3__prop.hit__ttest__rfg.rfbg)$statistic, ", *d* = ", round(as.vector(exp3__prop.hit__ttest__rfg.rfbg__eff.size$effsize), 2))`. This is considered a `r as.vector(exp3__prop.hit__ttest__rfg.rfbg__eff.size$magnitude)` effect size. 
