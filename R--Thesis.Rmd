...
#### Response option:
In the ANOVA on the mean proportion of overall hits, there was a significant main effect of response option, `r apa::anova_apa(exp3__prop.hit.aov, effect = "resp_option", format = "rmarkdown", print = FALSE)`. Follow up t-tests showed a higher proportion of hits in the RFG group (*M*= `r round(exp3__prop.hit__resp.option.means %>% filter(resp_option == "RFG") %>% pull(emmean),2)`) compared to the RFBG group (*M*= `r round(exp3__prop.hit__resp.option.means %>% filter(resp_option == "RFBG") %>% pull(emmean),2)`), `r papaja::apa_print(exp3__prop.hit__ttest__rfg.rfbg)$statistic %>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")` - though the size of the effect was `r as.vector(exp3__prop.hit__ttest__rfg.rfbg__eff.size$magnitude)` (`r str_c("*d* = ", round(abs(as.vector(exp3__prop.hit__ttest__rfg.rfbg__eff.size$effsize)), 2))`).


In the ANOVAs on the mean proportion of overall FAs, hits assigned *Guessing*, FAs assigned *Recollection*, and FAs assigned *Guessing*, there were no significant main effects of response option condition.
...
exp3__resp.option__main.effects__table <- tribble(
        ~"Variable means", ~"Main effect of response option:", ~"Signif",
        "Hits",
...
```{r, echo=FALSE, cache=FALSE}
    exp3__resp.option__main.effects__table %>%
        kable("markdown", booktabs = T, full_width= T)
```
