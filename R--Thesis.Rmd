...
```{r, echo=FALSE, cache=TRUE}
    exp3__prop.hit.FA.d.prime__table1 <- exp3__prop.hit.FA.d.prime__table %>%
        kable("latex", booktabs = T) %>%
        #kable_styling(latex_options = c("scale_down", "hold_position"), full_width = F) %>%
        pack_rows("Stimuli format", 1, 3) %>%
        pack_rows("Response option", 4, 5) %>%
        add_header_above("Experiment 3: Grey", 5, line = F)

    exp4__prop.hit.FA.d.prime__table1 <- exp4__prop.hit.FA.d.prime__table %>%
        kable("latex", booktabs = T) %>%
        #kable_styling(latex_options = c("scale_down", "hold_position"), full_width = F) %>%
        pack_rows("Stimuli format", 1, 3) %>%
        pack_rows("Response option", 4, 5) %>%
        add_header_above("Experiment 4: Colour", 5, line = F)
  

knitr::kables(list(exp3__prop.hit.FA.d.prime__table1, exp4__prop.hit.FA.d.prime__table1))


    test <- tribble(
        ~" ", ~Hits, ~FAs, ~"d'", ~"   ", ~"  ", ~Hits, ~FAs, ~"d'",
        "Words", 1,1,1,"","Words", 1,1,1,
        "Drawings", 1,1,1,"","Drawings", 1,1,1,
        "Photographs",1,1,1,"","Photographs",1,1,1,
        "RFG",1,1,1,"","RFG",1,1,1,
        "RFBG",1,1,1,"", "RFBG",1,1,1)
    ...
    test %>%
        ...
        pack_rows("Stimuli format", 1, 3) %>%
        pack_rows("Response option", 4, 5) %>%
        add_header_above(c("Experiment 3: Grey" = 4, " " = 1, "Experiment 4: Colour" = 4), line = F, bold = T) %>%
        column_spec(1, width = "3.8cm") %>%
        column_spec(2:4, width = "1cm") %>%
        column_spec(5, width = "2cm") %>%
        column_spec(6, width = "3.8cm") %>%
        column_spec(7:9, width = "1cm")
```
...
(*M*= `r round(exp4__prop.FA.F__stim.means %>% filter(stim == "colour.drawing") %>% pull(emmean),2)`), `r exp4__prop.FA.F__ttest__col.photo.vs.col.drawing`. There were no significant interaction effects between stimuli format and response option condition, `r apa::anova_apa(exp4__prop.FA.F.aov, effect = "resp_option:stim", format = "rmarkdown", print = FALSE)`.
