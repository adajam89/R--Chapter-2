...
Results from the ANOVAs demonstrated a significant main effect of stimuli format for each of these variables; hits (`r apa::anova_apa(exp4__prop.hit.aov, effect = "stim", format = "rmarkdown", print = FALSE)`), FAs (`r apa::anova_apa(exp4__prop.FA.aov, effect = "stim", format = "rmarkdown", print = FALSE)`), and *d'* scores (`r apa::anova_apa(exp4__d.prime.aov, effect = "stim", format = "rmarkdown", print = FALSE)`). though no interaction effects were evident between stimuli format and response option for any; hits (`r apa::anova_apa(exp4__prop.hit.aov, effect = "resp_option:stim", format = "rmarkdown", print = FALSE)`), FAs(`r apa::anova_apa(exp4__prop.FA.aov, effect = "resp_option:stim", format = "rmarkdown", print = FALSE)`), or *d'* scores (`r apa::anova_apa(exp4__d.prime.aov, effect = "resp_option:stim", format = "rmarkdown", print = FALSE)`). 

To determine whether photo superiority effects were also evident in the current set of colour pictures - comparable to those observed using greyscale stimuli (Experiment 3) - pairwise t-tests were performed between the colour photos and drawings. For the mean proportion of hits, colour photographs (*M*= `r round(exp4__prop.hit__stim.means %>% filter(stim == "col.photo") %>% pull(emmean),2)`) exhibited a significantly higher proportion than colour drawings (*M*= `r round(exp4__prop.hit__stim.means %>% filter(stim == "col.drawing") %>% pull(emmean),2)`), `r exp4__prop.hit__ttest__col.photo.vs.col.drawing`. The photographs (*M*= `r round(exp4__prop.FA__stim.means %>% filter(stim == "col.photo") %>% pull(emmean),2)`) also produced significantly fewer FAs compared to drawings (*M*= `r round(exp4__prop.FA__stim.means %>% filter(stim == "col.drawing") %>% pull(emmean),2)`), `r exp4__prop.FA__ttest__col.photo.vs.col.drawing`). Mean *d'* scores were also significantly higher for the colour photographs (*M*= `r round(exp4__d.prime__stim.means %>% filter(stim == "col.photo") %>% pull(emmean),2)`) compared to the colour drawings (*M*= `r round(exp4__d.prime__stim.means %>% filter(stim == "col.drawing") %>% pull(emmean),2)`), `r exp4__d.prime__ttest__col.photo.vs.col.drawing`.

Visual comparison of the data (and significant results) across Experiments 3 and 4 reveals only one difference with regard to response option: the ANOVA on *d'* scores failed to produce a significant interaction between stimuli format and response option in the current experiment, as was previously demonstrated in Experiment 3. However, none of the follow-up comparisons
...
##  Create figure:
    exp4__prop.hit.RFG__figure <-
        ...
        scale_fill_manual("legend", values = c("Recollection" = "#845E7D", "Familiarity" = "#428495",
                                               "Guessing" = "#529B6A")) + 
        geom_col(position = position_dodge()) +
        geom_errorbar(aes(ymin = pmax(emmean-SE, 0), ymax = emmean+SE), 
                      width = 0.2, size = 0.3, position = position_dodge(0.9)) +
...
exp3__prop.hit.RFG__figure1 <- exp3__prop.hit.RFG__figure + labs(subtitle = "Experiment 3: Greyscale") + 
