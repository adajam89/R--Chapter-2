...
##    Write references file for all of the packages used:
      write.bib(packages, "resources/packages.bib")
##    Merge Zotero references with packages references into a single file:
      cat(paste("% Automatically generated", Sys.time()), "\n% DO NOT EDIT",
          { readLines("resources/refs.bib") %>% 
              paste(collapse = "\n") },
          { readLines("resources/packages.bib") %>% 
              paste(collapse = "\n") },
          file = "resources/biblio.bib", sep = "\n")
...
To determine whether a PSE is evident in the current paradigm, *d'* (d-prime) scores will be calculated for each participant. *d'* is a signal detection statistic, calculated by taking the standardised difference between the signal (i.e. correct hits) and *lack* of signal (i.e. false alarms); in other words, *d'* offers a representation of participants' ability to distinguish target items from lures [@doi:https://doi.org/10.1002/9781118445112.stat06743]. A *d'* score of 


[see `r appendix_numbering("d.prime.calc", display = "cite")`]
...
*d'* scores were calculated by `r toBibtex(citation("psycho"))`
