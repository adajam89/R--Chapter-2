...
##  Double spacing:
    - \usepackage{setspace}
    - \doublespacing
##  Reduces spacing after headings a little bit (looks better - only applies to heading levels 1-3):
    - \usepackage{titlesec}
    - \titlespacing{\section}{0pt}{12pt plus 2pt minus 1pt}{0pt plus 1pt minus 1pt}
    - \titlespacing{\subsection}{0pt}{12pt plus 2pt minus 1pt}{0pt plus 1pt minus 1pt}
    - \titlespacing{\subsubsection}{0pt}{12pt plus 2pt minus 1pt}{0pt plus 1pt minus 1pt}    
##  Formats headings to APA:
    - \usepackage{sectsty} \sectionfont{\centering}  ## Heading 1: centred + bold. 
    - \usepackage{sectsty} \subsubsectionfont{\emph} ## Heading 3: left + bold + italics.   
##  Headers and footers:
    - \usepackage{fancyhdr}   
    - \pagestyle{fancy}                              ##  Header template (sets H1 right / H2 left / page numbers).
    - \renewcommand{\headrulewidth}{1.5pt}           ##  Set width of header line. 
    - \renewcommand{\href}[2]{#2\footnote{\url{#1}}} ##  Puts in-text htmk links as a footnote.
    - \setlength{\headheight}{25.66263pt}
##  Allow for mid-document landscape pages:
...
##  Captioner:
    ...
    appendix_numbering <- captioner(prefix = "Appendix", type = c("C")) ## Use letters rather than numbers. 
    appendix_numbering("spelling.corrections", 
                       "Manipulations / spelling corrections of naming responses (Experiment 2).", display = FALSE)
    appendix_numbering("exp2.full.norms", 
                       "Normative data for newly curated photograph items (Experiment 2).", display = FALSE)
```
...
##    Set correct row for rownames (Qualtrics outputs with 3 header rows - we only want one): 
    ...
    ##    Reformat from long to wide:
          ...
          arrange(test_id)
#------------------------------------------------------------------------------------------------------------------
    exp1__test.data <- bind_rows(exp1__data.part1, exp1__data.part2)
##    Temporarily extract justifications:
      exp1__justifications <- exp1__test.data %>%
...
##    Tidy test data:
      exp1__test.data <- exp1__test.data %>%
          filter(question != "ip" & question != "start_date" & question != "progress" & 
                   question != "recorded_date" & question != "rec_justification" & 
                   question != "fam_justification" & question != "both_justification" & 
                   question != "guess_justification" & question != "comments") %>%
    ##  Remove empty / unimportant rows:
          filter(!str_detect(question, "timing")) %>%
          drop_na(response) %>%
    ##  Replace stim code with actual item name:
...
##    List all the unique participants we have in 'exp1__study.data': 
      exp1__unique_p_at_study <- exp1__study.data %>%
          ...
          arrange(ip, time)
##    List all unique participants we have in 'exp1__test.data', and format test_start_time to simplified version:
      exp1__unique_p_at_test <- exp1__test.data %>%
...
##    duplicated ids back into the NA gaps of the corresponding study_ids.  
      exp1__all_participants <- exp1__all_participants %>%
          filter(is.na(test_id) | (test_id != "2fwc8n0fuonvta3" & test_id != "247estlk1nx9ljy"
                                 & test_id != "uenhxpj5gjopn4t" & test_id != "3p3jpgswhnvaycz")) %>%
          mutate(test_id = ifelse(str_detect(study_id, "pfexrfarjkpsygf"), "2fwc8n0fuonvta3", test_id)) %>%
          mutate(test_id = ifelse(str_detect(study_id, "3i69jnaz1wrx4fq"), "247estlk1nx9ljy", test_id)) %>%
          mutate(test_id = ifelse(str_detect(study_id, "3sbkutkpbz0t7y1"), "uenhxpj5gjopn4t", test_id)) %>%
          mutate(test_id = ifelse(str_detect(study_id, "1i3sv43b6k977mc"), "3p3jpgswhnvaycz", test_id)) %>%
##    Some P only have study data, and no test data, so we remove those:
          drop_na(test_id) %>%
          select(-ip, -time) %>%
...
##    Prepare to join data for participants who completed both study and test:
      exp1__study.data <- exp1__study.data %>%
...
####################################################################################################################
      exp1__test.stats <- exp1__all_participants %>%
          full_join(exp1__study.data, by = "study_id") %>%
          full_join(exp1__test.data, by = "test_id") %>%
          group_by(id) %>%
    ##  Total hits and misses:
          add_tally(studied_words == belong_to_study_list & shown_format == "word" & old.new == "old",
                    name = "hit__word") %>%
          add_tally(studied_words == belong_to_study_list & shown_format == "word" & old.new == "new", 
                    name = "miss__word") %>%
          add_tally(studied_pics == belong_to_study_list & shown_format == "picture" & old.new == "old", 
                    name = "hit__pic") %>%
          add_tally(studied_pics == belong_to_study_list & shown_format == "picture" & old.new == "new", 
                    name = "miss__pic") %>%
...
##    Rec/fam hits: 
          add_tally((studied_words == belong_to_study_list & shown_format == "word") & 
                      ((resp_option != "rf_ratings" & rfg1 == "rec") |
                         (resp_option == "rf_ratings" & rfg1 == "rec_3" | rfg1 == "rec_4" | rfg1 == "rec_5")), 
                              name = "hit.R__word") %>%
          add_tally((studied_words == belong_to_study_list & shown_format == "word") & 
                      ((resp_option != "rf_ratings" & rfg1 == "fam") |
                         (resp_option == "rf_ratings" & rfg2 == "fam_3" | rfg2 == "fam_4" | rfg2 == "fam_5")), 
                              name = "hit.F__word") %>%
          add_tally((studied_words == belong_to_study_list & shown_format == "word") & 
                      rfg1 == "both", name = "hit__word_both") %>%
          add_tally((studied_words == belong_to_study_list & shown_format == "word") & 
                      ((resp_option != "rf_ratings" & rfg1 == "guess") |
                         (resp_option == "rf_ratings" & (rfg1 == "rec_0" & rfg2 == "fam_0"))),
                              name = "hit.G__word") %>%
          add_tally((studied_pics == belong_to_study_list & shown_format == "picture") & 
                      ((resp_option != "rf_ratings" & rfg1 == "rec") |
                         (resp_option == "rf_ratings" & rfg1 == "rec_3" | rfg1 == "rec_4" | rfg1 == "rec_5")), 
                              name = "hit.R__pic") %>%
          add_tally((studied_pics == belong_to_study_list & shown_format == "picture") & 
          ...
          add_tally((studied_pics != belong_to_study_list & shown_format == "picture") & 
                      ((resp_option != "rf_ratings" & rfg1 == "fam") |
                         (resp_option == "rf_ratings" & rfg2 == "fam_3" | rfg2 == "fam_4" | rfg2 == "fam_5")), 
                              name = "FA.F__pic") %>%
          add_tally((studied_pics != belong_to_study_list & shown_format == "picture") & 
                      rfg1 == "both", name = "FA__pic_both") %>%
          add_tally((studied_pics != belong_to_study_list & shown_format == "picture") & 
                      ((resp_option != "rf_ratings" & rfg1 == "guess") |
                         (resp_option == "rf_ratings" & (rfg1 == "rec_0" & rfg2 == "fam_0"))),
                            name = "FA.G__pic") %>%
          select(-item_name, -old.new, -rfg1, -rfg2) %>%
...
```{r echo=FALSE, results = FALSE, cache=FALSE}
    table_numbering(name = "exp1__age.gender__table", 
                     caption = "Gender and age (*SD*) of the current sample.")
```

### Method
#### Participants
\hfill\break A total of `r nrow(exp1__age.gender)` subjects completed the online experiment (*M* = `r round(mean(exp1__age.gender$age, na.rm = T), 2)` years [*SD* = `r round(sd(exp1__age.gender$age, na.rm = T), 2)`]; see `r table_numbering("exp1__age.gender__table", display = "cite")` for a comprehensive breakdown of the sample). The current sample was primarily comprised of participants sourced from voluntary participation websites such as \href{https://www.prolific.co/}{Prolific Academic} (`r exp1__n.prolific`%)
(where they received payment at the rate of £5/hr) and via the in-school \href{https://keelepsychology.sona-systems.com/}{research participation system} (where they received course participation credits; `r exp1__n.sona`%). A small number of participants were also recruited from social media and other online sources (Facebook: `r exp1__n.facebook`%; Call For Participants: `r exp1__n.c4p`%; Reddit: `r exp1__n.reddit`%; unspecified: `r exp1__n.unspecified`%). To meet our YA requirements, all participants were required to be between 18-59 years of age (actual range: `r min(exp1__age.gender$age, na.rm = T)`-`r max(exp1__age.gender$age, na.rm = T)`). As our experiment involved English word stimuli, we also asked subjects whether English was their first language; the vast majority (`r exp1__n.eng.lang`%) reported that English was indeed their first language.  

`r table_numbering("exp1__age.gender__table")`
```{r echo=FALSE, cache=TRUE}
    exp1__age.gender__table %>%
...
##################################################################################################################
##  Two-way mixed ANOVA:
    exp1__prop.hit.aov <- afex::aov_ez('id', 'prop.hit', fun_aggregate = mean, exp1__prop.hit, 
                                       between = "resp_option", within = 'stim',
                                       anova_table=list(correction = "none", es = "ges"))
##  Mauchlys test of sphericity:
    exp1__prop.hit.aov__sphericity <- test_sphericity(exp1__prop.hit.aov)
...
##  RFG vs. RFBG:
    exp1__prop.hit__levene.test__rfg.rfbg <- exp1__prop.hit %>% 
        filter(resp_option == "RFG" | resp_option == "RFBG") %>% leveneTest(prop.hit ~ resp_option, data = .)
    ##  Levene's test significant (variances not equal), so perform corrected Welch two-sample t-test: 
    exp1__prop.hit__t.test__rfg.rfbg <- exp1__prop.hit %>% 
        filter(resp_option == "RFG" | resp_option == "RFBG") %>%
        t_test(prop.hit ~ resp_option, paired = FALSE, data = ., var.equal = FALSE)
    ##  Calculate effect size:
    exp1__prop.hit__t.test__rfg.rfbg__eff.size <- exp1__prop.hit %>% 
        filter(resp_option == "RFG" | resp_option == "RFBG") %>%
        rstatix::cohens_d(prop.hit ~ resp_option, var.equal = FALSE, paired = FALSE, ci = TRUE, conf.level = 0.95)
    
##  RFG vs. RF-Ratings:
    exp1__prop.hit__levene.test__rfg.rfr <- exp1__prop.hit %>% 
        filter(resp_option == "RFG" | resp_option == "RF_Ratings") %>% leveneTest(prop.hit ~ resp_option, data = .)
    ##  Levene's test significant (variances not equal), so perform corrected Welch two-sample t-test: 
    exp1__prop.hit__t.test__rfg.rfr <- exp1__prop.hit %>% 
        filter(resp_option == "RFG" | resp_option == "RF_Ratings") %>%
        t_test(prop.hit ~ resp_option, paired = FALSE, data = ., var.equal = FALSE)
    ##  Calculate effect size:
    exp1__prop.hit__t.test__rfg.rfr__eff.size <- exp1__prop.hit %>% 
        filter(resp_option == "RFG" | resp_option == "RF_Ratings") %>%
        rstatix::cohens_d(prop.hit ~ resp_option, var.equal = FALSE, paired = FALSE, ci = TRUE, conf.level = 0.95)    
    
##  RFBG vs. RF-Ratings:
    exp1__prop.hit__levene.test__rfbg.rfr <- exp1__prop.hit %>% 
        filter(resp_option == "RFBG" | resp_option == "RF_Ratings") %>% leveneTest(prop.hit ~ resp_option, data = .)
    ##  Levene's test NOT significant (variances equal), so perform un-corrected standard two-sample t-test: 
    exp1__prop.hit__t.test__rfbg.rfr <- exp1__prop.hit %>% 
        filter(resp_option == "RFBG" | resp_option == "RF_Ratings") %>%
        t_test(prop.hit ~ resp_option, paired = FALSE, data = ., var.equal = TRUE)
    ##  Calculate effect size:
    exp1__prop.hit__t.test__rfbg.rfr__eff.size <- exp1__prop.hit %>% 
        filter(resp_option == "RFBG" | resp_option == "RF_Ratings") %>%
...
##  Significant main effect of STIMULI FORMAT, so perform paired contrasts:
    exp1__prop.hit__pairwise__stim.format <- exp1__prop.hit %>% 
        pairwise_t_test(prop.hit ~ stim, paired = TRUE, p.adjust.method = "bonferroni")
##  Calculate effect size:      
    exp1__prop.hit__pairwise__stim.format__eff.size <- exp1__prop.hit %>% 
        rstatix::cohens_d(prop.hit ~ stim, paired = TRUE, ci = TRUE, conf.level = 0.95)   
##  Format for report (words vs. pics t-test):
    exp1__prop.hit__ttest__pic.vs.word <- str_c(ttest.writeup(exp1__prop.hit__pairwise__stim.format), 
        "; *d* = ", round(as.vector(exp1__prop.hit__pairwise__stim.format__eff.size %>% pull(effsize)), 2),
        ", 95% CI [", exp1__prop.hit__pairwise__stim.format__eff.size %>% pull(conf.low), ", ",
                      exp1__prop.hit__pairwise__stim.format__eff.size %>% pull(conf.high), "]")
##-----------------------------------------------------------------------------------------------------------------
##  Get all means:
    exp1__prop.hit__stim.means <- as.tibble(summary(emmeans(exp1__prop.hit.aov, ~stim)))
    exp1__prop.hit__resp.option.means <- as.tibble(summary(emmeans(exp1__prop.hit.aov, ~resp_option)))

##################################################################################################################
##  Exp1 FAs:                                                                                                  ##
##################################################################################################################
##  Two-way mixed ANOVA:
    exp1__prop.FA.aov <- afex::aov_ez('id', 'prop.FA', fun_aggregate = mean, exp1__prop.FA, 
                                       between = "resp_option", within = 'stim',
                                       anova_table=list(correction = "none", es = "ges"))
##  Mauchlys test of sphericity:
    exp1__prop.FA.aov__sphericity <- test_sphericity(exp1__prop.FA.aov)
##  Assumption of sphericity violated (p<.05), so apply Greenhouse-Geisser correction:
    exp1__prop.FA.aov <- afex::aov_ez('id', 'prop.FA', fun_aggregate = mean, exp1__prop.FA, 
                                       between = "resp_option", within = 'stim',
                                       anova_table=list(correction = "GG", es = "ges"))
...
##-----------------------------------------------------------------------------------------------------------------
##  Significant main effect of STIMULI FORMAT, so perform paired contrasts:
    exp1__prop.FA__pairwise__stim.format <- exp1__prop.FA %>% 
        pairwise_t_test(prop.FA ~ stim, paired = TRUE, p.adjust.method = "bonferroni")
##  Calculate effect size:      
    exp1__prop.FA__pairwise__stim.format__eff.size <- exp1__prop.FA %>% 
        rstatix::cohens_d(prop.FA ~ stim, paired = TRUE, ci = TRUE, conf.level = 0.95)   
##  Format for report (words vs. pics t-test):
    exp1__prop.FA__ttest__pic.vs.word <- str_c(ttest.writeup(exp1__prop.FA__pairwise__stim.format), 
        "; *d* = ", round(as.vector(exp1__prop.FA__pairwise__stim.format__eff.size %>% pull(effsize)), 2),
        ", 95% CI [", exp1__prop.FA__pairwise__stim.format__eff.size %>% pull(conf.low), ", ",
                      exp1__prop.FA__pairwise__stim.format__eff.size %>% pull(conf.high), "]")
##-----------------------------------------------------------------------------------------------------------------
##  Get all means:
    exp1__prop.FA__stim.means <- as.tibble(summary(emmeans(exp1__prop.FA.aov, ~stim)))
    exp1__prop.FA__resp.option.means <- as.tibble(summary(emmeans(exp1__prop.FA.aov, ~resp_option)))
    
##################################################################################################################
##  Exp1 d':                                                                                                    ##
##################################################################################################################
##  Two-way mixed ANOVA:
    exp1__d.prime.aov <- afex::aov_ez('id', 'd.prime', fun_aggregate = mean, exp1__d.prime, 
                                       between = "resp_option", within = 'stim',
                                       anova_table=list(correction = "none", es = "ges"))
##  Mauchlys test of sphericity:
    exp1__d.prime.aov__sphericity <- test_sphericity(exp1__d.prime.aov)
##  Assumption of sphericity violated (p<.05), so apply Greenhouse-Geisser correction:
    exp1__d.prime.aov <- afex::aov_ez('id', 'd.prime', fun_aggregate = mean, exp1__d.prime, 
                                       between = "resp_option", within = 'stim',
                                       anova_table=list(correction = "GG", es = "ges"))    
...
##-----------------------------------------------------------------------------------------------------------------
##  Significant main effect of STIMULI FORMAT, so perform paired contrasts:
    exp1__d.prime__pairwise__stim.format <- exp1__d.prime %>% 
        pairwise_t_test(d.prime ~ stim, paired = TRUE, p.adjust.method = "bonferroni")
##  Calculate effect size:      
    exp1__d.prime__pairwise__stim.format__eff.size <- exp1__d.prime %>% 
        rstatix::cohens_d(d.prime ~ stim, paired = TRUE, ci = TRUE, conf.level = 0.95)   
##  Format for report (words vs. pics t-test):
    exp1__d.prime__ttest__pic.vs.word <- str_c(ttest.writeup(exp1__d.prime__pairwise__stim.format), 
        "; *d* = ", round(as.vector(exp1__d.prime__pairwise__stim.format__eff.size %>% pull(effsize)), 2),
        ", 95% CI [", exp1__d.prime__pairwise__stim.format__eff.size %>% pull(conf.low), ", ",
                      exp1__d.prime__pairwise__stim.format__eff.size %>% pull(conf.high), "]")
##-----------------------------------------------------------------------------------------------------------------
##  Get all means:
    exp1__d.prime__stim.means <- as.tibble(summary(emmeans(exp1__d.prime.aov, ~stim)))
    exp1__d.prime__resp.option.means <- as.tibble(summary(emmeans(exp1__d.prime.aov, ~resp_option)))
    
##################################################################################################################
##  Exp1 Hits, FAs, d' table:                                                                                   ##
##################################################################################################################
    exp1__prop.hit.FA.d.prime__table <- tribble(
        ~"Stim", ~Hits,  ~FAs, ~"d'",
        "Words",
            round(exp1__prop.hit__stim.means %>% filter(stim == "word") %>% pull(emmean),2),
            round(exp1__prop.FA__stim.means %>% filter(stim == "word") %>% pull(emmean),2),
            round(exp1__d.prime__stim.means %>% filter(stim == "word") %>% pull(emmean),2),
        "Pictures",  
            round(exp1__prop.hit__stim.means %>% filter(stim == "pic") %>% pull(emmean),2),
            round(exp1__prop.FA__stim.means %>% filter(stim == "pic") %>% pull(emmean),2),
            round(exp1__d.prime__stim.means %>% filter(stim == "pic") %>% pull(emmean),2),
        "RFG",
            round(exp1__prop.hit__resp.option.means %>% filter(resp_option == "RFG") %>% pull(emmean),2),
            round(exp1__prop.FA__resp.option.means %>% filter(resp_option == "RFG") %>% pull(emmean),2),
            round(exp1__d.prime__resp.option.means %>% filter(resp_option == "RFG") %>% pull(emmean),2),
        "RFBG",
            round(exp1__prop.hit__resp.option.means %>% filter(resp_option == "RFBG") %>% pull(emmean),2),
            round(exp1__prop.FA__resp.option.means %>% filter(resp_option == "RFBG") %>% pull(emmean),2),
            round(exp1__d.prime__resp.option.means %>% filter(resp_option == "RFBG") %>% pull(emmean),2),
        "RF-Ratings",
            round(exp1__prop.hit__resp.option.means %>% filter(resp_option == "RF_Ratings") %>% pull(emmean),2),
            round(exp1__prop.FA__resp.option.means %>% filter(resp_option == "RF_Ratings") %>% pull(emmean),2),
            round(exp1__d.prime__resp.option.means %>% filter(resp_option == "RF_Ratings") %>% pull(emmean),2)) %>%
        column_to_rownames(var = "Stim")
```

```{r echo=FALSE, results = FALSE, cache=FALSE}
table_numbering(name = "exp1__prop.hit.FA.d.prime__table", caption = "Mean proportion of hits, FAs, and mean *d'* scores, by stimuli format and response option condition. Signif. codes:  \\*\\*\\**p* < .001; \\*\\**p* < .01; \\**p* < .05; \\+ involved in significant interaction.")
```
#### Picture superiority
To establish baseline picture superiority effects in the current paradigm, and assess whether there were any interactions with the availability of different response options options at test, a series of 2 (stimuli format: words, pictures) x 3 (response option option condition: RFG-judgements, RFBG-judgements, RF-ratings) mixed ANOVAs were conducted on a number of outcome variables. Namely, the signal detection measures of *d'* (sensitivity) and *c* (decision criterion), as well as the proportion of overall hits, false alarms (FAs), and overall recognition (hits - FAs) [see `r table_numbering("exp1__prop.hit.FA.d.prime__table", display = "cite")`]. Significant main effects and interaction effects were followed-up with Bonferroni-adjusted pairwise comparisons.

The ANOVA on *d'* scores demonstrated a significant main effect of stimuli-format, `r apa::anova_apa(exp1__d.prime.aov, effect = "stim", format = "rmarkdown", print = FALSE)`, a PSE was evident, with pictures (*M*= `r round(exp1__d.prime__stim.means %>% filter(stim == "pic") %>% pull(emmean),2)`) producing significantly better discrimination between hits and FAs than words (*M*= `r round(exp1__d.prime__stim.means %>% filter(stim == "word") %>% pull(emmean),2)`), `r exp1__d.prime__ttest__pic.vs.word`. The ANOVAs on the proportion of hits, FAs, and overall recognition also produced findings consistent with a PSE. For hits, there was a significant main effect of stimuli-format, `r apa::anova_apa(exp1__prop.hit.aov, effect = "stim", format = "rmarkdown", print = FALSE)`, with pictures (*M*= `r round(exp1__prop.hit__stim.means %>% filter(stim == "pic") %>% pull(emmean),2)`) showing a higher number of overall hits compared to words (*M*= `r round(exp1__prop.hit__stim.means %>% filter(stim == "word") %>% pull(emmean),2)`), `r exp1__prop.hit__ttest__pic.vs.word`. Similarly, the ANOVA on the proportion of FAs showed a significant main effect of stimuli-format, `r apa::anova_apa(exp1__prop.FA.aov, effect = "stim", format = "rmarkdown", print = FALSE)`, with pictures (*M*= `r round(exp1__prop.FA__stim.means %>% filter(stim == "pic") %>% pull(emmean),2)`) producing significantly fewer FAs than words (*M*= `r round(exp1__prop.FA__stim.means %>% filter(stim == "word") %>% pull(emmean),2)`), `r exp1__prop.FA__ttest__pic.vs.word`. No interaction effects were found between stimuli format and response option for any of the variables. 
...
`r table_numbering("exp1__prop.hit.FA.d.prime__table")`
```{r echo=FALSE, cache=TRUE}
    exp1__prop.hit.FA.d.prime__table %>%
...
##################################################################################################################
##  Two-way mixed ANOVA:
    exp1__prop.hit.R.aov <- afex::aov_ez('id', 'prop.hit.R', fun_aggregate = mean, exp1__prop.hit.R,
                                         between = "resp_option", within = 'stim',
                                         anova_table=list(correction = "none", es = "ges"))
##  Mauchlys test of sphericity:
    exp1__prop.hit.R.aov__sphericity <- test_sphericity(exp1__prop.hit.R.aov)
##  Assumption of sphericity violated (p<.05), so apply Greenhouse-Geisser correction:
    exp1__prop.hit.R.aov <- afex::aov_ez('id', 'prop.hit.R', fun_aggregate = mean, exp1__prop.hit.R, 
                                       between = "resp_option", within = 'stim',
                                       anova_table=list(correction = "GG", es = "ges"))
...
##  Significant INTERACTION, so perform paired contrasts:
    exp1__prop.hit.R__pairs__interaction <- 
        pairs(emmeans(exp1__prop.hit.R.aov, ~ resp_option:stim), adjust = "bonferroni")
##  Get all means:
    exp1__prop.hit.R__stim.means <- as.tibble(summary(emmeans(exp1__prop.hit.R.aov, ~stim)))
    exp1__prop.hit.R__resp.option.means <- as.tibble(summary(emmeans(exp1__prop.hit.R.aov, ~resp_option)))    
    exp1__prop.hit.R__interaction.means <- as.tibble(summary(emmeans(exp1__prop.hit.R.aov, ~stim:resp_option))) %>%
...
##  Create labels for figure (replaces plot legend):
    exp1__prop.hit.R__interaction.figure.labels <- exp1__prop.hit.R__interaction.means %>%
        select(stim, resp_option, emmean) %>%
        filter(stim == "Pictures") %>%
        mutate(label = case_when(
                  resp_option == "RFG" ~ "RFG",
                  resp_option == "RFBG" ~ "RFBG",
                  resp_option == "RF_Ratings" ~ "RF_Ratings")) %>%
        group_by(stim, resp_option, label)
##  Interaction figure:
    exp1__prop.hit.R__interaction.figure <- exp1__prop.hit.R__interaction.means %>%
        select(stim, resp_option, emmean) %>%
        ggplot() +
            ...
            geom_text(size = 4, aes(label = label, color = resp_option), nudge_x = 0.08, hjust = 0,
                      data = exp1__prop.hit.R__interaction.figure.labels) +
...
To determine the impact of stimuli format on the rates of R and F, additional 2 (words, pictures) x 3 (RFG-judgements, RFBG-judgements, RF-ratings) mixed ANOVAs were conducted on the mean proportion of hits (see `r figure_numbering("exp1__prop.hit__figure", display = "cite")`) and FAs (see `r figure_numbering("exp1__prop.FA__figure", display = "cite")`) assigned R, F, and G. 


```{r echo=FALSE, results = FALSE, cache=FALSE}
figure_numbering(name = "exp1__prop.hit.R__interaction.figure", caption = "Interaction plot between stimuli format and response option for the mean proportion of hits assigned Recollection.")
```
**Recollection (hits):** For R hits, there was a significant interaction between stimuli format and response option option condition, `r apa::anova_apa(exp1__prop.hit.R.aov, effect = "resp_option:stim", format = "rmarkdown", print = FALSE)` (see `r figure_numbering("exp1__prop.hit.R__interaction.figure", display = "cite")`).

```{r fig.width=7, fig.height=4, echo=FALSE, cache=TRUE}
    exp1__prop.hit.R__interaction.figure
```
`r figure_numbering("exp1__prop.hit.R__interaction.figure")`

Pictures produced a higher proportion of hits assigned *Recollection* than words in the RFG group (words [*M*= `r round(exp1__prop.hit.R__interaction.means %>% filter(stim == "Words" & resp_option == "RFG") %>% pull(emmean),2)`] vs. pictures [*M*= `r round(exp1__prop.hit.R__interaction.means %>% filter(stim == "Pictures" & resp_option == "RFG") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.R__pairs__interaction)$statistic$RFGword_RFGpic%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`), RFBG group (words [*M*= `r round(exp1__prop.hit.R__interaction.means %>% filter(stim == "Words" & resp_option == "RFBG") %>% pull(emmean),2)`] vs. pictures [*M*= `r round(exp1__prop.hit.R__interaction.means %>% filter(stim == "Pictures" & resp_option == "RFBG") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.R__pairs__interaction)$statistic$RFBGword_RFBGpic%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`) and RF-Ratings group (words [*M*= `r round(exp1__prop.hit.R__interaction.means %>% filter(stim == "Words" & resp_option == "RF_Ratings") %>% pull(emmean),2)`] vs. pictures [*M*= `r round(exp1__prop.hit.R__interaction.means %>% filter(stim == "Pictures" & resp_option == "RF_Ratings") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.R__pairs__interaction)$statistic$RF_Ratingsword_RF_Ratingspic%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`). 
 
The interaction is evident following comparisons of the same stimuli format across response option conditions. For words, the proportion of R hits was significantly higher in the RF-Ratings group compared to both the RFG group 
(RFG [*M* = `r round(exp1__prop.hit.R__interaction.means %>% filter(resp_option == "RFG" & stim == "Words") %>% pull(emmean),2)`] vs. RF-Ratings [*M* = `r round(exp1__prop.hit.R__interaction.means %>% filter(resp_option == "RF_Ratings" & stim == "Words") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.R__pairs__interaction)$statistic$RF_Ratingsword_RFGword%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`) and the the RFBG group (RFBG [*M* = `r round(exp1__prop.hit.R__interaction.means %>% filter(resp_option == "RFBG" & stim == "Words") %>% pull(emmean),2)`] vs. RF-Ratings [*M* = `r round(exp1__prop.hit.R__interaction.means %>% filter(resp_option == "RF_Ratings" & stim == "Words") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.R__pairs__interaction)$statistic$RF_Ratingsword_RFBGword%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`). The RFG and RFBG groups did not significantly differ in the proportion of word hits assigned *Recollection* (RFG [*M* = `r round(exp1__prop.hit.R__interaction.means %>% filter(resp_option == "RFG" & stim == "Words") %>% pull(emmean),2)`] vs. RFBG [*M* = `r round(exp1__prop.hit.R__interaction.means %>% filter(resp_option == "RFBG" & stim == "Words") %>% pull(emmean),2)`],  `r papaja::apa_print(exp1__prop.hit.R__pairs__interaction)$statistic$RFBGword_RFGword%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`). 

For pictures, the RF-Ratings group again showed a significantly higher proportion of R hits than the RFBG group (RFBG [*M* = `r round(exp1__prop.hit.R__interaction.means %>% filter(resp_option == "RFBG" & stim == "Pictures") %>% pull(emmean),2)`] vs. RF-Ratings [*M* = `r round(exp1__prop.hit.R__interaction.means %>% filter(resp_option == "RF_Ratings" & stim == "Pictures") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.R__pairs__interaction)$statistic$RF_Ratingspic_RFBGpic%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`). While words produced a significant difference between the RFG and RF-Ratings group for R hits, the same was not found for pictures (RFG [*M* = `r round(exp1__prop.hit.R__interaction.means %>% filter(resp_option == "RFG" & stim == "Pictures") %>% pull(emmean),2)`] vs. RF-Ratings [*M* = `r round(exp1__prop.hit.R__interaction.means %>% filter(resp_option == "RF_Ratings" & stim == "Pictures") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.R__pairs__interaction)$statistic$RF_Ratingspic_RFGpic%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`). Instead, the RFG group also produced a significantly higher proportion of R hits than the RFBG group (RFG [*M* = `r round(exp1__prop.hit.R__interaction.means %>% filter(resp_option == "RFG" & stim == "Pictures") %>% pull(emmean),2)`] vs. RFBG [*M* = `r round(exp1__prop.hit.R__interaction.means %>% filter(resp_option == "RFBG" & stim == "Pictures") %>% pull(emmean),2)`],  `r papaja::apa_print(exp1__prop.hit.R__pairs__interaction)$statistic$RFBGpic_RFGpic%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`).
...
################################################################################################################## 
##  Create labels for figure (replaces plot legend):
    exp1__prop.hit.F__interaction.figure.labels <- exp1__prop.hit.F__interaction.means %>%
        select(stim, resp_option, emmean) %>%
        filter(stim == "Pictures") %>%
        mutate(label = case_when(
                  resp_option == "RFG" ~ "RFG",
        ...
        group_by(stim, resp_option, label)
##  Interaction figure:
    exp1__prop.hit.F__interaction.figure <- exp1__prop.hit.F__interaction.means %>%
        select(stim, resp_option, emmean) %>%
        ggplot() +
            aes(x = stim, color = resp_option, group = resp_option, y = emmean) +
            stat_summary(fun.y = mean, geom = "point", size = 2.4, shape = 15) +
            stat_summary(fun.y = mean, geom = "line", size = 1.2) +
            geom_text(size = 4, aes(label = label, color = resp_option), nudge_x = 0.08, hjust = 0,
                      data = exp1__prop.hit.F__interaction.figure.labels) +
            scale_color_manual(values = c('#8ec6c5', '#7d9ab5', '#8566aa')) +
...
```{r echo=FALSE, results = FALSE, cache=FALSE}
figure_numbering(name = "exp1__prop.hit.F__interaction.figure", caption = "Interaction plot between stimuli format and response option for the mean proportion of hits assigned Recollection.")
```
**Familiarity (hits):** For F hits, there was a significant interaction between stimuli format and response option option condition, `r apa::anova_apa(exp1__prop.hit.F.aov, effect = "resp_option:stim", format = "rmarkdown", print = FALSE)` (see `r figure_numbering("exp1__prop.hit.F__interaction.figure", display = "cite")`).

```{r fig.width=7, fig.height=4, echo=FALSE, cache=TRUE}
    exp1__prop.hit.F__interaction.figure
```
`r figure_numbering("exp1__prop.hit.F__interaction.figure")`

Pictures produced a higher proportion of hits assigned *Familiarity* than words in the RF-Ratings group only (words [*M*= `r round(exp1__prop.hit.F__interaction.means %>% filter(stim == "Words" & resp_option == "RF_Ratings") %>% pull(emmean),2)`] vs. pictures [*M*= `r round(exp1__prop.hit.F__interaction.means %>% filter(stim == "Pictures" & resp_option == "RF_Ratings") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.F__pairs__interaction)$statistic$RF_Ratingsword_RF_Ratingspic%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`). In both the RFG and RFBG groups, words produced a higher proportion of hits assigned *Familiarity* than pictures (RFG: words [*M*= `r round(exp1__prop.hit.F__interaction.means %>% filter(stim == "Words" & resp_option == "RFG") %>% pull(emmean),2)`] vs. pictures [*M*= `r round(exp1__prop.hit.F__interaction.means %>% filter(stim == "Pictures" & resp_option == "RFG") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.F__pairs__interaction)$statistic$RFGword_RFGpic%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`; RFBG: words [*M*= `r round(exp1__prop.hit.F__interaction.means %>% filter(stim == "Words" & resp_option == "RFBG") %>% pull(emmean),2)`] vs. pictures [*M*= `r round(exp1__prop.hit.F__interaction.means %>% filter(stim == "Pictures" & resp_option == "RFBG") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.F__pairs__interaction)$statistic$RFBGword_RFBGpic%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`).
 
Comparisons of the same stimuli format across response option conditions showed that, for words, the proportion of F hits was significantly higher in the RF-Ratings group compared to both the RFG group (RFG [*M* = `r round(exp1__prop.hit.F__interaction.means %>% filter(resp_option == "RFG" & stim == "Words") %>% pull(emmean),2)`] vs. RF-Ratings [*M* = `r round(exp1__prop.hit.F__interaction.means %>% filter(resp_option == "RF_Ratings" & stim == "Words") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.F__pairs__interaction)$statistic$RF_Ratingsword_RFGword%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`) and the the RFBG group (RFBG [*M* = `r round(exp1__prop.hit.F__interaction.means %>% filter(resp_option == "RFBG" & stim == "Words") %>% pull(emmean),2)`] vs. RF-Ratings [*M* = `r round(exp1__prop.hit.F__interaction.means %>% filter(resp_option == "RF_Ratings" & stim == "Words") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.F__pairs__interaction)$statistic$RF_Ratingsword_RFBGword%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`). The RFG and RFBG groups did not significantly differ in the proportion of word hits assigned *Familiarity* (RFG [*M* = `r round(exp1__prop.hit.F__interaction.means %>% filter(resp_option == "RFG" & stim == "Words") %>% pull(emmean),2)`] vs. RFBG [*M* = `r round(exp1__prop.hit.F__interaction.means %>% filter(resp_option == "RFBG" & stim == "Words") %>% pull(emmean),2)`],  `r papaja::apa_print(exp1__prop.hit.F__pairs__interaction)$statistic$RFBGword_RFGword%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`). 

For pictures, the RF-Ratings group again showed a significantly higher proportion of F hits than both the RFG group (RFG [*M* = `r round(exp1__prop.hit.F__interaction.means %>% filter(resp_option == "RFG" & stim == "Pictures") %>% pull(emmean),2)`] vs. RF-Ratings [*M* = `r round(exp1__prop.hit.F__interaction.means %>% filter(resp_option == "RF_Ratings" & stim == "Pictures") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.F__pairs__interaction)$statistic$RF_Ratingspic_RFGpic%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`) and the RFBG group (RFBG [*M* = `r round(exp1__prop.hit.F__interaction.means %>% filter(resp_option == "RFBG" & stim == "Pictures") %>% pull(emmean),2)`] vs. RF-Ratings [*M* = `r round(exp1__prop.hit.F__interaction.means %>% filter(resp_option == "RF_Ratings" & stim == "Pictures") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.F__pairs__interaction)$statistic$RF_Ratingspic_RFBGpic%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`). Similar to words, the RFG and RFBG groups did not significantly differ in the proportion of F hits produced by pictures (RFG [*M* = `r round(exp1__prop.hit.F__interaction.means %>% filter(resp_option == "RFG" & stim == "Pictures") %>% pull(emmean),2)`] vs. RFBG [*M* = `r round(exp1__prop.hit.F__interaction.means %>% filter(resp_option == "RFBG" & stim == "Pictures") %>% pull(emmean),2)`],  `r papaja::apa_print(exp1__prop.hit.F__pairs__interaction)$statistic$RFBGpic_RFGpic%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`).
...
##----------------------------------------------------------------------------------------------------------------- 
##  Significant INTERACTION, so perform paired contrasts:
    exp1__prop.hit.G__pairs__interaction <- 
        pairs(emmeans(exp1__prop.hit.G.aov, ~ resp_option:stim), adjust = "bonferroni")
##  Get all means:
    exp1__prop.hit.G__stim.means <- as.tibble(summary(emmeans(exp1__prop.hit.G.aov, ~stim)))
    exp1__prop.hit.G__resp.option.means <- as.tibble(summary(emmeans(exp1__prop.hit.G.aov, ~resp_option)))    
    exp1__prop.hit.G__interaction.means <- as.tibble(summary(emmeans(exp1__prop.hit.G.aov, ~stim:resp_option))) %>%
        mutate(stim = str_replace(stim, "word", "Words")) %>%
        mutate(stim = str_replace(stim, "pic", "Pictures")) %>%
        transform(stim = factor(stim, levels=c("Words","Pictures")))
##################################################################################################################
##  Prop G Hits interaction figure:                                                                             ##
################################################################################################################## 
##  Create labels for figure (replaces plot legend):
    exp1__prop.hit.G__interaction.figure.labels <- exp1__prop.hit.G__interaction.means %>%
        select(stim, resp_option, emmean) %>%
        filter(stim == "Pictures") %>%
        mutate(label = case_when(
                  resp_option == "RFG" ~ "RFG",
        ...
        group_by(stim, resp_option, label)
##  Interaction figure:
    exp1__prop.hit.G__interaction.figure <- exp1__prop.hit.G__interaction.means %>%
        select(stim, resp_option, emmean) %>%
        ggplot() +
            aes(x = stim, color = resp_option, group = resp_option, y = emmean) +
            stat_summary(fun.y = mean, geom = "point", size = 2.4, shape = 15) +
            stat_summary(fun.y = mean, geom = "line", size = 1.2) +
            geom_text(size = 4, aes(label = label, color = resp_option), nudge_x = 0.08, hjust = 0,
                      data = exp1__prop.hit.G__interaction.figure.labels) +
            scale_color_manual(values = c('#8ec6c5', '#7d9ab5', '#8566aa')) +
            labs(x = "Stimuli format", y = "Mean proportion of hits assigned Familiarity") +
            scale_y_continuous(limits = c(0, 3.5),
                               breaks = c(0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5),
...
```{r echo=FALSE, results = FALSE, cache=FALSE}
figure_numbering(name = "exp1__prop.hit.G__interaction.figure", caption = "Interaction plot between stimuli format and response option for the mean proportion of hits assigned Recollection.")
```
**Guessing (hits):** For G hits, there was a significant interaction between stimuli format and response option option condition, `r apa::anova_apa(exp1__prop.hit.G.aov, effect = "resp_option:stim", format = "rmarkdown", print = FALSE)` (see `r figure_numbering("exp1__prop.hit.G__interaction.figure", display = "cite")`).

```{r fig.width=7, fig.height=4, echo=FALSE, cache=TRUE}
    exp1__prop.hit.G__interaction.figure
```
`r figure_numbering("exp1__prop.hit.G__interaction.figure")`

Words produced a higher proportion of hits assigned *Guessing* than pictures in both the RFG group (words [*M*= `r round(exp1__prop.hit.G__interaction.means %>% filter(stim == "Words" & resp_option == "RFG") %>% pull(emmean),2)`] vs. pictures [*M*= `r round(exp1__prop.hit.G__interaction.means %>% filter(stim == "Pictures" & resp_option == "RFG") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.G__pairs__interaction)$statistic$RFGword_RFGpic%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`) and RFBG group [*M*= `r round(exp1__prop.hit.G__interaction.means %>% filter(stim == "Words" & resp_option == "RFBG") %>% pull(emmean),2)`] vs. pictures [*M*= `r round(exp1__prop.hit.G__interaction.means %>% filter(stim == "Pictures" & resp_option == "RFBG") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.G__pairs__interaction)$statistic$RFBGword_RFBGpic%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`). Words and pictures did not significantly differ in RF-Ratings group (words [*M*= `r round(exp1__prop.hit.G__interaction.means %>% filter(stim == "Words" & resp_option == "RF_Ratings") %>% pull(emmean),2)`] vs. pictures [*M*= `r round(exp1__prop.hit.G__interaction.means %>% filter(stim == "Pictures" & resp_option == "RF_Ratings") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.G__pairs__interaction)$statistic$RF_Ratingsword_RF_Ratingspic%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`). 

Comparisons of the same stimuli format across response option conditions showed that, for words, the proportion of G hits was significantly lower in the RF-Ratings group compared to both the RFG group (RFG [*M* = `r round(exp1__prop.hit.G__interaction.means %>% filter(resp_option == "RFG" & stim == "Words") %>% pull(emmean),2)`] vs. RF-Ratings [*M* = `r round(exp1__prop.hit.G__interaction.means %>% filter(resp_option == "RF_Ratings" & stim == "Words") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.G__pairs__interaction)$statistic$RF_Ratingsword_RFGword%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`) and the RFBG group (RFBG [*M* = `r round(exp1__prop.hit.G__interaction.means %>% filter(resp_option == "RFBG" & stim == "Words") %>% pull(emmean),2)`] vs. RF-Ratings [*M* = `r round(exp1__prop.hit.G__interaction.means %>% filter(resp_option == "RF_Ratings" & stim == "Words") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.G__pairs__interaction)$statistic$RF_Ratingsword_RFBGword%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`). The RFG and RFBG groups did not significantly differ in the proportion of word hits assigned *Guessing* (RFG [*M* = `r round(exp1__prop.hit.G__interaction.means %>% filter(resp_option == "RFG" & stim == "Words") %>% pull(emmean),2)`] vs. RFBG [*M* = `r round(exp1__prop.hit.G__interaction.means %>% filter(resp_option == "RFBG" & stim == "Words") %>% pull(emmean),2)`],  `r papaja::apa_print(exp1__prop.hit.G__pairs__interaction)$statistic$RFBGword_RFGword%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`). 

For pictures, the RF-Ratings group again showed a significantly lower proportion of G hits than the RFBG group (RFBG [*M* = `r round(exp1__prop.hit.G__interaction.means %>% filter(resp_option == "RFBG" & stim == "Pictures") %>% pull(emmean),2)`] vs. RF-Ratings [*M* = `r round(exp1__prop.hit.G__interaction.means %>% filter(resp_option == "RF_Ratings" & stim == "Pictures") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.G__pairs__interaction)$statistic$RF_Ratingspic_RFBGpic%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`), however, the comparison with the RFG group did not reach significance (RFG [*M* = `r round(exp1__prop.hit.G__interaction.means %>% filter(resp_option == "RFG" & stim == "Pictures") %>% pull(emmean),2)`] vs. RF-Ratings [*M* = `r round(exp1__prop.hit.G__interaction.means %>% filter(resp_option == "RF_Ratings" & stim == "Pictures") %>% pull(emmean),2)`], `r papaja::apa_print(exp1__prop.hit.G__pairs__interaction)$statistic$RF_Ratingspic_RFGpic%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`). Again, the RFG and RFBG groups did not significantly differ 
in the proportion of G hits produced by pictures (RFG [*M* = `r round(exp1__prop.hit.G__interaction.means %>% filter(resp_option == "RFG" & stim == "Pictures") %>% pull(emmean),2)`] vs. RFBG [*M* = `r round(exp1__prop.hit.G__interaction.means %>% filter(resp_option == "RFBG" & stim == "Pictures") %>% pull(emmean),2)`],  `r papaja::apa_print(exp1__prop.hit.G__pairs__interaction)$statistic$RFBGpic_RFGpic%>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*")`).
...
##################################################################################################################
##  Two-way mixed ANOVA:
    exp1__prop.FA.R.aov <- afex::aov_ez('id', 'prop.FA.R', fun_aggregate = mean, exp1__prop.FA.R,
                                         between = "resp_option", within = 'stim',
                                         anova_table=list(correction = "none", es = "ges"))
##  Mauchlys test of sphericity:
    exp1__prop.FA.R.aov__sphericity <- test_sphericity(exp1__prop.FA.R.aov)
##  Assumption of sphericity violated (p<.05), so apply Greenhouse-Geisser correction:
    exp1__prop.FA.R.aov <- afex::aov_ez('id', 'prop.FA.R', fun_aggregate = mean, exp1__prop.FA.R, 
                                       between = "resp_option", within = 'stim',
                                       anova_table=list(correction = "GG", es = "ges"))
...
##  Significant main effect of RESPONSE OPTION, so first check if variances are equal using Levene's test:
##  RFG vs. RFBG:
    exp1__prop.FA.R__levene.test__rfg.rfbg <- exp1__prop.FA.R %>% 
        filter(resp_option == "RFG" | resp_option == "RFBG") %>% leveneTest(prop.FA.R ~ resp_option, data = .)
    ##  Levene's test NOT significant (variances equal), so perform un-corrected standard two-sample t-test: 
    exp1__prop.FA.R__t.test__rfg.rfbg <- exp1__prop.FA.R %>% 
        filter(resp_option == "RFG" | resp_option == "RFBG") %>%
        t_test(prop.FA.R ~ resp_option, paired = FALSE, data = ., var.equal = TRUE)
    ##  Calculate effect size:
    exp1__prop.FA.R__t.test__rfg.rfbg__eff.size <- exp1__prop.FA.R %>% 
        filter(resp_option == "RFG" | resp_option == "RFBG") %>%
        rstatix::cohens_d(prop.FA.R ~ resp_option, var.equal = TRUE, paired = FALSE, ci = TRUE, conf.level = 0.95)
    
##  RFG vs. RF-Ratings:
    exp1__prop.FA.R__levene.test__rfg.rfr <- exp1__prop.FA.R %>% 
        filter(resp_option == "RFG" | resp_option == "RF_Ratings") %>% leveneTest(prop.FA.R ~ resp_option, data = .)
    ##  Levene's test significant (variances not equal), so perform corrected Welch two-sample t-test: 
    exp1__prop.FA.R__t.test__rfg.rfr <- exp1__prop.FA.R %>% 
        filter(resp_option == "RFG" | resp_option == "RF_Ratings") %>%
        t_test(prop.FA.R ~ resp_option, paired = FALSE, data = ., var.equal = FALSE)
    ##  Calculate effect size:
    exp1__prop.FA.R__t.test__rfg.rfr__eff.size <- exp1__prop.FA.R %>% 
        filter(resp_option == "RFG" | resp_option == "RF_Ratings") %>%
        rstatix::cohens_d(prop.FA.R ~ resp_option, var.equal = FALSE, paired = FALSE, ci = TRUE, conf.level = 0.95) 
    
##  RFBG vs. RF-Ratings:
    exp1__prop.FA.R__levene.test__rfbg.rfr <- exp1__prop.FA.R %>% 
        filter(resp_option == "RFBG" | resp_option == "RF_Ratings") %>% leveneTest(prop.FA.R ~ resp_option, data = .)
    ##  Levene's test significant (variances not equal), so perform corrected Welch two-sample t-test: 
    exp1__prop.FA.R__t.test__rfbg.rfr <- exp1__prop.FA.R %>% 
        filter(resp_option == "RFBG" | resp_option == "RF_Ratings") %>%
        t_test(prop.FA.R ~ resp_option, paired = FALSE, data = ., var.equal = FALSE)
    ##  Calculate effect size:
    exp1__prop.FA.R__t.test__rfbg.rfr__eff.size <- exp1__prop.FA.R %>% 
        filter(resp_option == "RFBG" | resp_option == "RF_Ratings") %>%
        rstatix::cohens_d(prop.FA.R ~ resp_option, var.equal = FALSE, paired = FALSE, ci = TRUE, conf.level = 0.95) 
    ##  Get all means:
    exp1__prop.FA.R__stim.means <- as.tibble(summary(emmeans(exp1__prop.FA.R.aov, ~stim)))
    exp1__prop.FA.R__resp.option.means <- as.tibble(summary(emmeans(exp1__prop.FA.R.aov, ~resp_option)))
##################################################################################################################
##  Exp1 Prop F FAs:                                                                                            ##
##################################################################################################################
##  Two-way mixed ANOVA:
    exp1__prop.FA.F.aov <- afex::aov_ez('id', 'prop.FA.F', fun_aggregate = mean, exp1__prop.FA.F,
                                         between = "resp_option", within = 'stim',
                                         anova_table=list(correction = "none", es = "ges"))
##  Mauchlys test of sphericity:
    exp1__prop.FA.F.aov__sphericity <- test_sphericity(exp1__prop.FA.F.aov)
##  Assumption of sphericity violated (p<.05), so apply Greenhouse-Geisser correction:
    exp1__prop.FA.F.aov <- afex::aov_ez('id', 'prop.FA.F', fun_aggregate = mean, exp1__prop.FA.F, 
                                       between = "resp_option", within = 'stim',
                                       anova_table=list(correction = "GG", es = "ges"))
    ##  Get all means:
    exp1__prop.FA.F__stim.means <- as.tibble(summary(emmeans(exp1__prop.FA.F.aov, ~stim)))
    exp1__prop.FA.F__resp.option.means <- as.tibble(summary(emmeans(exp1__prop.FA.F.aov, ~resp_option)))    
    
##################################################################################################################
##  Exp1 Prop G FAs:                                                                                            ##
##################################################################################################################
##  Two-way mixed ANOVA:
    exp1__prop.FA.G.aov <- afex::aov_ez('id', 'prop.FA.G', fun_aggregate = mean, exp1__prop.FA.G,
                                         between = "resp_option", within = 'stim',
                                         anova_table=list(correction = "none", es = "ges"))
##  Mauchlys test of sphericity:
    exp1__prop.FA.G.aov__sphericity <- test_sphericity(exp1__prop.FA.G.aov)
##  Assumption of sphericity violated (p<.05), so apply Greenhouse-Geisser correction:
    exp1__prop.FA.G.aov <- afex::aov_ez('id', 'prop.FA.G', fun_aggregate = mean, exp1__prop.FA.G, 
                                       between = "resp_option", within = 'stim',
                                       anova_table=list(correction = "GG", es = "ges"))        
##  Significant main effect of RESPONSE OPTION, so first check if variances are equal using Levene's test:
##  RFG vs. RFBG:
    exp1__prop.FA.G__levene.test__rfg.rfbg <- exp1__prop.FA.G %>% 
        filter(resp_option == "RFG" | resp_option == "RFBG") %>% leveneTest(prop.FA.G ~ resp_option, data = .)
    ##  Levene's test NOT significant (variances equal), so perform un-corrected standard two-sample t-test: 
    exp1__prop.FA.G__t.test__rfg.rfbg <- exp1__prop.FA.G %>% 
        filter(resp_option == "RFG" | resp_option == "RFBG") %>%
        t_test(prop.FA.G ~ resp_option, paired = FALSE, data = ., var.equal = TRUE)
    ##  Calculate effect size:
    exp1__prop.FA.G__t.test__rfg.rfbg__eff.size <- exp1__prop.FA.G %>% 
        filter(resp_option == "RFG" | resp_option == "RFBG") %>%
        rstatix::cohens_d(prop.FA.G ~ resp_option, var.equal = TRUE, paired = FALSE, ci = TRUE, conf.level = 0.95)
    
##  RFG vs. RF-Ratings:
    exp1__prop.FA.G__levene.test__rfg.rfr <- exp1__prop.FA.G %>% 
        filter(resp_option == "RFG" | resp_option == "RF_Ratings") %>% leveneTest(prop.FA.G ~ resp_option, data = .)
    ##  Levene's test significant (variances not equal), so perform corrected Welch two-sample t-test: 
    exp1__prop.FA.G__t.test__rfg.rfr <- exp1__prop.FA.G %>% 
        filter(resp_option == "RFG" | resp_option == "RF_Ratings") %>%
        t_test(prop.FA.G ~ resp_option, paired = FALSE, data = ., var.equal = FALSE)
    ##  Calculate effect size:
    exp1__prop.FA.G__t.test__rfg.rfr__eff.size <- exp1__prop.FA.G %>% 
        filter(resp_option == "RFG" | resp_option == "RF_Ratings") %>%
        rstatix::cohens_d(prop.FA.G ~ resp_option, var.equal = FALSE, paired = FALSE, ci = TRUE, conf.level = 0.95) 
    
##  RFBG vs. RF-Ratings:
    exp1__prop.FA.G__levene.test__rfbg.rfr <- exp1__prop.FA.G %>% 
        filter(resp_option == "RFBG" | resp_option == "RF_Ratings") %>% leveneTest(prop.FA.G ~ resp_option, data = .)
    ##  Levene's test significant (variances not equal), so perform corrected Welch two-sample t-test: 
    exp1__prop.FA.G__t.test__rfbg.rfr <- exp1__prop.FA.G %>% 
        filter(resp_option == "RFBG" | resp_option == "RF_Ratings") %>%
        t_test(prop.FA.G ~ resp_option, paired = FALSE, data = ., var.equal = FALSE)
    ##  Calculate effect size:
    exp1__prop.FA.G__t.test__rfbg.rfr__eff.size <- exp1__prop.FA.G %>% 
        filter(resp_option == "RFBG" | resp_option == "RF_Ratings") %>%
        rstatix::cohens_d(prop.FA.G ~ resp_option, var.equal = FALSE, paired = FALSE, ci = TRUE, conf.level = 0.95) 
    ##  Get all means:
    exp1__prop.FA.G__stim.means <- as.tibble(summary(emmeans(exp1__prop.FA.G.aov, ~stim)))
    exp1__prop.FA.G__resp.option.means <- as.tibble(summary(emmeans(exp1__prop.FA.G.aov, ~resp_option)))    
```
...
2. were incorrectly named by the majority of subjects (i.e. if the proportion of correct responses equalled ≤ 50%, since it was essential for the photographs to depict the same concepts as those found in the shaded drawings and word stimuli);
...
##  Add 'exp3and4__study_totals' to 'exp3and4__participant.info' df:
    exp3and4__participant.info <- exp3and4__participant.info %>%
...
##    Merge 'exp3__participant.info' and 'exp3__test.stats' to create final tidy df:  ---------------------------- 
      exp3__participant.info <- exp3and4__participant.info %>%
          filter(col_or_grey == "grey")
...
#### Materials
A total of 126 innocuous, everyday objects (e.g. clock, rabbit, shoe) were presented across three individual stimuli formats: written words, shaded drawings, and photographs. The drawings were obtained from @rossion2004, and consisted of greyscale shaded illustrations that contained some surface details. The word stimuli were simply the written word names of the line-drawn objects, presented in a clear Sans-serif typeface. The photograph stimuli were curated in the previous study; high quality photographs were sourced to similarly depict the same everyday objects as the shaded drawings. All objects in the photographs were isolated from their original background, converted to greyscale, and rotated to match the orientations shown in the line-drawn items.
...
##----------------------------------------------------------------------------------------------------------------- 
##  Significant main effect of RESPONSE OPTION, so first check if variances are equal using Levene's test:
    exp3__prop.hit__levene.test__rfg.rfbg <- exp3__prop.hit %>% leveneTest(prop.hit ~ resp_option, data = .)
##  Levene's test NOT significant (variances equal), so perform standard two-sample t-test:
    exp3__prop.hit__ttest__rfg.rfbg <- exp3__prop.hit %>%
        t_test(prop.hit ~ resp_option, paired = FALSE, data = ., var.equal = TRUE)
##  Calculate effect size:
    exp3__prop.hit__ttest__rfg.rfbg__eff.size <- exp3__prop.hit %>% 
        rstatix::cohens_d(prop.hit ~ resp_option, var.equal = TRUE, paired = FALSE, ci = TRUE, conf.level = 0.95)
##----------------------------------------------------------------------------------------------------------------- 
##  Significant main effect of STIMULI FORMAT, so perform paired contrasts:
    exp3__prop.hit__pairwise__stim.format <- exp3__prop.hit %>% 
        pairwise_t_test(prop.hit ~ stim, paired = TRUE, p.adjust.method = "bonferroni")
##  Calculate effect size:      
    exp3__prop.hit__pairwise__stim.format__eff.size <- exp3__prop.hit %>% 
        rstatix::cohens_d(prop.hit ~ stim, paired = TRUE, ci = TRUE, conf.level = 0.95)   
##  Format for report (grey photos vs. words t-test):
    temp <- exp3__prop.hit__pairwise__stim.format %>% filter(group1 == "grey.photo" & group2 == "word")
    exp3__prop.hit__ttest__grey.photo.vs.word <- str_c(ttest.writeup(temp), "; *d* = ", 
        round(as.vector(exp3__prop.hit__pairwise__stim.format__eff.size %>% 
                          filter(group1 == "grey.photo" & group2 == "word") %>% pull(effsize)), 2),
        ", 95% CI [", 
        exp3__prop.hit__pairwise__stim.format__eff.size %>%
            filter(group1 == "grey.photo" & group2 == "word") %>% pull(conf.low), ", ",
                    exp3__prop.hit__pairwise__stim.format__eff.size %>%
                        filter(group1 == "grey.photo" & group2 == "word") %>% pull(conf.high), "]")
    
##  Format for report (grey drawings vs. words t-test):
    temp <- exp3__prop.hit__pairwise__stim.format %>% filter(group1 == "grey.drawing" & group2 == "word")
    exp3__prop.hit__ttest__grey.drawing.vs.word <- str_c(ttest.writeup(temp), "; *d* = ", 
        round(as.vector(exp3__prop.hit__pairwise__stim.format__eff.size %>% 
                          filter(group1 == "grey.drawing" & group2 == "word") %>% pull(effsize)), 2),
        ", 95% CI [", 
        exp3__prop.hit__pairwise__stim.format__eff.size %>%
            filter(group1 == "grey.drawing" & group2 == "word") %>% pull(conf.low), ", ",
                    exp3__prop.hit__pairwise__stim.format__eff.size %>%
                        filter(group1 == "grey.drawing" & group2 == "word") %>% pull(conf.high), "]")                                                         
##  Format for report (grey drawings vs. grey photos t-test):
    temp <- exp3__prop.hit__pairwise__stim.format %>% filter(group1 == "grey.drawing" & group2 == "grey.photo")
    exp3__prop.hit__ttest__grey.photo.vs.grey.drawing <- str_c(ttest.writeup(temp), "; *d* = ", 
        round(as.vector(exp3__prop.hit__pairwise__stim.format__eff.size %>% 
                          filter(group1 == "grey.drawing" & group2 == "grey.photo") %>% pull(effsize)), 2),
        ", 95% CI [", 
        exp3__prop.hit__pairwise__stim.format__eff.size %>%
            filter(group1 == "grey.drawing" & group2 == "grey.photo") %>% pull(conf.low), ", ",
                    exp3__prop.hit__pairwise__stim.format__eff.size %>%
                        filter(group1 == "grey.drawing" & group2 == "grey.photo") %>% pull(conf.high), "]") 
##-----------------------------------------------------------------------------------------------------------------
##  Get all means:
...
##----------------------------------------------------------------------------------------------------------------- ##  Significant main effect of STIMULI FORMAT, so perform paired contrasts:
    exp3__prop.FA__pairwise__stim.format <- exp3__prop.FA %>% 
        pairwise_t_test(prop.FA ~ stim, paired = TRUE, p.adjust.method = "bonferroni")
##  Calculate effect size:      
    exp3__prop.FA__pairwise__stim.format__eff.size <- exp3__prop.FA %>% 
        rstatix::cohens_d(prop.FA ~ stim, paired = TRUE, ci = TRUE, conf.level = 0.95)       
    
##  Format for report (grey photos vs. words t-test):    
    temp <- exp3__prop.FA__pairwise__stim.format %>% filter(group1 == "grey.photo" & group2 == "word")
    exp3__prop.FA__ttest__grey.photo.vs.word <- str_c(ttest.writeup(temp), "; *d* = ", 
        round(as.vector(exp3__prop.FA__pairwise__stim.format__eff.size %>% 
                          filter(group1 == "grey.photo" & group2 == "word") %>% pull(effsize)), 2),
        ", 95% CI [", 
        exp3__prop.FA__pairwise__stim.format__eff.size %>%
            filter(group1 == "grey.photo" & group2 == "word") %>% pull(conf.low), ", ",
                    exp3__prop.FA__pairwise__stim.format__eff.size %>%
                        filter(group1 == "grey.photo" & group2 == "word") %>% pull(conf.high), "]")                                                         
##  Format for report (grey drawings vs. words t-test):
    temp <- exp3__prop.FA__pairwise__stim.format %>% filter(group1 == "grey.drawing" & group2 == "word")
    exp3__prop.FA__ttest__grey.drawing.vs.word <- str_c(ttest.writeup(temp), "; *d* = ", 
        round(as.vector(exp3__prop.FA__pairwise__stim.format__eff.size %>% 
                          filter(group1 == "grey.drawing" & group2 == "word") %>% pull(effsize)), 2),
        ", 95% CI [", 
        exp3__prop.FA__pairwise__stim.format__eff.size %>%
...
##  Format for report (grey drawings vs. grey photos t-test):
    ...
    exp3__prop.FA__ttest__grey.photo.vs.grey.drawing <- str_c(ttest.writeup(temp), "; *d* = ", 
        round(as.vector(exp3__prop.FA__pairwise__stim.format__eff.size %>% 
                          filter(group1 == "grey.drawing" & group2 == "grey.photo") %>% pull(effsize)), 2),
        ", 95% CI [", 
        exp3__prop.FA__pairwise__stim.format__eff.size %>%
...
################################################################################################################## ##  Create labels for figure (replaces plot legend):
    exp3__d.prime__interaction.figure.labels <- exp3__d.prime__interaction.means %>%
        select(stim, resp_option, emmean) %>%
        filter(stim == "Grey photographs") %>%  # Because we want the label to be next to photo data, on the right. 
        mutate(label = case_when(
                  resp_option == "RFG" ~ "RFG",
                  resp_option == "RFBG" ~ "RFBG")) %>%
        group_by(stim, resp_option, label)
...
##  Calculate effect size:      
    exp3__prop.hit.G__pairwise__stim.format__eff.size <- exp3__prop.hit.G %>% 
...
##  t-test - grey photos vs. words:
    ...
    exp3__prop.hit.G__ttest__grey.photo.vs.word <- str_c(ttest.writeup(temp), "; *d* = ", 
        round(as.vector(exp3__prop.hit.G__pairwise__stim.format__eff.size %>% 
                          filter(group1 == "grey.photo" & group2 == "word") %>% pull(effsize)), 2),
        ", 95% CI [", 
        exp3__prop.hit.G__pairwise__stim.format__eff.size %>%
            filter(group1 == "grey.photo" & group2 == "word") %>% pull(conf.low), ", ",
                    exp3__prop.hit.G__pairwise__stim.format__eff.size %>%
                        filter(group1 == "grey.photo" & group2 == "word") %>% pull(conf.high), "]")      
...
##  t-test - grey drawings vs. words:    
    ...
    exp3__prop.hit.G__ttest__grey.drawing.vs.word <- str_c(ttest.writeup(temp), "; *d* = ", 
        round(as.vector(exp3__prop.hit.G__pairwise__stim.format__eff.size %>% 
                          filter(group1 == "grey.drawing" & group2 == "word") %>% pull(effsize)), 2),
        ", 95% CI [", 
        exp3__prop.hit.G__pairwise__stim.format__eff.size %>%
...
##  t-test - grey drawings vs. grey photos:    
    ...
    exp3__prop.hit.G__ttest__grey.photo.vs.grey.drawing <- str_c(ttest.writeup(temp), "; *d* = ", 
        round(as.vector(exp3__prop.hit.G__pairwise__stim.format__eff.size %>% 
                          filter(group1 == "grey.drawing" & group2 == "grey.photo") %>% pull(effsize)), 2),
        ", 95% CI [", 
        exp3__prop.hit.G__pairwise__stim.format__eff.size %>%
...
##  Prep data for figures:  
    ...
    exp3__prop.hit.RFG__figure.data <- 
        ...
        mutate(stim = str_replace(stim, "word", "Words")) %>%
        mutate(stim = str_replace(stim, "grey.drawing", "Grey\ndrawings")) %>%
        mutate(stim = str_replace(stim, "grey.photo", "Grey\nphotographs")) %>%
        transform(stim = factor(stim, levels=c("Words", "Grey\ndrawings", "Grey\nphotographs")))
...
```{r exp3__prop-rec-fam-guess-hits--figurecaption, echo=FALSE, results = FALSE, cache=FALSE}
    figure_numbering(name = "exp3__prop.hit.RFG__figure",
...
###### Guessing (hits)
The ANOVA on the mean proportion of hits assigned Guessing demonstrated a significant main effect of stimuli format `r apa::anova_apa(exp3__prop.hit.G.aov, effect = "stim", format = "rmarkdown", print = FALSE)`. Grey photographs (*M*= `r round(exp3__prop.hit.G__stim.means %>% filter(stim == "grey.photo") %>% pull(emmean),2)`) produced significantly fewer G hits in comparison to both words (*M*= `r round(exp3__prop.hit.G__stim.means %>% filter(stim == "word") %>% pull(emmean),2)`; `r exp3__prop.hit.G__ttest__grey.photo.vs.word`) and grey drawings (*M*= `r round(exp3__prop.hit.G__stim.means %>% filter(stim == "grey.drawing") %>% pull(emmean),2)`; `r exp3__prop.hit.G__ttest__grey.photo.vs.grey.drawing`). The grey drawings (*M*= `r round(exp3__prop.hit.G__stim.means %>% filter(stim == "grey.drawing") %>% pull(emmean),2)`) also showed a significantly lower proportion of G hits compared to words (*M*= `r round(exp3__prop.hit.G__stim.means %>% filter(stim == "word") %>% pull(emmean),2)`; `r exp3__prop.hit.G__ttest__grey.drawing.vs.word`. There were no significant interaction effects between stimuli format and response option condition `r apa::anova_apa(exp3__prop.hit.G.aov, effect = "resp_option:stim", format = "rmarkdown", print = FALSE)`.
...
##################################################################################################################
##  Two-way mixed ANOVA:
    exp3__prop.FA.F.aov <- afex::aov_ez('id', 'prop.FA.F', fun_aggregate = mean, exp3__prop.FA.F, 
                                       between = "resp_option", within = 'stim',
                                       anova_table=list(correction = "none", es = "ges"))
##  Mauchlys test of sphericity:
    exp3__prop.FA.F.aov__sphericity <- test_sphericity(exp3__prop.FA.F.aov)
##  Assumption of sphericity not violated (p>.05). Greenhouse-Geisser correction not applied.
##-----------------------------------------------------------------------------------------------------------------
##  Significant main effect of RESPONSE OPTION, so first check if variances are equal using Levene's test:
    exp3__prop.FA.F__levene.test__rfg.rfbg <- exp3__prop.FA.F %>% leveneTest(prop.FA.F ~ resp_option, data = .)
##  Levene's test significant (variances not equal), so perform corrected Welch two-sample t-test: 
    exp3__prop.FA.F__ttest__rfg.rfbg <- exp3__prop.FA.F %>%
        t_test(prop.FA.F ~ resp_option, paired = FALSE, data = ., var.equal = FALSE)
##  Calculate effect size:
    exp3__prop.FA.F__ttest__rfg.rfbg__eff.size <- exp3__prop.FA.F %>% 
        rstatix::cohens_d(prop.FA.F ~ resp_option, var.equal = FALSE, paired = FALSE, ci = TRUE, conf.level = 0.95)
##----------------------------------------------------------------------------------------------------------------- ##  Significant main effect of STIMULI FORMAT, so perform paired contrasts:
    exp3__prop.FA.F__pairwise__stim.format <- exp3__prop.FA.F %>% 
        pairwise_t_test(prop.FA.F ~ stim, paired = TRUE, p.adjust.method = "bonferroni")
##  Calculate effect size:      
    exp3__prop.FA.F__pairwise__stim.format__eff.size <- exp3__prop.FA.F %>% 
        rstatix::cohens_d(prop.FA.F ~ stim, paired = TRUE, ci = TRUE, conf.level = 0.95)      
##  t-test - grey photos vs. words:
    temp <- exp3__prop.FA.F__pairwise__stim.format %>% filter(group1 == "grey.photo" & group2 == "word")
    exp3__prop.FA.F__ttest__grey.photo.vs.word <- str_c(ttest.writeup(temp), "; *d* = ", 
        round(as.vector(exp3__prop.FA.F__pairwise__stim.format__eff.size %>% 
                          filter(group1 == "grey.photo" & group2 == "word") %>% pull(effsize)), 2),
        ", 95% CI [", 
        exp3__prop.FA.F__pairwise__stim.format__eff.size %>%
            filter(group1 == "grey.photo" & group2 == "word") %>% pull(conf.low), ", ",
                    exp3__prop.FA.F__pairwise__stim.format__eff.size %>%
                        filter(group1 == "grey.photo" & group2 == "word") %>% pull(conf.high), "]")          
    
##  t-test - grey drawings vs. words:    
    temp <- exp3__prop.FA.F__pairwise__stim.format %>% filter(group1 == "grey.drawing" & group2 == "word")
    exp3__prop.FA.F__ttest__grey.drawing.vs.word <- str_c(ttest.writeup(temp), "; *d* = ", 
        round(as.vector(exp3__prop.FA.F__pairwise__stim.format__eff.size %>% 
                          filter(group1 == "grey.drawing" & group2 == "word") %>% pull(effsize)), 2),
        ", 95% CI [", 
        exp3__prop.FA.F__pairwise__stim.format__eff.size %>%
...
##  t-test - grey drawings vs. grey photos:    
    temp <- exp3__prop.FA.F__pairwise__stim.format %>% filter(group1 == "grey.drawing" & group2 == "grey.photo")
    exp3__prop.FA.F__ttest__grey.photo.vs.grey.drawing <- str_c(ttest.writeup(temp), "; *d* = ", 
        round(as.vector(exp3__prop.FA.F__pairwise__stim.format__eff.size %>% 
                          filter(group1 == "grey.drawing" & group2 == "grey.photo") %>% pull(effsize)), 2),
        ", 95% CI [", 
        exp3__prop.FA.F__pairwise__stim.format__eff.size %>%
            filter(group1 == "grey.drawing" & group2 == "grey.photo") %>% pull(conf.low), ", ",
                    exp3__prop.FA.F__pairwise__stim.format__eff.size %>%
                        filter(group1 == "grey.drawing" & group2 == "grey.photo") %>% pull(conf.high), "]")
##----------------------------------------------------------------------------------------------------------------- ##  Get all means:
    exp3__prop.FA.F__stim.means <- as.tibble(summary(emmeans(exp3__prop.FA.F.aov, ~stim)))
    exp3__prop.FA.F__resp.option.means <- as.tibble(summary(emmeans(exp3__prop.FA.F.aov, ~resp_option)))
    
##################################################################################################################
##  Exp3 Prop G FAs:                                                                                           ##
##################################################################################################################
##  Two-way mixed ANOVA:
    exp3__prop.FA.G.aov <- afex::aov_ez('id', 'prop.FA.G', fun_aggregate = mean, exp3__prop.FA.G, 
                                       between = "resp_option", within = 'stim',
                                       anova_table=list(correction = "none", es = "ges"))
##  Mauchlys test of sphericity:
    exp3__prop.FA.G.aov__sphericity <- test_sphericity(exp3__prop.FA.G.aov)
##  Assumption of sphericity violated (p<.05), so apply Greenhouse-Geisser correction:
    exp3__prop.FA.G.aov <- afex::aov_ez('id', 'prop.FA.G', fun_aggregate = mean, exp3__prop.FA.G, 
                                       between = "resp_option", within = 'stim',
                                       anova_table=list(correction = "GG", es = "ges"))
##----------------------------------------------------------------------------------------------------------------- ##  No significant main effect of RESPONSE OPTION.
##----------------------------------------------------------------------------------------------------------------- ##  Significant main effect of STIMULI FORMAT, so perform paired contrasts:
    exp3__prop.FA.G__pairwise__stim.format <- exp3__prop.FA.G %>% 
        pairwise_t_test(prop.FA.G ~ stim, paired = TRUE, p.adjust.method = "bonferroni")   
##  Calculate effect size:      
    exp3__prop.FA.G__pairwise__stim.format__eff.size <- exp3__prop.FA.G %>% 
        rstatix::cohens_d(prop.FA.G ~ stim, paired = TRUE, ci = TRUE, conf.level = 0.95)
##  t-test - grey photos vs. words:
    temp <- exp3__prop.FA.G__pairwise__stim.format %>% filter(group1 == "grey.photo" & group2 == "word")
    exp3__prop.FA.G__ttest__grey.photo.vs.word <- str_c(ttest.writeup(temp), "; *d* = ", 
        round(as.vector(exp3__prop.FA.G__pairwise__stim.format__eff.size %>% 
                          filter(group1 == "grey.photo" & group2 == "word") %>% pull(effsize)), 2),
        ", 95% CI [", 
        exp3__prop.FA.G__pairwise__stim.format__eff.size %>%
            filter(group1 == "grey.photo" & group2 == "word") %>% pull(conf.low), ", ",
                    exp3__prop.FA.G__pairwise__stim.format__eff.size %>%
                        filter(group1 == "grey.photo" & group2 == "word") %>% pull(conf.high), "]")      

##  t-test - grey drawings vs. words:    
    temp <- exp3__prop.FA.G__pairwise__stim.format %>% filter(group1 == "grey.drawing" & group2 == "word")
    exp3__prop.FA.G__ttest__grey.drawing.vs.word <- str_c(ttest.writeup(temp), "; *d* = ", 
        round(as.vector(exp3__prop.FA.G__pairwise__stim.format__eff.size %>% 
                          filter(group1 == "grey.drawing" & group2 == "word") %>% pull(effsize)), 2),
        ", 95% CI [", 
        exp3__prop.FA.G__pairwise__stim.format__eff.size %>%
...
##  t-test - grey drawings vs. grey photos:    
    temp <- exp3__prop.FA.G__pairwise__stim.format %>% filter(group1 == "grey.drawing" & group2 == "grey.photo")
    exp3__prop.FA.G__ttest__grey.photo.vs.grey.drawing <- str_c(ttest.writeup(temp), "; *d* = ", 
        round(as.vector(exp3__prop.FA.G__pairwise__stim.format__eff.size %>% 
                          filter(group1 == "grey.drawing" & group2 == "grey.photo") %>% pull(effsize)), 2),
        ", 95% CI [", 
        exp3__prop.FA.G__pairwise__stim.format__eff.size %>%
            filter(group1 == "grey.drawing" & group2 == "grey.photo") %>% pull(conf.low), ", ",
                    exp3__prop.FA.G__pairwise__stim.format__eff.size %>%
                        filter(group1 == "grey.drawing" & group2 == "grey.photo") %>% pull(conf.high), "]")        
##----------------------------------------------------------------------------------------------------------------- ##  Get all means:
    exp3__prop.FA.G__stim.means <- as.tibble(summary(emmeans(exp3__prop.FA.G.aov, ~stim)))
    exp3__prop.FA.G__resp.option.means <- as.tibble(summary(emmeans(exp3__prop.FA.G.aov, ~resp_option)))
    
##################################################################################################################
##    FAs FIGURE:                                                                                               ##
##################################################################################################################
##  Prep data for figures:  
    exp3__prop.FA.R__figure.data <- 
        as.tibble(emmeans(exp3__prop.FA.R.aov, ~ resp_option:stim)) %>%
        select(-df, -lower.CL, -upper.CL)
    exp3__prop.FA.F__figure.data <- 
        as.tibble(emmeans(exp3__prop.FA.F.aov, ~ resp_option:stim)) %>%
        select(-df, -lower.CL, -upper.CL)
    exp3__prop.FA.G__figure.data <- 
        as.tibble(emmeans(exp3__prop.FA.G.aov, ~ resp_option:stim)) %>%
        select(-df, -lower.CL, -upper.CL)    

    exp3__prop.FA.RFG__figure.data <- 
        tribble(~"resp_option", ~"stim", ~"rfg", ~"emmean", ~"SE",
            as.character(pull(exp3__prop.FA.R__figure.data[1,1])), ... = 
            as.character(pull(exp3__prop.FA.R__figure.data[1,2])), "Recollection",
                as.double(pull(exp3__prop.FA.R__figure.data[1,3])), 
                as.double(pull(exp3__prop.FA.R__figure.data[1,4])),
            as.character(pull(exp3__prop.FA.R__figure.data[2,1])), 
            as.character(pull(exp3__prop.FA.R__figure.data[2,2])), "Recollection",
                as.double(pull(exp3__prop.FA.R__figure.data[2,3])), 
                as.double(pull(exp3__prop.FA.R__figure.data[2,4])),
            as.character(pull(exp3__prop.FA.R__figure.data[3,1])), 
            as.character(pull(exp3__prop.FA.R__figure.data[3,2])), "Recollection",
                as.double(pull(exp3__prop.FA.R__figure.data[3,3])), 
                as.double(pull(exp3__prop.FA.R__figure.data[3,4])),
            as.character(pull(exp3__prop.FA.R__figure.data[4,1])), 
            as.character(pull(exp3__prop.FA.R__figure.data[4,2])), "Recollection",
                as.double(pull(exp3__prop.FA.R__figure.data[4,3])), 
                as.double(pull(exp3__prop.FA.R__figure.data[4,4])),
            as.character(pull(exp3__prop.FA.R__figure.data[5,1])), 
            as.character(pull(exp3__prop.FA.R__figure.data[5,2])), "Recollection",
                as.double(pull(exp3__prop.FA.R__figure.data[5,3])), 
                as.double(pull(exp3__prop.FA.R__figure.data[5,4])),
            as.character(pull(exp3__prop.FA.R__figure.data[6,1])), 
            as.character(pull(exp3__prop.FA.R__figure.data[6,2])), "Recollection",
                as.double(pull(exp3__prop.FA.R__figure.data[6,3])), 
                as.double(pull(exp3__prop.FA.R__figure.data[6,4])),
            as.character(pull(exp3__prop.FA.F__figure.data[1,1])), 
            as.character(pull(exp3__prop.FA.F__figure.data[1,2])), "Familiarity",
                as.double(pull(exp3__prop.FA.F__figure.data[1,3])), 
                as.double(pull(exp3__prop.FA.F__figure.data[1,4])),
            as.character(pull(exp3__prop.FA.F__figure.data[2,1])), 
            as.character(pull(exp3__prop.FA.F__figure.data[2,2])), "Familiarity",
                as.double(pull(exp3__prop.FA.F__figure.data[2,3])), 
                as.double(pull(exp3__prop.FA.F__figure.data[2,4])),
            as.character(pull(exp3__prop.FA.F__figure.data[3,1])), 
            as.character(pull(exp3__prop.FA.F__figure.data[3,2])), "Familiarity",
                as.double(pull(exp3__prop.FA.F__figure.data[3,3])), 
                as.double(pull(exp3__prop.FA.F__figure.data[3,4])),
            as.character(pull(exp3__prop.FA.F__figure.data[4,1])), 
            as.character(pull(exp3__prop.FA.F__figure.data[4,2])), "Familiarity",
                as.double(pull(exp3__prop.FA.F__figure.data[4,3])), 
                as.double(pull(exp3__prop.FA.F__figure.data[4,4])),
            as.character(pull(exp3__prop.FA.F__figure.data[5,1])), 
            as.character(pull(exp3__prop.FA.F__figure.data[5,2])), "Familiarity",
                as.double(pull(exp3__prop.FA.F__figure.data[5,3])), 
                as.double(pull(exp3__prop.FA.F__figure.data[5,4])),
            as.character(pull(exp3__prop.FA.F__figure.data[6,1])), 
            as.character(pull(exp3__prop.FA.F__figure.data[6,2])), "Familiarity",
                as.double(pull(exp3__prop.FA.F__figure.data[6,3])), 
                as.double(pull(exp3__prop.FA.F__figure.data[6,4])),
            as.character(pull(exp3__prop.FA.G__figure.data[1,1])), 
            as.character(pull(exp3__prop.FA.G__figure.data[1,2])), "Guessing",
                as.double(pull(exp3__prop.FA.G__figure.data[1,3])), 
                as.double(pull(exp3__prop.FA.G__figure.data[1,4])),
            as.character(pull(exp3__prop.FA.G__figure.data[2,1])), 
            as.character(pull(exp3__prop.FA.G__figure.data[2,2])), "Guessing",
                as.double(pull(exp3__prop.FA.G__figure.data[2,3])), 
                as.double(pull(exp3__prop.FA.G__figure.data[2,4])),
            as.character(pull(exp3__prop.FA.G__figure.data[3,1])), 
            as.character(pull(exp3__prop.FA.G__figure.data[3,2])), "Guessing",
                as.double(pull(exp3__prop.FA.G__figure.data[3,3])), 
                as.double(pull(exp3__prop.FA.G__figure.data[3,4])),
            as.character(pull(exp3__prop.FA.G__figure.data[4,1])), 
            as.character(pull(exp3__prop.FA.G__figure.data[4,2])), "Guessing",
                as.double(pull(exp3__prop.FA.G__figure.data[4,3])), 
                as.double(pull(exp3__prop.FA.G__figure.data[4,4])),
            as.character(pull(exp3__prop.FA.G__figure.data[5,1])), 
            as.character(pull(exp3__prop.FA.G__figure.data[5,2])), "Guessing",
                as.double(pull(exp3__prop.FA.G__figure.data[5,3])), 
                as.double(pull(exp3__prop.FA.G__figure.data[5,4])),
            as.character(pull(exp3__prop.FA.G__figure.data[6,1])), 
            as.character(pull(exp3__prop.FA.G__figure.data[6,2])), "Guessing",
                as.double(pull(exp3__prop.FA.G__figure.data[6,3])), 
                as.double(pull(exp3__prop.FA.G__figure.data[6,4]))) %>%
        transform(resp_option = factor(resp_option, levels=c("RFG","RFBG"))) %>%
        transform(rfg = factor(rfg, levels=c("Recollection","Familiarity","Guessing"))) %>%
        mutate(stim = str_replace(stim, "word", "Words")) %>%
        mutate(stim = str_replace(stim, "grey.drawing", "Grey\ndrawings")) %>%
...
##  Create figure:
    exp3__prop.FA.RFG__figure <-
        ggplot(exp3__prop.FA.RFG__figure.data, aes(x = stim, y = emmean, fill = rfg)) +
        scale_fill_manual("legend", values = c("Recollection" = "#1B4251", "Familiarity" = "#6A6C93",
                                               "Guessing" = "#D491CA")) + 
        geom_col(position = position_dodge()) +
        geom_errorbar(aes(ymin = pmax(emmean-SE, 0), ymax = emmean+SE), 
...
###### Familiarity (FAs)
The ANOVA for FAs assigned *Familiarity* showed a significant main effect of stimuli format, `r apa::anova_apa(exp3__prop.FA.F.aov, effect = "stim", format = "rmarkdown", print = FALSE)`. Grey photographs (*M*= `r round(exp3__prop.FA.F__stim.means %>% filter(stim == "grey.photo") %>% pull(emmean),2)`) produced significantly fewer F FAs than words (*M*= `r round(exp3__prop.FA.F__stim.means %>% filter(stim == "word") %>% pull(emmean),2)`), `r exp3__prop.FA.F__ttest__grey.photo.vs.word`. Likewise, grey drawings (*M*= `r round(exp3__prop.FA.F__stim.means %>% filter(stim == "grey.drawing") %>% pull(emmean),2)`) also showed a significantly lower proportion of FAs compared to words (*M*= `r round(exp3__prop.FA.F__stim.means %>% filter(stim == "word") %>% pull(emmean),2)`), `r exp3__prop.FA.F__ttest__grey.drawing.vs.word`. However, there was no significant difference in the proportion of FAs assigned Familiarity between grey photographs (*M*= `r round(exp3__prop.FA.F__stim.means %>% filter(stim == "grey.photo") %>% pull(emmean),2)`) and grey drawings
(*M*= `r round(exp3__prop.FA.F__stim.means %>% filter(stim == "grey.drawing") %>% pull(emmean),2)`), `r exp3__prop.FA.F__ttest__grey.photo.vs.grey.drawing`. There were no significant interaction effects between stimuli format and response option condition, `r apa::anova_apa(exp3__prop.FA.F.aov, effect = "resp_option:stim", format = "rmarkdown", print = FALSE)`.

###### Guessing (FAs)
The ANOVA on the mean proportion of FAs assigned *Guessing* demonstrated a significant main effect of stimuli format `r apa::anova_apa(exp3__prop.FA.G.aov, effect = "stim", format = "rmarkdown", print = FALSE)`. Grey photographs (*M*= `r round(exp3__prop.FA.G__stim.means %>% filter(stim == "grey.photo") %>% pull(emmean),2)`) produced significantly fewer G FAs in comparison to words (*M*= `r round(exp3__prop.FA.G__stim.means %>% filter(stim == "word") %>% pull(emmean),2)`; `r exp3__prop.FA.G__ttest__grey.photo.vs.word`). Likewise, grey drawings (*M*= `r round(exp3__prop.FA.G__stim.means %>% filter(stim == "grey.drawing") %>% pull(emmean),2)`) also showed a significantly lower proportion of FAs compared to words (*M*= `r round(exp3__prop.FA.G__stim.means %>% filter(stim == "word") %>% pull(emmean),2)`; `r exp3__prop.FA.G__ttest__grey.drawing.vs.word`. However, there was no significant difference in the proportion of FAs assigned Guessing between grey photographs (*M*= `r round(exp3__prop.FA.G__stim.means %>% filter(stim == "grey.photo") %>% pull(emmean),2)`) and grey drawings (*M*= `r round(exp3__prop.FA.G__stim.means %>% filter(stim == "grey.drawing") %>% pull(emmean),2)`; `r exp3__prop.FA.G__ttest__grey.photo.vs.grey.drawing`). There were also no significant interaction effects between stimuli format and response option condition `r apa::anova_apa(exp3__prop.FA.G.aov, effect = "resp_option:stim", format = "rmarkdown", print = FALSE)`.

#### Response option availability
```{r echo=FALSE, results = FALSE, message=FALSE, warning=FALSE, cache=TRUE, fig.show='hide'}
##################################################################################################################
##  Exp3 Resp option main effects table:                                                                        ##
##################################################################################################################
exp3__resp.option__main.effects__table <- tribble(
        ~"Variable", ~"Main effect of response option", ~"Signif.",
        "Mean proportion: ***Hits***",
            apa::anova_apa(exp3__prop.hit.aov, effect = "resp_option", format = "rmarkdown", print = FALSE),
            "*",
        "Mean proportion: ***FAs***",
            apa::anova_apa(exp3__prop.FA.aov, effect = "resp_option", format = "rmarkdown", print = FALSE),
            "",
        "Mean scores: ***d'***",  
            apa::anova_apa(exp3__d.prime.aov, effect = "resp_option", format = "rmarkdown", print = FALSE),
            "\\+",
        "Mean proportion: ***Recollection hits***",
            apa::anova_apa(exp3__prop.hit.R.aov, effect = "resp_option", format = "rmarkdown", print = FALSE),
            "\\+", 
        "Mean proportion: ***Familiarity hits***",
            apa::anova_apa(exp3__prop.hit.F.aov, effect = "resp_option", format = "rmarkdown", print = FALSE),
            "\\+", 
        "Mean proportion: ***Guessing hits***", 
            apa::anova_apa(exp3__prop.hit.G.aov, effect = "resp_option", format = "rmarkdown", print = FALSE),
            "", 
        "Mean proportion: ***Recollection FAs***",
            apa::anova_apa(exp3__prop.FA.R.aov, effect = "resp_option", format = "rmarkdown", print = FALSE),
            "", 
        "Mean proportion: ***Familiarity FAs***",
            apa::anova_apa(exp3__prop.FA.F.aov, effect = "resp_option", format = "rmarkdown", print = FALSE),
            "*", 
        "Mean proportion: ***Guessing FAs***",
            apa::anova_apa(exp3__prop.FA.G.aov, effect = "resp_option", format = "rmarkdown", print = FALSE),
            "")
```

```{r echo=FALSE, results = FALSE, cache=FALSE}
table_numbering(name = "exp3__resp.option__main.effects__table", caption = "Main effects of response option condition across all variables of interest. Signif. codes:  \\*\\*\\**p* < .001; \\*\\**p* < .01; \\**p* < .05; \\+ involved in significant interaction [see previous section for interpretation]).")
```

In the previously discussed ANOVAs, significant main effects were also observed for response option condition (RFG / RFBG) for the mean proportion of hits (`r apa::anova_apa(exp3__prop.hit.aov, effect = "resp_option", format = "rmarkdown", print = FALSE)`) and the mean proportion of FAs assigned *Familiarity* (`r apa::anova_apa(exp3__prop.FA.F.aov, effect = "resp_option", format = "rmarkdown", print = FALSE)`). For all other variables, response option was found to either significantly interact with stimuli format (discussed previously), or had no significant main effect (see `r table_numbering("exp3__resp.option__main.effects__table", display = "cite")` for all response option ANOVA results).

For the mean proportion of hits, follow up t-tests showed a higher proportion of hits in the RFG group (*M*= `r round(exp3__prop.hit__resp.option.means %>% filter(resp_option == "RFG") %>% pull(emmean),2)`) compared to the RFBG group (*M*= `r round(exp3__prop.hit__resp.option.means %>% filter(resp_option == "RFBG") %>% pull(emmean),2)`), `r str_c(papaja::apa_print(exp3__prop.hit__ttest__rfg.rfbg)$statistic %>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*"), ", *d* = ", round(abs(as.vector(exp3__prop.hit__ttest__rfg.rfbg__eff.size$effsize)), 2))`. For the mean proportion of FAs assigned *Familiarity*, those in the RFG group (*M*= `r round(exp3__prop.FA.F__resp.option.means %>% filter(resp_option == "RFG") %>% pull(emmean),2)`) showed a significantly higher proportion than those in the RFBG group (*M*= `r round(exp3__prop.FA.F__resp.option.means %>% filter(resp_option == "RFBG") %>% pull(emmean),2)`), `r str_c(papaja::apa_print(exp3__prop.FA.F__ttest__rfg.rfbg)$statistic %>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*"), ", *d* = ", round(abs(as.vector(exp3__prop.FA.F__ttest__rfg.rfbg__eff.size$effsize)), 2))`. 

&nbsp;
`r table_numbering("exp3__resp.option__main.effects__table")`
```{r echo=FALSE, cache=TRUE}
    exp3__resp.option__main.effects__table %>%
        kable("markdown", booktabs = T, full_width= T, align=rep('r', 3))
```
...
3. Any effects associated with manipulating the availability of response options at test (RFG/RFBG) will remain unaffected by the addition of colour information to image stimuli. Based on the findings of *Experiment 3*, it is expected that:  
...
##    Merge 'exp4__participant.info' and 'exp4__test.stats' to create final tidy df:  ---------------------------- 
      exp4__participant.info <- exp3and4__participant.info %>%
          filter(col_or_grey == "col")
...
##################################################################################################################
    ...
    exp3.and.4__prop.hit.FA.d.prime__table <- tribble(
        ~" ", ~"  ", ~Hits, ~FAs, ~"d'", ~"    ", ~"     ", ~"   ", ~Hits, ~FAs, ~"d'",
        "Stimuli format:", "", "***", "***", "+", "", "Stimuli format:", "", "***", "***", "***",
        "", "Words",  as.character(exp3__prop.hit.FA.d.prime__table['Words',] %>% pull(Hits)),
                      as.character(exp3__prop.hit.FA.d.prime__table['Words',] %>% pull(FAs)),
                      as.character(exp3__prop.hit.FA.d.prime__table['Words',] %>% pull(`d'`)),
        "","",
            "Words",  as.character(exp4__prop.hit.FA.d.prime__table['Words',] %>% pull(Hits)),
                      as.character(exp4__prop.hit.FA.d.prime__table['Words',] %>% pull(FAs)),
                      as.character(exp4__prop.hit.FA.d.prime__table['Words',] %>% pull(`d'`)),
        "", "Drawings",   as.character(exp3__prop.hit.FA.d.prime__table['Grey drawings',] %>% pull(Hits)),
                          as.character(exp3__prop.hit.FA.d.prime__table['Grey drawings',] %>% pull(FAs)),
                          as.character(exp3__prop.hit.FA.d.prime__table['Grey drawings',] %>% pull(`d'`)),
        "","",
            "Drawings",   as.character(exp4__prop.hit.FA.d.prime__table['Colour drawings',] %>% pull(Hits)),
                          as.character(exp4__prop.hit.FA.d.prime__table['Colour drawings',] %>% pull(FAs)),
                          as.character(exp4__prop.hit.FA.d.prime__table['Colour drawings',] %>% pull(`d'`)),
        "", "Photographs",  as.character(exp3__prop.hit.FA.d.prime__table['Grey photographs',] %>% pull(Hits)),
                            as.character(exp3__prop.hit.FA.d.prime__table['Grey photographs',] %>% pull(FAs)),
                            as.character(exp3__prop.hit.FA.d.prime__table['Grey photographs',] %>% pull(`d'`)),
        "","",
            "Photographs",  as.character(exp4__prop.hit.FA.d.prime__table['Colour photographs',] %>% pull(Hits)),
                            as.character(exp4__prop.hit.FA.d.prime__table['Colour photographs',] %>% pull(FAs)),
                            as.character(exp4__prop.hit.FA.d.prime__table['Colour photographs',] %>% pull(`d'`)),
        "Response option:", "", "*", "", "+", "", "Response option:", "", "", "*", "",
        "", "RFG",            as.character(exp3__prop.hit.FA.d.prime__table['RFG',] %>% pull(Hits)),
                              as.character(exp3__prop.hit.FA.d.prime__table['RFG',] %>% pull(FAs)),
                              as.character(exp3__prop.hit.FA.d.prime__table['RFG',] %>% pull(`d'`)),
        "","",
            "RFG",            as.character(exp4__prop.hit.FA.d.prime__table['RFG',] %>% pull(Hits)),
                              as.character(exp4__prop.hit.FA.d.prime__table['RFG',] %>% pull(FAs)),
                              as.character(exp4__prop.hit.FA.d.prime__table['RFG',] %>% pull(`d'`)),        
        "", "RFBG",            as.character(exp3__prop.hit.FA.d.prime__table['RFBG',] %>% pull(Hits)),
                              as.character(exp3__prop.hit.FA.d.prime__table['RFBG',] %>% pull(FAs)),
                              as.character(exp3__prop.hit.FA.d.prime__table['RFBG',] %>% pull(`d'`)),
        "","",
            "RFBG",            as.character(exp4__prop.hit.FA.d.prime__table['RFBG',] %>% pull(Hits)),
                              as.character(exp4__prop.hit.FA.d.prime__table['RFBG',] %>% pull(FAs)),
                              as.character(exp4__prop.hit.FA.d.prime__table['RFBG',] %>% pull(`d'`)))
```

```{r echo=FALSE, results = FALSE, cache=FALSE}
table_numbering(name = "exp3.and.4__prop.hit.FA.d.prime__table", caption = "Data from Experiment 3 (using greyscale stimuli) shown alongside that of the current experiment (using colour stimuli), showing the mean proportion of hits, FAs, and mean *d'* scores, by stimuli format and response option condition. Signif. codes:  \\*\\*\\**p* < .001; \\*\\**p* < .01; \\**p* < .05; \\+ involved in significant interaction.")
```
\hfill\break Mean proportions of hits and FAs, and mean *d'* scores are presented for Experiments 3 and 4 in `r table_numbering("exp3.and.4__prop.hit.FA.d.prime__table", display = "cite")`. Visual inspection of the data shows some expected patterns with regard to stimuli distinctiveness. As the intended distinctiveness increases (from words, to drawings, to photographs), the i) mean proportion of hits increase; ii) mean proportion of FAs decrease; iii) mean *d'* scores increase.

Results from the ANOVAs demonstrated a significant main effect of stimuli format for each of the key variables of interest; hits (`r apa::anova_apa(exp4__prop.hit.aov, effect = "stim", format = "rmarkdown", print = FALSE)`), FAs (`r apa::anova_apa(exp4__prop.FA.aov, effect = "stim", format = "rmarkdown", print = FALSE)`), and *d'* scores (`r apa::anova_apa(exp4__d.prime.aov, effect = "stim", format = "rmarkdown", print = FALSE)`) - though no interaction effects were evident between stimuli format and response option; hits (`r apa::anova_apa(exp4__prop.hit.aov, effect = "resp_option:stim", format = "rmarkdown", print = FALSE)`), FAs(`r apa::anova_apa(exp4__prop.FA.aov, effect = "resp_option:stim", format = "rmarkdown", print = FALSE)`), or *d'* scores (`r apa::anova_apa(exp4__d.prime.aov, effect = "resp_option:stim", format = "rmarkdown", print = FALSE)`). 

\newpage
`r table_numbering("exp3.and.4__prop.hit.FA.d.prime__table")`
```{r echo=FALSE, cache=TRUE}
    exp3.and.4__prop.hit.FA.d.prime__table %>%
        kable("latex", booktabs = T, align = "llccccllccc", escape = FALSE) %>%
        kable_styling(latex_options = c("scale_down", "hold_position"), full_width = F, 
                      bootstrap_options = "condensed", font_size = 13) %>%
        add_header_above(c("Experiment 3: Grey" = 2, " " = 4, "Experiment 4: Colour" = 2), line = F, bold = T) %>%
        column_spec(1, width = "1.9cm") %>%
        column_spec(2, width = "2.5cm") %>%
        column_spec(3:5, width = "1cm") %>%
        column_spec(6, width = "1.8cm") %>%
        column_spec(7, width = "1.9cm") %>%
        column_spec(8, width = "2.5cm") %>%
        column_spec(9:11, width = "1cm") %>%
        row_spec(c(1,5), bold = T)
```
&nbsp;
&nbsp;

To determine whether photo superiority effects were exhibited in the current set of colour stimuli - comparable to those previously observed using greyscale items (Experiment 3) - pairwise t-tests were performed between the colour photos and drawings. For the mean proportion of hits, colour photographs (*M*= `r round(exp4__prop.hit__stim.means %>% filter(stim == "col.photo") %>% pull(emmean),2)`) exhibited a significantly higher proportion than colour drawings (*M*= `r round(exp4__prop.hit__stim.means %>% filter(stim == "col.drawing") %>% pull(emmean),2)`), `r exp4__prop.hit__ttest__col.photo.vs.col.drawing`. The photographs (*M*= `r round(exp4__prop.FA__stim.means %>% filter(stim == "col.photo") %>% pull(emmean),2)`) also produced significantly fewer FAs compared to drawings (*M*= `r round(exp4__prop.FA__stim.means %>% filter(stim == "col.drawing") %>% pull(emmean),2)`), `r exp4__prop.FA__ttest__col.photo.vs.col.drawing`). Mean *d'* scores were also significantly higher for the colour photographs (*M*= `r round(exp4__d.prime__stim.means %>% filter(stim == "col.photo") %>% pull(emmean),2)`) compared to the colour drawings (*M*= `r round(exp4__d.prime__stim.means %>% filter(stim == "col.drawing") %>% pull(emmean),2)`), `r exp4__d.prime__ttest__col.photo.vs.col.drawing`. These findings replicate those found previously using greyscale items, whereby photographs offer a number of recognition benefits when compared to less detailed line-drawn illustrations.
...
#################################################################################################################
##  Prep data for figures:  
    exp4__prop.hit.R__figure.data <- 
        as.tibble(emmeans(exp4__prop.hit.R.aov, ~ resp_option:stim)) %>%
        select(-df, -lower.CL, -upper.CL)
    exp4__prop.hit.F__figure.data <- 
        as.tibble(emmeans(exp4__prop.hit.F.aov, ~ resp_option:stim)) %>%
        select(-df, -lower.CL, -upper.CL)
    exp4__prop.hit.G__figure.data <- 
        as.tibble(emmeans(exp4__prop.hit.G.aov, ~ resp_option:stim)) %>%
        select(-df, -lower.CL, -upper.CL)    

    exp4__prop.hit.RFG__figure.data <- 
        tribble(~"resp_option", ~"stim", ~"rfg", ~"emmean", ~"SE",
            as.character(pull(exp4__prop.hit.R__figure.data[1,1])), ... = 
            as.character(pull(exp4__prop.hit.R__figure.data[1,2])), "Recollection",
                as.double(pull(exp4__prop.hit.R__figure.data[1,3])), 
                as.double(pull(exp4__prop.hit.R__figure.data[1,4])),
            as.character(pull(exp4__prop.hit.R__figure.data[2,1])), 
            as.character(pull(exp4__prop.hit.R__figure.data[2,2])), "Recollection",
                as.double(pull(exp4__prop.hit.R__figure.data[2,3])), 
                as.double(pull(exp4__prop.hit.R__figure.data[2,4])),
            as.character(pull(exp4__prop.hit.R__figure.data[3,1])), 
            as.character(pull(exp4__prop.hit.R__figure.data[3,2])), "Recollection",
                as.double(pull(exp4__prop.hit.R__figure.data[3,3])), 
                as.double(pull(exp4__prop.hit.R__figure.data[3,4])),
            as.character(pull(exp4__prop.hit.R__figure.data[4,1])), 
            as.character(pull(exp4__prop.hit.R__figure.data[4,2])), "Recollection",
                as.double(pull(exp4__prop.hit.R__figure.data[4,3])), 
                as.double(pull(exp4__prop.hit.R__figure.data[4,4])),
            as.character(pull(exp4__prop.hit.R__figure.data[5,1])), 
            as.character(pull(exp4__prop.hit.R__figure.data[5,2])), "Recollection",
                as.double(pull(exp4__prop.hit.R__figure.data[5,3])), 
                as.double(pull(exp4__prop.hit.R__figure.data[5,4])),
            as.character(pull(exp4__prop.hit.R__figure.data[6,1])), 
            as.character(pull(exp4__prop.hit.R__figure.data[6,2])), "Recollection",
                as.double(pull(exp4__prop.hit.R__figure.data[6,3])), 
                as.double(pull(exp4__prop.hit.R__figure.data[6,4])),
            as.character(pull(exp4__prop.hit.F__figure.data[1,1])), 
            as.character(pull(exp4__prop.hit.F__figure.data[1,2])), "Familiarity",
                as.double(pull(exp4__prop.hit.F__figure.data[1,3])), 
                as.double(pull(exp4__prop.hit.F__figure.data[1,4])),
            as.character(pull(exp4__prop.hit.F__figure.data[2,1])), 
            as.character(pull(exp4__prop.hit.F__figure.data[2,2])), "Familiarity",
                as.double(pull(exp4__prop.hit.F__figure.data[2,3])), 
                as.double(pull(exp4__prop.hit.F__figure.data[2,4])),
            as.character(pull(exp4__prop.hit.F__figure.data[3,1])), 
            as.character(pull(exp4__prop.hit.F__figure.data[3,2])), "Familiarity",
                as.double(pull(exp4__prop.hit.F__figure.data[3,3])), 
                as.double(pull(exp4__prop.hit.F__figure.data[3,4])),
            as.character(pull(exp4__prop.hit.F__figure.data[4,1])), 
            as.character(pull(exp4__prop.hit.F__figure.data[4,2])), "Familiarity",
                as.double(pull(exp4__prop.hit.F__figure.data[4,3])), 
                as.double(pull(exp4__prop.hit.F__figure.data[4,4])),
            as.character(pull(exp4__prop.hit.F__figure.data[5,1])), 
            as.character(pull(exp4__prop.hit.F__figure.data[5,2])), "Familiarity",
                as.double(pull(exp4__prop.hit.F__figure.data[5,3])), 
                as.double(pull(exp4__prop.hit.F__figure.data[5,4])),
            as.character(pull(exp4__prop.hit.F__figure.data[6,1])), 
            as.character(pull(exp4__prop.hit.F__figure.data[6,2])), "Familiarity",
                as.double(pull(exp4__prop.hit.F__figure.data[6,3])), 
                as.double(pull(exp4__prop.hit.F__figure.data[6,4])),
            as.character(pull(exp4__prop.hit.G__figure.data[1,1])), 
            as.character(pull(exp4__prop.hit.G__figure.data[1,2])), "Guessing",
                as.double(pull(exp4__prop.hit.G__figure.data[1,3])), 
                as.double(pull(exp4__prop.hit.G__figure.data[1,4])),
            as.character(pull(exp4__prop.hit.G__figure.data[2,1])), 
            as.character(pull(exp4__prop.hit.G__figure.data[2,2])), "Guessing",
                as.double(pull(exp4__prop.hit.G__figure.data[2,3])), 
                as.double(pull(exp4__prop.hit.G__figure.data[2,4])),
            as.character(pull(exp4__prop.hit.G__figure.data[3,1])), 
            as.character(pull(exp4__prop.hit.G__figure.data[3,2])), "Guessing",
                as.double(pull(exp4__prop.hit.G__figure.data[3,3])), 
                as.double(pull(exp4__prop.hit.G__figure.data[3,4])),
            as.character(pull(exp4__prop.hit.G__figure.data[4,1])), 
            as.character(pull(exp4__prop.hit.G__figure.data[4,2])), "Guessing",
                as.double(pull(exp4__prop.hit.G__figure.data[4,3])), 
                as.double(pull(exp4__prop.hit.G__figure.data[4,4])),
            as.character(pull(exp4__prop.hit.G__figure.data[5,1])), 
            as.character(pull(exp4__prop.hit.G__figure.data[5,2])), "Guessing",
                as.double(pull(exp4__prop.hit.G__figure.data[5,3])), 
                as.double(pull(exp4__prop.hit.G__figure.data[5,4])),
            as.character(pull(exp4__prop.hit.G__figure.data[6,1])), 
            as.character(pull(exp4__prop.hit.G__figure.data[6,2])), "Guessing",
                as.double(pull(exp4__prop.hit.G__figure.data[6,3])), 
                as.double(pull(exp4__prop.hit.G__figure.data[6,4]))) %>%
        transform(resp_option = factor(resp_option, levels=c("RFG","RFBG"))) %>%
        transform(rfg = factor(rfg, levels=c("Recollection","Familiarity","Guessing"))) %>%
        mutate(stim = str_replace(stim, "word", "Words")) %>%
        mutate(stim = str_replace(stim, "col.drawing", "Colour\ndrawings")) %>%
        mutate(stim = str_replace(stim, "col.photo", "Colour\nphotographs")) %>%
        transform(stim = factor(stim, levels=c("Words", "Colour\ndrawings", "Colour\nphotographs")))
    
##  Create figure:
    exp4__prop.hit.RFG__figure <-
        ggplot(exp4__prop.hit.RFG__figure.data, aes(x = stim, y = emmean, fill = rfg)) +
        scale_fill_manual("legend", values = c("Recollection" = "#1B4251", "Familiarity" = "#6A6C93",
                                               "Guessing" = "#D491CA")) +
        geom_col(position = position_dodge()) +
        geom_errorbar(aes(ymin = pmax(emmean-SE, 0), ymax = emmean+SE), 
        ...
        theme(
            ...
            plot.margin = margin(20, 20, 24, 30))
```
```{r exp4__prop-rec-fam-guess-hits--figurecaption, echo=FALSE, results = FALSE, cache=FALSE}
    figure_numbering(name = "exp4__prop.hit.RFG__figure",
                     caption = "Proportion of hits assigned Recollection, Familiarity, and Guessing, by stimuli format and response option condition.")
```

#### Recollection and Familiarity
\hfill\break To determine the effects of stimuli format and response option on the classification of recognition memory judgements, separate 3 (stimuli format: words, drawings, photographs) x 2 (response option condition: RFG-judgements, RFBG-judgements) mixed ANOVAs were conducted on the mean proportion of hits assigned Recollection, Familiarity, and Guessing (see `r figure_numbering("exp4__prop.hit.RFG__figure", display = "cite")`).

```{r fig.height=8.2, echo=FALSE, warning = FALSE, message=FALSE, cache=TRUE}
    exp3__prop.hit.RFG__figure1 <-
        ggplot(exp3__prop.hit.RFG__figure.data, aes(x = stim, y = emmean, fill = rfg)) +
        scale_fill_manual("legend", values = c("Recollection" = "#202124", "Familiarity" = "#808083",
                                               "Guessing" = "#C0C0C0")) + 
        geom_col(position = position_dodge()) +
        geom_errorbar(aes(ymin = pmax(emmean-SE, 0), ymax = emmean+SE), 
                      width = 0.2, size = 0.3, position = position_dodge(0.9)) +
        facet_wrap(~resp_option, strip.position = "bottom") +
        labs(x = "Response option condition", y = "Mean proportion of hits", 
             subtitle = "Experiment 3: Greyscale") +
        scale_y_continuous(expand = c(0, 0), limits = c(0, 1)) +
        theme_classic() +    # Removes grey background and grid lines. 
        theme(
            strip.placement = "outside",
...
exp4__prop.hit.RFG__figure1 <- exp4__prop.hit.RFG__figure + labs(subtitle = "Experiment 4: Colour") + 
    theme(plot.subtitle = element_text(hjust = 0.5, face = "bold"))

grid.arrange(exp3__prop.hit.RFG__figure1, exp4__prop.hit.RFG__figure1,
                   ncol=1, nrow=2) #top = textGrob("Mean proportion of hits"))
```
`r figure_numbering("exp4__prop.hit.RFG__figure")`
...
```
**Recollection (hits):** Results from the ANOVA on the mean proportion of hits assigned Recollection showed a significant interaction between stimuli format and response option condition, `r apa::anova_apa(exp4__prop.hit.R.aov, effect = "resp_option:stim", format = "rmarkdown", print = FALSE)` (see `r figure_numbering("exp4__prop.hit.R__interaction.figure", display = "cite")`).
...
##################################################################################################################
##  Two-way mixed ANOVA:
    exp4__prop.FA.G.aov <- afex::aov_ez('id', 'prop.FA.G', fun_aggregate = mean, exp4__prop.FA.G, 
                                       between = "resp_option", within = 'stim',
                                       anova_table=list(correction = "none", es = "ges"))
##  Mauchlys test of sphericity:
    exp4__prop.FA.G.aov__sphericity <- test_sphericity(exp4__prop.FA.G.aov)
##  Assumption of sphericity not violated (p>.05), so no correction applied.
##----------------------------------------------------------------------------------------------------------------- ##  No significant main effect of RESPONSE OPTION.
##----------------------------------------------------------------------------------------------------------------- ##  Significant main effect of STIMULI FORMAT, so perform paired contrasts:
    exp4__prop.FA.G__pairwise__stim.format <- exp4__prop.FA.G %>% 
        pairwise_t_test(prop.FA.G ~ stim, paired = TRUE, p.adjust.method = "bonferroni")   
##  Calculate effect size:      
    exp4__prop.FA.G__pairwise__stim.format__eff.size <- exp4__prop.FA.G %>% 
        rstatix::cohens_d(prop.FA.G ~ stim, paired = TRUE, ci = TRUE, conf.level = 0.95)
##  t-test - colour photos vs. words:
    temp <- exp4__prop.FA.G__pairwise__stim.format %>% filter(group1 == "col.photo" & group2 == "word")
    exp4__prop.FA.G__ttest__col.photo.vs.word <- str_c(ttest.writeup(temp), "; *d* = ", 
        round(as.vector(exp4__prop.FA.G__pairwise__stim.format__eff.size %>% 
                          filter(group1 == "col.photo" & group2 == "word") %>% pull(effsize)), 2),
        ", 95% CI [", 
        exp4__prop.FA.G__pairwise__stim.format__eff.size %>%
            filter(group1 == "col.photo" & group2 == "word") %>% pull(conf.low), ", ",
                    exp4__prop.FA.G__pairwise__stim.format__eff.size %>%
                        filter(group1 == "col.photo" & group2 == "word") %>% pull(conf.high), "]")      

##  t-test - col drawings vs. words:    
    temp <- exp4__prop.FA.G__pairwise__stim.format %>% filter(group1 == "col.drawing" & group2 == "word")
    exp4__prop.FA.G__ttest__col.drawing.vs.word <- str_c(ttest.writeup(temp), "; *d* = ", 
        round(as.vector(exp4__prop.FA.G__pairwise__stim.format__eff.size %>% 
                          filter(group1 == "col.drawing" & group2 == "word") %>% pull(effsize)), 2),
        ", 95% CI [", 
        exp4__prop.FA.G__pairwise__stim.format__eff.size %>%
...
##  t-test - col drawings vs. col photos:    
    temp <- exp4__prop.FA.G__pairwise__stim.format %>% filter(group1 == "col.drawing" & group2 == "col.photo")
    exp4__prop.FA.G__ttest__col.photo.vs.col.drawing <- str_c(ttest.writeup(temp), "; *d* = ", 
        round(as.vector(exp4__prop.FA.G__pairwise__stim.format__eff.size %>% 
                          filter(group1 == "col.drawing" & group2 == "col.photo") %>% pull(effsize)), 2),
        ", 95% CI [", 
        exp4__prop.FA.G__pairwise__stim.format__eff.size %>%
            filter(group1 == "col.drawing" & group2 == "col.photo") %>% pull(conf.low), ", ",
                    exp4__prop.FA.G__pairwise__stim.format__eff.size %>%
                        filter(group1 == "col.drawing" & group2 == "col.photo") %>% pull(conf.high), "]")        
##----------------------------------------------------------------------------------------------------------------- ##  Get all means:
    exp4__prop.FA.G__stim.means <- as.tibble(summary(emmeans(exp4__prop.FA.G.aov, ~stim)))
    exp4__prop.FA.G__resp.option.means <- as.tibble(summary(emmeans(exp4__prop.FA.G.aov, ~resp_option)))
    
##################################################################################################################
##    FAs FIGURE:                                                                                               ##
##################################################################################################################
##  Prep data for figures:  
    exp4__prop.FA.R__figure.data <- 
        as.tibble(emmeans(exp4__prop.FA.R.aov, ~ resp_option:stim)) %>%
        select(-df, -lower.CL, -upper.CL)
    exp4__prop.FA.F__figure.data <- 
        as.tibble(emmeans(exp4__prop.FA.F.aov, ~ resp_option:stim)) %>%
        select(-df, -lower.CL, -upper.CL)
    exp4__prop.FA.G__figure.data <- 
        as.tibble(emmeans(exp4__prop.FA.G.aov, ~ resp_option:stim)) %>%
        select(-df, -lower.CL, -upper.CL)    

    exp4__prop.FA.RFG__figure.data <- 
        tribble(~"resp_option", ~"stim", ~"rfg", ~"emmean", ~"SE",
            as.character(pull(exp4__prop.FA.R__figure.data[1,1])), ... = 
            as.character(pull(exp4__prop.FA.R__figure.data[1,2])), "Recollection",
                as.double(pull(exp4__prop.FA.R__figure.data[1,3])), 
                as.double(pull(exp4__prop.FA.R__figure.data[1,4])),
            as.character(pull(exp4__prop.FA.R__figure.data[2,1])), 
            as.character(pull(exp4__prop.FA.R__figure.data[2,2])), "Recollection",
                as.double(pull(exp4__prop.FA.R__figure.data[2,3])), 
                as.double(pull(exp4__prop.FA.R__figure.data[2,4])),
            as.character(pull(exp4__prop.FA.R__figure.data[3,1])), 
            as.character(pull(exp4__prop.FA.R__figure.data[3,2])), "Recollection",
                as.double(pull(exp4__prop.FA.R__figure.data[3,3])), 
                as.double(pull(exp4__prop.FA.R__figure.data[3,4])),
            as.character(pull(exp4__prop.FA.R__figure.data[4,1])), 
            as.character(pull(exp4__prop.FA.R__figure.data[4,2])), "Recollection",
                as.double(pull(exp4__prop.FA.R__figure.data[4,3])), 
                as.double(pull(exp4__prop.FA.R__figure.data[4,4])),
            as.character(pull(exp4__prop.FA.R__figure.data[5,1])), 
            as.character(pull(exp4__prop.FA.R__figure.data[5,2])), "Recollection",
                as.double(pull(exp4__prop.FA.R__figure.data[5,3])), 
                as.double(pull(exp4__prop.FA.R__figure.data[5,4])),
            as.character(pull(exp4__prop.FA.R__figure.data[6,1])), 
            as.character(pull(exp4__prop.FA.R__figure.data[6,2])), "Recollection",
                as.double(pull(exp4__prop.FA.R__figure.data[6,3])), 
                as.double(pull(exp4__prop.FA.R__figure.data[6,4])),
            as.character(pull(exp4__prop.FA.F__figure.data[1,1])), 
            as.character(pull(exp4__prop.FA.F__figure.data[1,2])), "Familiarity",
                as.double(pull(exp4__prop.FA.F__figure.data[1,3])), 
                as.double(pull(exp4__prop.FA.F__figure.data[1,4])),
            as.character(pull(exp4__prop.FA.F__figure.data[2,1])), 
            as.character(pull(exp4__prop.FA.F__figure.data[2,2])), "Familiarity",
                as.double(pull(exp4__prop.FA.F__figure.data[2,3])), 
                as.double(pull(exp4__prop.FA.F__figure.data[2,4])),
            as.character(pull(exp4__prop.FA.F__figure.data[3,1])), 
            as.character(pull(exp4__prop.FA.F__figure.data[3,2])), "Familiarity",
                as.double(pull(exp4__prop.FA.F__figure.data[3,3])), 
                as.double(pull(exp4__prop.FA.F__figure.data[3,4])),
            as.character(pull(exp4__prop.FA.F__figure.data[4,1])), 
            as.character(pull(exp4__prop.FA.F__figure.data[4,2])), "Familiarity",
                as.double(pull(exp4__prop.FA.F__figure.data[4,3])), 
                as.double(pull(exp4__prop.FA.F__figure.data[4,4])),
            as.character(pull(exp4__prop.FA.F__figure.data[5,1])), 
            as.character(pull(exp4__prop.FA.F__figure.data[5,2])), "Familiarity",
                as.double(pull(exp4__prop.FA.F__figure.data[5,3])), 
                as.double(pull(exp4__prop.FA.F__figure.data[5,4])),
            as.character(pull(exp4__prop.FA.F__figure.data[6,1])), 
            as.character(pull(exp4__prop.FA.F__figure.data[6,2])), "Familiarity",
                as.double(pull(exp4__prop.FA.F__figure.data[6,3])), 
                as.double(pull(exp4__prop.FA.F__figure.data[6,4])),
            as.character(pull(exp4__prop.FA.G__figure.data[1,1])), 
            as.character(pull(exp4__prop.FA.G__figure.data[1,2])), "Guessing",
                as.double(pull(exp4__prop.FA.G__figure.data[1,3])), 
                as.double(pull(exp4__prop.FA.G__figure.data[1,4])),
            as.character(pull(exp4__prop.FA.G__figure.data[2,1])), 
            as.character(pull(exp4__prop.FA.G__figure.data[2,2])), "Guessing",
                as.double(pull(exp4__prop.FA.G__figure.data[2,3])), 
                as.double(pull(exp4__prop.FA.G__figure.data[2,4])),
            as.character(pull(exp4__prop.FA.G__figure.data[3,1])), 
            as.character(pull(exp4__prop.FA.G__figure.data[3,2])), "Guessing",
                as.double(pull(exp4__prop.FA.G__figure.data[3,3])), 
                as.double(pull(exp4__prop.FA.G__figure.data[3,4])),
            as.character(pull(exp4__prop.FA.G__figure.data[4,1])), 
            as.character(pull(exp4__prop.FA.G__figure.data[4,2])), "Guessing",
                as.double(pull(exp4__prop.FA.G__figure.data[4,3])), 
                as.double(pull(exp4__prop.FA.G__figure.data[4,4])),
            as.character(pull(exp4__prop.FA.G__figure.data[5,1])), 
            as.character(pull(exp4__prop.FA.G__figure.data[5,2])), "Guessing",
                as.double(pull(exp4__prop.FA.G__figure.data[5,3])), 
                as.double(pull(exp4__prop.FA.G__figure.data[5,4])),
            as.character(pull(exp4__prop.FA.G__figure.data[6,1])), 
            as.character(pull(exp4__prop.FA.G__figure.data[6,2])), "Guessing",
                as.double(pull(exp4__prop.FA.G__figure.data[6,3])), 
                as.double(pull(exp4__prop.FA.G__figure.data[6,4]))) %>%
        transform(resp_option = factor(resp_option, levels=c("RFG","RFBG"))) %>%
        transform(rfg = factor(rfg, levels=c("Recollection","Familiarity","Guessing"))) %>%
        mutate(stim = str_replace(stim, "word", "Words")) %>%
        mutate(stim = str_replace(stim, "col.drawing", "Colour\ndrawings")) %>%
...
##  Create figure:
    exp4__prop.FA.RFG__figure <-
        ggplot(exp4__prop.FA.RFG__figure.data, aes(x = stim, y = emmean, fill = rfg)) +
        scale_fill_manual("legend", values = c("Recollection" = "#1B4251", "Familiarity" = "#6A6C93",
                                               "Guessing" = "#D491CA")) +
        geom_col(position = position_dodge()) +
        geom_errorbar(aes(ymin = pmax(emmean-SE, 0), ymax = emmean+SE), 
...
Separate 3 (stimuli format: words, drawings, photographs) x 2 (response option condition: RFG-judgements, RFBG-judgements) mixed ANOVAs were also conducted on the mean proportion of FAs assigned Recollection, Familiarity, and Guessing (see `r figure_numbering("exp4__prop.FA.RFG__figure", display = "cite")`). 

```{r echo=FALSE, results = FALSE, cache=FALSE}
    figure_numbering(name = "exp4__prop.FA.RFG__figure",
                     caption = "Proportion of FAs assigned Recollection, Familiarity, and Guessing, by stimuli format and response option condition.")
```

```{r fig.height=8.2, echo=FALSE, warning = FALSE, message=FALSE, cache=TRUE}
    exp3__prop.FA.RFG__figure1 <-
        ggplot(exp3__prop.FA.RFG__figure.data, aes(x = stim, y = emmean, fill = rfg)) +
        scale_fill_manual("legend", values = c("Recollection" = "#202124", "Familiarity" = "#808083",
                                               "Guessing" = "#C0C0C0")) + 
        geom_col(position = position_dodge()) +
    ...
    exp4__prop.FA.RFG__figure1 <- exp4__prop.FA.RFG__figure + labs(subtitle = "Experiment 4: Colour") + 
        theme(plot.subtitle = element_text(hjust = 0.5, face = "bold"))

    grid.arrange(exp3__prop.FA.RFG__figure1, exp4__prop.FA.RFG__figure1,
                   ncol=1, nrow=2) #top = textGrob("Mean proportion of FAs"))
```
`r figure_numbering("exp4__prop.FA.RFG__figure")`

**Recollection (FAs):** For FAs assigned *Recollection*, there was no significant main effect of stimuli format [`r apa::anova_apa(exp4__prop.FA.R.aov, effect = "stim", format = "rmarkdown", print = FALSE)` or interaction [`r apa::anova_apa(exp4__prop.FA.R.aov, effect = "resp_option:stim", format = "rmarkdown", print = FALSE)`].
...
(*M*= `r round(exp4__prop.FA.F__stim.means %>% filter(stim == "col.drawing") %>% pull(emmean),2)`), `r exp4__prop.FA.F__ttest__col.photo.vs.col.drawing`. There were no significant interaction effects between stimuli format and response option condition, `r apa::anova_apa(exp4__prop.FA.F.aov, effect = "resp_option:stim", format = "rmarkdown", print = FALSE)`.

**Guessing (FAs):** The ANOVA on the mean proportion of FAs assigned *Guessing* demonstrated a significant main effect of stimuli format `r apa::anova_apa(exp4__prop.FA.G.aov, effect = "stim", format = "rmarkdown", print = FALSE)`. Colour photographs (*M*= `r round(exp4__prop.FA.G__stim.means %>% filter(stim == "col.photo") %>% pull(emmean),2)`) produced significantly fewer G FAs in comparison to words (*M*= `r round(exp4__prop.FA.G__stim.means %>% filter(stim == "word") %>% pull(emmean),2)`), `r exp4__prop.FA.G__ttest__col.photo.vs.word`). Likewise, colour drawings (*M*= `r round(exp4__prop.FA.G__stim.means %>% filter(stim == "col.drawing") %>% pull(emmean),2)`) also showed a significantly lower proportion of FAs compared to words (*M*= `r round(exp4__prop.FA.G__stim.means %>% filter(stim == "word") %>% pull(emmean),2)`), `r exp4__prop.FA.G__ttest__col.drawing.vs.word`. However, there was no significant difference in the proportion of FAs assigned Guessing between colour photographs (*M*= `r round(exp4__prop.FA.G__stim.means %>% filter(stim == "col.photo") %>% pull(emmean),2)`) and colour drawings (*M*= `r round(exp4__prop.FA.G__stim.means %>% filter(stim == "col.drawing") %>% pull(emmean),2)`), `r exp4__prop.FA.G__ttest__col.photo.vs.col.drawing`). There were also no significant interaction effects between stimuli format and response option condition `r apa::anova_apa(exp4__prop.FA.G.aov, effect = "resp_option:stim", format = "rmarkdown", print = FALSE)`.

Visual inspection of the data in `r figure_numbering("exp4__prop.hit.RFG__figure", display = "cite")` and `r figure_numbering("exp4__prop.FA.RFG__figure", display = "cite")` demonstrates a highly similar pattern of responding between *Experiment 3* and the current study, and suggest the addition of colour had little impact on RFG response patterns. 

```{r echo=FALSE, results = FALSE, message=FALSE, warning=FALSE, cache=TRUE, fig.show='hide'}
##################################################################################################################
##  exp4 Resp option main effects table:                                                                        ##
##################################################################################################################
    exp4__resp.option__main.effects__table <- tribble(
        ~"Variable", ~"Main effect of response option", ~"Signif.",
        "Mean proportion: ***Hits***",
            apa::anova_apa(exp4__prop.hit.aov, effect = "resp_option", format = "rmarkdown", print = FALSE),
            "",
        "Mean proportion: ***FAs***",
            apa::anova_apa(exp4__prop.FA.aov, effect = "resp_option", format = "rmarkdown", print = FALSE),
            "*",
        "Mean scores: ***d'***",  
            apa::anova_apa(exp4__d.prime.aov, effect = "resp_option", format = "rmarkdown", print = FALSE),
            "",
        "Mean proportion: ***Recollection hits***",
            apa::anova_apa(exp4__prop.hit.R.aov, effect = "resp_option", format = "rmarkdown", print = FALSE),
            "\\+", 
        "Mean proportion: ***Familiarity hits***",
            apa::anova_apa(exp4__prop.hit.F.aov, effect = "resp_option", format = "rmarkdown", print = FALSE),
            "", 
        "Mean proportion: ***Guessing hits***", 
            apa::anova_apa(exp4__prop.hit.G.aov, effect = "resp_option", format = "rmarkdown", print = FALSE),
            "", 
        "Mean proportion: ***Recollection FAs***",
            apa::anova_apa(exp4__prop.FA.R.aov, effect = "resp_option", format = "rmarkdown", print = FALSE),
            "", 
        "Mean proportion: ***Familiarity FAs***",
            apa::anova_apa(exp4__prop.FA.F.aov, effect = "resp_option", format = "rmarkdown", print = FALSE),
            "", 
        "Mean proportion: ***Guessing FAs***",
            apa::anova_apa(exp4__prop.FA.G.aov, effect = "resp_option", format = "rmarkdown", print = FALSE),
            "")
```

```{r echo=FALSE, results = FALSE, cache=FALSE}
table_numbering(name = "exp4__resp.option__main.effects__table", caption = "Main effects of response option condition across all variables of interest. Signif. codes:  \\*\\*\\**p* < .001; \\*\\**p* < .01; \\**p* < .05; \\+ involved in significant interaction [see previous section for interpretation]).")
```

#### Response option availability
\hfill\break In each of the aforementioned ANOVAs, the role of response option was also examined to determine whether the addition of colour information had any differential effects compared to the findings of *Experiment 3*. The only main effect of response option was observed in the ANOVA for the mean proportion of FAs (`r apa::anova_apa(exp3__prop.FA.aov, effect = "resp_option", format = "rmarkdown", print = FALSE)`), whereby a higher proportion of FAs was evident in the RFG group (*M*= `r round(exp4__prop.FA__resp.option.means %>% filter(resp_option == "RFG") %>% pull(emmean),2)`) compared to the RFBG group (*M*= `r round(exp4__prop.FA__resp.option.means %>% filter(resp_option == "RFBG") %>% pull(emmean),2)`), `r str_c(papaja::apa_print(exp4__prop.FA__ttest__rfg.rfbg)$statistic %>% str_remove_all("[$*]") %>% str_replace("t", "*t*") %>% str_replace("p", "*p*"), ", *d* = ", round(abs(as.vector(exp4__prop.FA__ttest__rfg.rfbg__eff.size$effsize)), 2))`. Such results do not support Hypothesis 3, where significant main effects of response option were expected in the ANOVAs for the mean proportion of hits and mean proportion of FAs assigned *Familiarity*. Similarly, the expected interactions for mean *d'* scores and proportion of hits assigned *Familiarity* were also absent. The only consistent finding across *Experiment 3* and the current experiment (with regard to response option) comes from the ANOVA for the mean proportion of hits assigned *Recollection* - where a significant interaction between stimuli format and response option was apparent in both experiments; in both experiments, the proportion of R hits did not differ across RFG and RFBG groups for word stimuli, whereas for drawings and photographs, the RFG group produced a significantly higher proportion of R hits than the RFBG group.
...
#Finally, the significant interaction for G hits demonstrated that word stimuli produced significantly fewer G hits in the RF-Ratings group (`r exp1__rfr_words_guess_hit_mean`) compared to both the RFG group (`r exp1__rfg_words_guess_hit_mean`), `r exp1__apa_anova_prop_guess_hit_interaction_pairwise_resp_optionoption$statistic$Words_RFG_RF_Ratings`, and the RFBG group (`r exp1__rfbg_words_guess_hit_mean`), `r exp1__apa_anova_prop_guess_hit_interaction_pairwise_resp_optionoption$statistic$Words_RFBG_RF_Ratings`. Likewise, pictures also produced significantly fewer G hits in the RF-Ratings group (`r exp1__rfr_pics_guess_hit_mean`) compared to both the RFG group (`r exp1__rfg_pics_guess_hit_mean`), `r exp1__apa_anova_prop_guess_hit_interaction_pairwise_resp_optionoption$statistic$Pictures_RFG_RF_Ratings` and RFBG group (`r exp1__rfbg_pics_guess_hit_mean`), `r exp1__apa_anova_prop_guess_hit_interaction_pairwise_resp_optionoption$statistic$Pictures_RFBG_RF_Ratings`. For G FAs, a significant main effect of resp_optiononse-option, `r exp1__apa_anova_prop_guess_FA$full$resp_optiononseOption` revealed that the RF-ratings group (`r exp1__rfr_guess_FA_mean`) again showed significantly fewer G FAs than both the RFG (`r exp1__rfg_guess_FA_mean`), `r exp1__rfg_rfr_guess_FA_ttest`, and RFBG groups (`r exp1__rfbg_guess_FA_mean`), `r exp1__rfbg_rfr_guess_FA_ttest`. These findings again align with the previous results that suggest those in the RF-Ratings group resp_optiononded conservatively overall, and were less likely to resp_optionond *Old* than the other groups unless they felt a high level of certainty. 
    ...
    #exp4__resp.option__main.effects__table %>%
        #kable("markdown", booktabs = T, full_width= T, align=rep('r', 3))
```
...
Such findings will establish a successful replication of the @ensor2019b manipulation, and allow for the examination of PSE elimination effects in the current paradigm. 
...
##  Create raw df from the above files, but only get the following columns:
    exp5__participant.info <- map_dfr(exp5__p_info_files,~read_csv(.x, col_types = cols_only(
...
##  Add 'exp5__study_totals' to 'exp5__participant.info' df:
    exp5__participant.info <- exp5__participant.info %>%
...
##  Merge 'exp5__participant.info' and 'exp5__test.stats' to create final tidy df: 
    exp5__final.data <- exp5__participant.info %>%
...
##  Prep data for participants section:
##----------------------------------------------------------------------------------------------------------------
    exp5__age.gender <- exp5__participant.info %>%
        select(age, gender) %>%
        mutate(age = as.double(age))

    exp5__age.gender__table <- tribble(~"Gender", ~N, ~Age, ~SD,
        "Female", exp5__age.gender %>% filter(gender == "female") %>% nrow(),
        exp5__age.gender %>% filter(gender == "female") %>% summarise(mean(age)) %>% pull() %>% round (2),
        exp5__age.gender %>% filter(gender == "female") %>% summarise(sd(age)) %>% pull() %>% round (2),
        "Male", exp5__age.gender %>% filter(gender == "male") %>% nrow(),
        exp5__age.gender %>% filter(gender == "male") %>% summarise(mean(age)) %>% pull() %>% round (2),
        exp5__age.gender %>% filter(gender == "male") %>% summarise(sd(age)) %>% pull() %>% round (2),
        "Female/Non-binary", exp5__age.gender %>% filter(gender == "female/non-binary") %>% nrow(),
        exp5__age.gender %>% filter(gender == "female/non-binary") %>% summarise(mean(age)) %>% pull(),0,
        "FTM Transgender", exp5__age.gender %>% filter(gender == "ftm transgender") %>% nrow(),
        exp5__age.gender %>% filter(gender == "ftm transgender") %>% summarise(mean(age)) %>% pull(),0,
        "Unspecified", exp5__age.gender %>% filter(gender == "unspecified") %>% nrow(),
        exp5__age.gender %>% filter(gender == "unspecified") %>% summarise(mean(age)) %>% pull(),0,
        "Total", exp5__age.gender %>% nrow(),
        exp5__age.gender %>% summarise(mean(age)) %>% pull() %>% round (2),
        exp5__age.gender %>% summarise(sd(age)) %>% pull() %>% round (2))

    exp5__source <- as_tibble(exp5__participant.info$prolific_id)
    exp5__source <- exp5__source %>% mutate(length = str_length(value)) %>%
        mutate(source = ifelse(length == 5, "sona", ifelse(length == 3, "social.media",
                                                           ifelse(length == 24, "prolific", value))))
    
    exp5__n.prolific <- nrow(exp5__source %>% filter(source == "prolific"))
    exp5__n.prolific <- round((exp5__n.prolific / nrow(exp5__source)) * 100, 2)
    
    exp5__n.sona <- nrow(exp5__source %>% filter(source == "sona"))
    exp5__n.sona <- round((exp5__n.sona / nrow(exp5__source)) * 100, 2)
    
    exp5__n.social.media <- nrow(exp5__source %>% filter(source == "social.media"))
    exp5__n.social.media <- round((exp5__n.social.media / nrow(exp5__source)) * 100, 2)
    
    exp5__n.eng.lang <- nrow(exp5__participant.info %>% filter(eng_first_lang == "yes"))
    exp5__n.eng.lang <- round((exp5__n.eng.lang / nrow(exp5__participant.info)) * 100, 2)
```
```{r echo=FALSE, results = FALSE, cache=FALSE}
    table_numbering(name = "exp5__age.gender__table", caption = "Gender and age (*SD*) of the current sample.")
```    

### Method
#### Participants
\hfill\break A total of `r nrow(exp5__participant.info)` participants completed the online experiment, all between the ages `r min(exp5__age.gender$age, na.rm = TRUE)`-`r max(exp5__age.gender$age, na.rm = TRUE)` years (see `r table_numbering("exp5__age.gender__table", display = "cite")` for a breakdown of the age/gender of the current sample). The majority of participants reported English as being their first language (`r exp5__n.eng.lang`%). Participants were primarily recruited from the in-house \href{https://keelepsychology.sona-systems.com/}{research participation system} (`r exp5__n.sona`%), though some were also sourced from the voluntary participation website \href{https://www.prolific.co/}{Prolific Academic} (`r exp5__n.prolific`%), where payment at the rate of £5/hr was given, and from other online social media (e.g. Facebook; `r exp5__n.social.media`%). Sample size was calculated a-priori using \href{https://www.psychologie.hhu.de/arbeitsgruppen/allgemeine-psychologie-und-arbeitspsychologie/gpower}{G*Power} [@faul2009] to detect a small effect size of Cohen’s *f* = 0.1 with 95% power (*α* = .05, two-tailed), 166 subjects would be necessary in a one-way repeated measures ANOVA.

&nbsp;
&nbsp;

`r table_numbering("exp5__age.gender__table")`
```{r echo=FALSE, cache=TRUE}
    exp5__age.gender__table %>%
        mutate(Age = round(Age,2)) %>%
        mutate(SD = round(SD,2)) %>%
        mutate("(SD)" = str_c("(", SD, ")")) %>%
        select(-SD) %>%
...
##    Assumption of sphericity violated (p<.05), so apply Greenhouse-Geisser correction:
      exp5__prop.hit.aov <- afex::aov_ez('id', 'prop.hit', fun_aggregate = mean, exp5__prop.hit, within = 'stim', 
                        anova_table=list(correction = "GG", es = "ges"))
#-----------------------------------------------------------------------------------------------------------------
##  Significant main effect, so perform Paired Samples t-tests:
  ##  Standard PSE:
      exp5__prop.hit__ttest__grey.word.vs.grey.drawing <- 
          exp5__prop.hit %>% filter(stim == "grey.word" | stim == "grey.drawing") %>%
              t.test(prop.hit~stim, paired=TRUE, data = .)    
      exp5__prop.hit__ttest__grey.word.vs.grey.photo <- 
          exp5__prop.hit %>% filter(stim == "grey.word" | stim == "grey.photo") %>%
              t.test(prop.hit~stim, paired=TRUE, data = .)
      exp5__prop.hit__ttest__grey.word.vs.col.drawing <- 
          exp5__prop.hit %>% filter(stim == "grey.word" | stim == "col.drawing") %>%
              t.test(prop.hit~stim, paired=TRUE, data = .)    
      exp5__prop.hit__ttest__grey.word.vs.col.photo <- 
          exp5__prop.hit %>% filter(stim == "grey.word" | stim == "col.photo") %>%
              t.test(prop.hit~stim, paired=TRUE, data = .)
##  Photo superiority:
      exp5__prop.hit__ttest__grey.drawing.vs.grey.photo <- 
          exp5__prop.hit %>% filter(stim == "grey.drawing" | stim == "grey.photo") %>%
              t.test(prop.hit~stim, paired=TRUE, data = .)
      exp5__prop.hit__ttest__col.drawing.vs.col.photo <- 
          exp5__prop.hit %>% filter(stim == "col.drawing" | stim == "col.photo") %>%
              t.test(prop.hit~stim, paired=TRUE, data = .)
      exp5__prop.hit__ttest__col.drawing.vs.grey.photo <- 
          exp5__prop.hit %>% filter(stim == "col.drawing" | stim == "grey.photo") %>%
              t.test(prop.hit~stim, paired=TRUE, data = .)
      
  ##  Distinctive words:        
      exp5__prop.hit__ttest__grey.word.vs.col.word <- 
          exp5__prop.hit %>% filter(stim == "grey.word" | stim == "col.word") %>%
              t.test(prop.hit~stim, paired=TRUE, data = .)
...
##    Get means:
      ...
      exp5__prop.hit.means__col.drawing <- summary(exp5__prop.hit.means)$emmean[4] %>% round(2)
      exp5__prop.hit.means__grey.photo <- summary(exp5__prop.hit.means)$emmean[5] %>% round(2)
      exp5__prop.hit.means__col.photo <- summary(exp5__prop.hit.means)$emmean[6] %>% round(2)
##################################################################################################################
##    Exp5 Prop FAs:                                                                                            ##
##################################################################################################################
##    One-way repeated measures ANOVA:
...
##  Significant main effect, so perform Paired Samples t-tests:
  ##  Standard PSE:      
      exp5__prop.FA__ttest__grey.word.vs.grey.drawing <- 
          exp5__prop.FA %>% filter(stim == "grey.word" | stim == "grey.drawing") %>%
              t.test(prop.FA~stim, paired=TRUE, data = .)    
      exp5__prop.FA__ttest__grey.word.vs.grey.photo <- 
          exp5__prop.FA %>% filter(stim == "grey.word" | stim == "grey.photo") %>%
              t.test(prop.FA~stim, paired=TRUE, data = .)
      exp5__prop.FA__ttest__grey.word.vs.col.drawing <- 
          exp5__prop.FA %>% filter(stim == "grey.word" | stim == "col.drawing") %>%
              t.test(prop.FA~stim, paired=TRUE, data = .)    
      exp5__prop.FA__ttest__grey.word.vs.col.photo <- 
          exp5__prop.FA %>% filter(stim == "grey.word" | stim == "col.photo") %>%
              t.test(prop.FA~stim, paired=TRUE, data = .)
##  Photo superiority:
      exp5__prop.FA__ttest__grey.drawing.vs.grey.photo <- 
          exp5__prop.FA %>% filter(stim == "grey.drawing" | stim == "grey.photo") %>%
              t.test(prop.FA~stim, paired=TRUE, data = .)
      exp5__prop.FA__ttest__col.drawing.vs.col.photo <- 
          exp5__prop.FA %>% filter(stim == "col.drawing" | stim == "col.photo") %>%
              t.test(prop.FA~stim, paired=TRUE, data = .)
      exp5__prop.FA__ttest__col.drawing.vs.grey.photo <- 
          exp5__prop.FA %>% filter(stim == "col.drawing" | stim == "grey.photo") %>%
              t.test(prop.FA~stim, paired=TRUE, data = .)
  ##  Distinctive words:
      exp5__prop.FA__ttest__grey.word.vs.col.word <- 
          exp5__prop.FA %>% filter(stim == "grey.word" | stim == "col.word") %>%
              t.test(prop.FA~stim, paired=TRUE, data = .)
...
##    Get means:
      ...
      exp5__prop.FA.means__col.drawing <- summary(exp5__prop.FA.means)$emmean[4] %>% round(2)
      exp5__prop.FA.means__grey.photo <- summary(exp5__prop.FA.means)$emmean[5] %>% round(2)
      exp5__prop.FA.means__col.photo <- summary(exp5__prop.FA.means)$emmean[6] %>% round(2)
...
##  Significant main effect, so perform Paired Samples t-tests:
  ##  Standard PSE:
      exp5__d.prime__ttest__grey.word.vs.grey.drawing <- 
          exp5__d.prime %>% filter(stim == "grey.word" | stim == "grey.drawing") %>%
              t.test(d.prime~stim, paired=TRUE, data = .)    
      exp5__d.prime__ttest__grey.word.vs.grey.photo <- 
          exp5__d.prime %>% filter(stim == "grey.word" | stim == "grey.photo") %>%
              t.test(d.prime~stim, paired=TRUE, data = .)
      exp5__d.prime__ttest__grey.word.vs.col.drawing <- 
          exp5__d.prime %>% filter(stim == "grey.word" | stim == "col.drawing") %>%
              t.test(d.prime~stim, paired=TRUE, data = .)    
      exp5__d.prime__ttest__grey.word.vs.col.photo <- 
          exp5__d.prime %>% filter(stim == "grey.word" | stim == "col.photo") %>%
              t.test(d.prime~stim, paired=TRUE, data = .)
##  Photo superiority:
      exp5__d.prime__ttest__grey.drawing.vs.grey.photo <- 
          exp5__d.prime %>% filter(stim == "grey.drawing" | stim == "grey.photo") %>%
              t.test(d.prime~stim, paired=TRUE, data = .)
      exp5__d.prime__ttest__col.drawing.vs.col.photo <- 
          exp5__d.prime %>% filter(stim == "col.drawing" | stim == "col.photo") %>%
              t.test(d.prime~stim, paired=TRUE, data = .)
      exp5__d.prime__ttest__col.drawing.vs.grey.photo <- 
          exp5__d.prime %>% filter(stim == "col.drawing" | stim == "grey.photo") %>%
              t.test(d.prime~stim, paired=TRUE, data = .)      
  ##  Distinctive words:
      exp5__d.prime__ttest__grey.word.vs.col.word <- 
          exp5__d.prime %>% filter(stim == "grey.word" | stim == "col.word") %>%
              t.test(d.prime~stim, paired=TRUE, data = .)
...
##    Get means:
      ...
      exp5__d.prime.means__col.drawing <- summary(exp5__d.prime.means)$emmean[4] %>% round(2)
      exp5__d.prime.means__grey.photo <- summary(exp5__d.prime.means)$emmean[5] %>% round(2)
      exp5__d.prime.means__col.photo <- summary(exp5__d.prime.means)$emmean[6] %>% round(2)      
...
##################################################################################################################
      ...
      exp5__prop.hit__grey.drawing <- mean_se(exp5__prop.hit %>% filter(stim == "grey.drawing") %>%
                                                pull(prop.hit))
...
To compare the results of the current study with previous findings, the raw data from the first two experiments of @ensor2019b was re-analysed in order to calculate statistics that were not reported in their paper. 

\newpage
#### Picture superiority (PSE): pictures (drawings + photographs) vs. regular words
Standard PSEs were apparent in the ANOVAs on the mean proportion of hits, mean proportion of FAs, and mean *d'* scores. Grey words (*M* = `r exp5__prop.hit.means__grey.word`) showed a significantly lower proportion of hits compared to all types of picture: grey drawings (*M* = `r exp5__prop.hit.means__grey.drawing`), `r apa::apa(exp5__prop.hit__ttest__grey.word.vs.grey.drawing, format = "rmarkdown", print = FALSE)`; grey photographs (*M* = `r exp5__prop.hit.means__grey.photo`), `r apa::apa(exp5__prop.hit__ttest__grey.word.vs.grey.photo, format = "rmarkdown", print = FALSE)`; colour drawings (*M* = `r exp5__prop.hit.means__col.drawing`), `r apa::apa(exp5__prop.hit__ttest__grey.word.vs.col.drawing, format = "rmarkdown", print = FALSE)`; and colour photographs (*M* = `r exp5__prop.hit.means__col.photo`), `r apa::apa(exp5__prop.hit__ttest__grey.word.vs.col.photo, format = "rmarkdown", print = FALSE)`. Grey words (*M* = `r exp5__prop.FA.means__grey.word`) also showed a significantly higher proportion of FAs compared to all types of picture: grey drawings (*M* = `r exp5__prop.FA.means__grey.drawing`), `r apa::apa(exp5__prop.FA__ttest__grey.word.vs.grey.drawing, format = "rmarkdown", print = FALSE)`; grey photographs (*M* = `r exp5__prop.FA.means__grey.photo`), `r apa::apa(exp5__prop.FA__ttest__grey.word.vs.grey.photo, format = "rmarkdown", print = FALSE)`; colour drawings (*M* = `r exp5__prop.FA.means__col.drawing`), `r apa::apa(exp5__prop.FA__ttest__grey.word.vs.col.drawing, format = "rmarkdown", print = FALSE)`; and colour photographs (*M* = `r exp5__prop.FA.means__col.photo`), `r apa::apa(exp5__prop.FA__ttest__grey.word.vs.col.photo, format = "rmarkdown", print = FALSE)`. Finally, mean *d'* scores were also significantly lower for grey words (*M* = `r exp5__d.prime.means__grey.word`) compared to all types of picture: grey drawings (*M* = `r exp5__d.prime.means__grey.drawing`), `r apa::apa(exp5__d.prime__ttest__grey.word.vs.grey.drawing, format = "rmarkdown", print = FALSE)`; grey photographs (*M* = `r exp5__d.prime.means__grey.photo`), `r apa::apa(exp5__d.prime__ttest__grey.word.vs.grey.photo, format = "rmarkdown", print = FALSE)`; colour drawings (*M* = `r exp5__d.prime.means__col.drawing`), `r apa::apa(exp5__d.prime__ttest__grey.word.vs.col.drawing, format = "rmarkdown", print = FALSE)`; and colour photographs (*M* = `r exp5__d.prime.means__col.photo`), `r apa::apa(exp5__d.prime__ttest__grey.word.vs.col.photo, format = "rmarkdown", print = FALSE)`.

\newpage
#### Photograph superiority (PhSE): photographs vs. drawings
Examining greyscale items only, a PhSE was apparent across each of the variables (as demonstrated previously in *Experiment 3*). Grey photographs (*M* = `r exp5__prop.hit.means__grey.photo`) showed a significantly higher proportion of hits compared to grey drawings (*M* = `r exp5__prop.hit.means__grey.drawing`), `r apa::apa(exp5__prop.hit__ttest__grey.drawing.vs.grey.photo, format = "rmarkdown", print = FALSE)`; grey photographs (*M* = `r exp5__prop.FA.means__grey.photo`) showed a significantly lower proportion of FAs compared to grey drawings (*M* = `r exp5__prop.FA.means__grey.drawing`), `r apa::apa(exp5__prop.FA__ttest__grey.drawing.vs.grey.photo, format = "rmarkdown", print = FALSE)`; and grey photographs (*M* = `r exp5__d.prime.means__grey.photo`) showed significantly higher mean *d'* scores than grey drawings (*M* = `r exp5__d.prime.means__grey.drawing`), `r apa::apa(exp5__d.prime__ttest__grey.drawing.vs.grey.photo, format = "rmarkdown", print = FALSE)`. Examining colour items only, a PhSE was similarly apparent across each of the variables (as demonstrated previously in *Experiment 4*). Colour photographs (*M* = `r exp5__prop.hit.means__col.photo`) showed a significantly higher proportion of hits compared to colour drawings (*M* = `r exp5__prop.hit.means__col.drawing`), `r apa::apa(exp5__prop.hit__ttest__col.drawing.vs.col.photo, format = "rmarkdown", print = FALSE)`; colour photographs (*M* = `r exp5__prop.FA.means__col.photo`) showed a significantly lower proportion of FAs compared to colour drawings (*M* = `r exp5__prop.FA.means__col.drawing`), `r apa::apa(exp5__prop.FA__ttest__col.drawing.vs.col.photo, format = "rmarkdown", print = FALSE)`; and colour photographs (*M* = `r exp5__d.prime.means__col.photo`) showed significantly higher mean *d'* scores than colour drawings (*M* = `r exp5__d.prime.means__col.drawing`), `r apa::apa(exp5__d.prime__ttest__col.drawing.vs.col.photo, format = "rmarkdown", print = FALSE)`. 

To examine the role of colour vs. greyscale in the PhSE, colour drawings were compared with greyscale photographs to examine whether the PhSE was eliminated. Colour drawings (*M* = `r exp5__prop.hit.means__col.drawing`) showed a significantly lower proportion of hits compared to grey photographs (*M* = `r exp5__prop.hit.means__grey.photo`),
`r apa::apa(exp5__prop.hit__ttest__col.drawing.vs.grey.photo, format = "rmarkdown", print = FALSE)`. Colour drawings also (*M* = `r exp5__d.prime.means__col.drawing`) showed a significantly lower mean *d'* scores compared to grey photographs (*M* = `r exp5__d.prime.means__grey.photo`),
`r apa::apa(exp5__d.prime__ttest__col.drawing.vs.grey.photo, format = "rmarkdown", print = FALSE)`. However, the PhSE was eliminated when comparing colour drawings (*M* = `r exp5__prop.FA.means__col.drawing`) and grey photographs (*M* = `r exp5__prop.FA.means__grey.photo`) on the mean proportion of FAs, where there was no difference between the formats, `r apa::apa(exp5__prop.FA__ttest__col.drawing.vs.grey.photo, format = "rmarkdown", print = FALSE)`.
      ...
      exp5__table__ensor.vs.current__hits <- 
            tribble(~" ", ~"Grey words", ~"Distinctive words", ~"Planned comparisons",
                  ...
                  "Trials: 60 study / 120 test:", 
                      ...
                      apa::apa(exp5__prop.hit__ttest__grey.word.vs.col.word, format = "rmarkdown", print = FALSE))
#################################################################################################################
      ...
      exp5__table__ensor.vs.current__FAs <- 
            tribble(~" ", ~"Grey words", ~"Distinctive words", ~"Planned comparisons",
                  ...
                  "Trials: 60 study / 120 test:", 
                      ...
                      apa::apa(exp5__prop.FA__ttest__grey.word.vs.col.word, format = "rmarkdown", print = FALSE))
#################################################################################################################
      ...
      exp5__table__ensor.vs.current__d.prime <- 
            tribble(~" ", ~"Grey words", ~"Distinctive words", ~"Planned comparisons",
                  ...
                  "Trials: 60 study / 120 test:", 
                      ...
                      apa::apa(exp5__d.prime__ttest__grey.word.vs.col.word, format = "rmarkdown", print = FALSE)) 
##################################################################################################################
##    Ensor vs. current full table:                                                                             ##
##################################################################################################################
      exp5__table__ensor.vs.current__full <- exp5__table__ensor.vs.current__hits %>%
        bind_rows(exp5__table__ensor.vs.current__FAs) %>%
        bind_rows(exp5__table__ensor.vs.current__d.prime)
```

```{r echo=FALSE, results = FALSE, cache=FALSE}
      table_numbering(name = "exp5__table__ensor.vs.current__full",
...
Analysis of the @ensor2019b data showed evidence of the distinctiveness manipulation. For the mean proportion of hits, there were significant main effects of stimuli format in their first [`r apa::anova_apa(ensor.exp1__prop.hit.aov, effect = "stim", format = "rmarkdown", print = FALSE)`] and second [`r apa::anova_apa(ensor.exp2__prop.hit.aov, effect = "stim", format = "rmarkdown", print = FALSE)`] experiments. Follow-up comparisons showed distinctive words resulted in significantly more hits than regular words across both experiments (see `r table_numbering("exp5__table__ensor.vs.current__hits", display = "cite")`). For the mean proportion of FAs, the first experiment of @ensor2019b showed no significant main effect of stimuli format [`r apa::anova_apa(ensor.exp1__prop.FA.aov, effect = "stim", format = "rmarkdown", print = FALSE)`], however, the second experiment did [`r apa::anova_apa(ensor.exp2__prop.FA.aov, effect = "stim", format = "rmarkdown", print = FALSE)`]; though follow-up comparisons showed no difference in the proportion of FAs between distinctive and regular words. Finally, for mean *d'* scores, there was a significant main effect of stimuli format in both the first [`r apa::anova_apa(ensor.exp1__d.prime.aov, effect = "stim", format = "rmarkdown", print = FALSE)`]
and second [`r apa::anova_apa(ensor.exp2__d.prime.aov, effect = "stim", format = "rmarkdown", print = FALSE)`] experiments. Follow-up comparisons showed evidence of the distinctiveness manipulation, whereby distinctive words produced significantly higher *d'* scores compared to regular words in both experiments (see `r table_numbering("exp5__table__ensor.vs.current__full", display = "cite")`).

&nbsp;

`r table_numbering("exp5__table__ensor.vs.current__full")`
```{r echo=FALSE, cache=FALSE}
    exp5__table__ensor.vs.current__full.table <- exp5__table__ensor.vs.current__full %>%
        kable("latex", booktabs = T, align = "lccl", escape = F) %>%
        kable_styling(font_size = 10, full_width = T, latex_options = c("hold_position", "condensed")) %>%
        pack_rows("Hits", 1, 3, italic = T) %>%
        pack_rows("Ensor, Surprenant, & Neath (2019):", 1, 2) %>%
        pack_rows("Current study:", 3, 3) %>%
        pack_rows("FAs", 4, 6, italic = T) %>%
        pack_rows("Ensor, Surprenant, & Neath (2019):", 4, 5) %>%
        pack_rows("Current study:", 6, 6) %>%
        pack_rows("d'", 7, 9, italic = T) %>% 
        pack_rows("Ensor, Surprenant, & Neath (2019):", 7, 8) %>%
        pack_rows("Current study:", 9, 9) %>%
        column_spec(1, width = "5.5cm") %>%
        column_spec(2:3, width = "1.5cm") %>%
        row_spec(3, hline_after = TRUE) %>%
        row_spec(6, hline_after = TRUE)

knitr::asis_output(str_replace_all(exp5__table__ensor.vs.current__full.table, "\\*(t*?)\\*", "\textit{t}"))

```

\newpage
#### Elimination of PSE (distinctive words vs. drawings
```{r echo=FALSE, results = FALSE, message=FALSE, warning=FALSE, cache=TRUE, fig.show='hide'}
#################################################################################################################
##    Ensor vs. current prop hits table - PSE elimination:                                                     ##
#################################################################################################################
    ensor.exp1__prop.hit__ttest2 <- ensor.exp1__prop.hit %>% 
        filter(stim == "col.word" | stim == "grey.drawing") %>%
        t.test(prop.hit~stim, paired=TRUE, data = .)
    ensor.exp2__prop.hit__ttest2 <- ensor.exp2__prop.hit %>% 
        filter(stim == "col.word" | stim == "grey.drawing") %>%
        t.test(prop.hit~stim, paired=TRUE, data = .)
    exp5__prop.hit__ttest__grey.word.vs.col.word2 <- exp5__prop.hit %>% 
        filter(stim == "col.word" | stim == "grey.drawing") %>%
        t.test(prop.hit~stim, paired=TRUE, data = .)

    exp5__table__ensor.vs.current__prop.hit2 <- 
        tribble(~" ", ~"  ", ~"Distinctive words", ~"Grey drawings", ~"Planned comparisons",
            "Ensor, Surprenant, & Neath (2019)", "Trials: 40 study / 80 test:",
                round(mean(ensor.exp1.data$prop.hit__col.word),2),
                round(mean(ensor.exp1.data$prop.hit__grey.drawing),2),
                apa::apa(ensor.exp1__prop.hit__ttest2, format = "rmarkdown", print = FALSE),
            "Ensor, Surprenant, & Neath (2019)", "Trials: 80 study / 160 test:",
                  round(mean(ensor.exp2.data$prop.hit__col.word),2),
                  round(mean(ensor.exp2.data$prop.hit__grey.drawing),2),
                  apa::apa(ensor.exp2__prop.hit__ttest2, format = "rmarkdown", print = FALSE),
            "Current study", "Trials: 60 study / 120 test:", 
                  round(exp5__prop.hit %>% filter(stim == "col.word") %>% pull(prop.hit) %>% mean(),2),
                  round(exp5__prop.hit %>% filter(stim == "grey.drawing") %>% pull(prop.hit) %>% mean(),2),
                  apa::apa(exp5__prop.hit__ttest__grey.word.vs.col.word2, format = "rmarkdown", print = FALSE))      
##################################################################################################################
##    Ensor vs. current prop FAs table - PSE elimination:                                                       ##
##################################################################################################################
    ensor.exp1__prop.FA__ttest2 <- ensor.exp1__prop.FA %>% 
        filter(stim == "col.word" | stim == "grey.drawing") %>%
        t.test(prop.FA~stim, paired=TRUE, data = .)
    ensor.exp2__prop.FA__ttest2 <- ensor.exp2__prop.FA %>% 
        filter(stim == "col.word" | stim == "grey.drawing") %>%
        t.test(prop.FA~stim, paired=TRUE, data = .)
    exp5__prop.FA__ttest__grey.word.vs.col.word2 <- exp5__prop.FA %>% 
        filter(stim == "col.word" | stim == "grey.drawing") %>%
        t.test(prop.FA~stim, paired=TRUE, data = .)
    ...
    exp5__table__ensor.vs.current__prop.FA2 <- 
        tribble(~" ", ~"  ", ~"Distinctive words", ~"Grey drawings", ~"Planned comparisons",
            ...
            "Current study", "Trials: 60 study / 120 test:", 
                round(exp5__prop.FA %>% filter(stim == "col.word") %>% pull(prop.FA) %>% mean(),2),
                round(exp5__prop.FA %>% filter(stim == "grey.drawing") %>% pull(prop.FA) %>% mean(),2),
                apa::apa(exp5__prop.FA__ttest__grey.word.vs.col.word2, format = "rmarkdown", print = FALSE))
    
#################################################################################################################
##    Ensor vs. current d-prime table - PSE elimination:                                                       ##
################################################################################################################# 
    ensor.exp1__d.prime__ttest2 <- ensor.exp1__d.prime %>%
        filter(stim == "col.word" | stim == "grey.drawing") %>%
        t.test(d.prime~stim, paired=TRUE, data = .)
    ensor.exp2__d.prime__ttest2 <- ensor.exp2__d.prime %>% 
        filter(stim == "col.word" | stim == "grey.drawing") %>%
        t.test(d.prime~stim, paired=TRUE, data = .)
    exp5__d.prime__ttest2 <- exp5__d.prime %>% 
        filter(stim == "col.word" | stim == "grey.drawing") %>%
        t.test(d.prime~stim, paired=TRUE, data = .)
    ...
    exp5__table__ensor.vs.current__d.prime2 <- 
        tribble(~" ", ~"  ", ~"Distinctive words", ~"Grey drawings", ~"Planned comparisons",
            ...
            "Current study", "Trials: 60 study / 120 test:", 
                round(exp5__d.prime %>% filter(stim == "col.word") %>% pull(d.prime) %>% mean(),2),
                round(exp5__d.prime %>% filter(stim == "grey.drawing") %>% pull(d.prime) %>% mean(),2),
                apa::apa(exp5__d.prime__ttest2, format = "rmarkdown", print = FALSE))
##################################################################################################################
##    Ensor vs. current full table:                                                                             ##
##################################################################################################################
      exp5__table__ensor.vs.current2__full <- exp5__table__ensor.vs.current__prop.hit2 %>%
        bind_rows(exp5__table__ensor.vs.current__prop.FA2) %>%
        bind_rows(exp5__table__ensor.vs.current__d.prime2)
```      

```{r echo=FALSE, results = FALSE, cache=FALSE}
      table_numbering(name = "exp5__table__ensor.vs.current2__full",
...
A key finding in @ensor2019b data was an elimination of the PSE; they demonstrated no difference in the number of hits, FAs, or *d'* scores between the distinctive words and grey drawings across both experiments. Such findings suggest their manipulation was successful in increasing the distinctiveness of word stimuli. In the current experiment, however, these findings were not replicated; grey drawings showed superior performance in comparison to the distinctive words for the proportion of hits and mean *d'* scores. Interestingly though, there were significantly more FAs for the grey drawings, as opposed to the distinctive word stimuli. This is somewhat unexpected, since the rest of the findings identify the grey drawings as having increased memorability compared to the distinctive words.

\newpage
`r table_numbering("exp5__table__ensor.vs.current2__full")`
```{r echo=FALSE, cache=TRUE}
    exp5__table__ensor.vs.current2__full %>%
        kable("latex", booktabs = T, align = "rlccl") %>%
        kable_styling(font_size = 10, full_width = T, latex_options = c("hold_position", "condensed")) %>%
...
### Discussion
@ensor2019b demonstrated how picture superiority effects (PSEs) could be eliminated by increasing the distinctiveness of word stimuli (relative to shaded drawings) via manipulations to colour, font, size, and capitalisation. The current study had two primary aims: i) replicate this elimination of the PSE; ii) examine whether this effect can extend to attenuate / eliminate the *photograph* superiority effect (PhSE). 

Despite predictions that the distinctive word stimuli would produce a higher proportion of correct hits, lower proportion of false alarms (FAs), and higher overall *d'* scores compared to regular word stimuli, the results suggest the manipulation was not wholly effective. While a lower proportion of FAs and higher *d'* scores were found for the distinctive words, they did not differ from regular words in the proportion of obtained hits. Visual inspection of the data does not suggest the number of trials had an impact on the number of hits; while @ensor2019b did observe a reduction in the proportion of hits for both word formats when trial numbers were increased, the current study (whose trial numbers sit between the two @ensor2019b experiments) still showed a lower proportion of hits than both. Obtained *d'* scores sit between the @ensor2019b experiments - in line with reducing scores as the number of trials increases - but visual inspection of the data shows that subjects generally made more FAs in the @ensor2019b experiments compared to the current study; this is especially apparent when comparisons are made between the current experiment and the @ensor2019b experiment with more trials. Such results make it difficult to conclude whether the distinctive word stimuli used in the current experiment were indeed more distinctive than the regular words. In particular, one would expect items of a higher distinctiveness (and thus enhanced memorability) to specifically show a higher number of hits than items of lower distinctiveness. Indeed, this was the case in both @ensor2019b experiments, whereby the distinctive words produced a significantly higher proportion of hits compared to the regular words. The current findings could be interpreted within a DCT framework, whereby *all* words are inferior to the dual-codes of pictures (and thus 'distinctive' words will not produce better performance), there is still clear support for the distinctiveness hypothesis in the current results. While standard PSEs were expected - a finding that was abundantly clear from the data, with *all* pictures (greyscale drawings; colour drawings; greyscale photographs; colour photographs) demonstrating superior performance compared to regular words (manifesting as significantly higher proportions of hits, lower proportions of FAs, and a higher mean *d'* scores) - the *photo* superiority effects (PhSEs) of previous experiments was again evident. Whether recognition performance was compared between greyscale drawings and photographs, or colour drawings and photographs, the photograph items consistently demonstrated superior performance over drawings. A replication of the PhSE from *Experiment 3* (greyscale) and *Experiment 4* (colour) in the current study indicate that DCT is not sufficient to explain the current results. Rather, it seems the word stimuli of the current experiment failed to be enhanced to the level seen in @ensor2019b.

&nbsp;
&nbsp;

Not only were the distinctive words comparable to regular words for the number of hits, but as a result, they also did not clearly eliminate the PSE in comparison to drawings as was expected. This is somewhat surprising given the similarities between the distinctive word stimuli used in the current study and those utilised by @ensor2019b. The majority of items were those provided by the authors. There were some restraints, in that our photograph items had been matched with a specific subset of the @rossion2004 drawings. These items didn't match the stimuli used by @ensor2019b exactly, and so while many of the same stimuli could indeed be used, some new distinctive word items had to be created. This process was conducted carefully, using the same distinctive text generator, and using combinations utilised by @ensor2019b for words not in the current list (i.e. if @ensor2019b presented "truck" in a distinctive blue and capitalised font, this same combination was used in the current study, but the object "truck" replaced with one from the current list). The criteria currently applied when selecting a pool of items from @rossion2004 appear to be more stringent than those applied by @ensor2019b; they excluded any items that were best described by two-word names (e.g. "baby carriage"), whilst our criteria consisted of: i) items with a written name between 4 and 7 letters; ii) items that would conjure the same intended concept in UK-based samples (e.g. “ladder” should be universally understood across English-speaking cultures, whereas “wagon” or “pants” can be interpreted differently); iii) items that were not unknown, or uncommon, for our sample (e.g. Americanisms such as “wrench”); and iv) non-specific concepts such as “bird” (since the pool of items already contained specific exemplars of birds, such as “peacock” and “penguin”). Other methodological differences include the fact that the current paradigm had participants make responses at encoding to ensure their attention was directed toward each item, in contrast to the passive presentation of items used by @ensor2019b. Subjects were also required to provide follow up recognition judgements in the current paradigm, rather than just an old/new response. It seems likely that the distinctiveness benefits demonstrated by @ensor2019b are sensitive to these methodological inconsistencies. 

A limitation of the current study (and indeed, of @ensor2019b) lies in its' inability to determine how a number of factors separately contribute toward an overall level of distinctiveness. The distinctive word stimuli utilised in these experiments contained manipulations to colour, font, size, and capitalisation, though little rationale was provided as to why these particular factors were selected. While not arbitrary - as one might consider these factors to be the most effective in creating item-to-item variability - there is little justification as to why they should be selected over others such as letter height, spacing, italics, underlining etc. Such comparisons are outside the scope of the current programme of research, where innumerable combinations of factors could be compared, but they raise the question as to the factors that contribute most to the notion of distinctiveness. It could simply be assumed that each of these factors provide similar contributions, and that they each play a role in the overall levels of distinctiveness of a stimulus. However, across three experiments, the current programme of research has consistently demonstrated that colour information has no discernible impact on recognition performance - and as such, does not contribute to making stimuli more distinctive. This seems to be an important distinction to make, especially as colour inherently seems to be regarded as important 
...
DCT does not operate in absolutes.
Both words and pictures have the ability to generate dual codes:
words (verbal by default, but image code can be generated by conjuring a mental image of the items referent)
pictures (imagery by default, but verbal name can be internally generated when identifying the item)

Explanations of the PSE boil down to *likelihood*. It's much more likely that pictures are also named (low effort process) than it is for words to be imagined (effortful). Thus, the likelihood of pictures generating dual codes is higher than that for words, aiding recognition / retrieval later on.

While results this far seem best explained by the physical distinctiveness account, DCT cannot be discounted entirely. Manipulations to stimuli distinctiveness might simply affect the likelihood that 2nd codes are generated. Dual codes highly likely for photographs, likely for drawings, less likely for words. Perhaps results are best explained by a unified theory of the PSE. 
concreteness effect - see @ensor2019b, blue highlight on 2nd page.

## Experiment: Manipulating word labels
```{r echo=FALSE, results = FALSE, message=FALSE, warning=FALSE, cache=TRUE, fig.show='hide'}
##################################################################################################################
...
##  Import demographics, RFG justifications, and RFBG justifications:
    exp6__p_info_files <- c(
        ...
        "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_questionnaire-z6yh.csv", # Demog (v10)
        ...
        "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_questionnaire-h32i.csv")  # RFG just (v10)

##  Create raw df from the above files, but only get the following columns:
    exp6__participant.info <- map_dfr(exp6__p_info_files,~read_csv(.x, col_types = cols_only(
        "Participant Private ID"='c', "Participant Public ID"='c', "UTC Date"='c', "Participant OS"='c',
        "Participant Browser"='c', "Participant Monitor Size"='c', "branch-ebg9"='c', "randomiser-6j1g"='c', 
        "randomiser-rj7w"='c', "randomiser-enay"='c', "randomiser-gw7n"='c', "randomiser-uc11"='c', 
...
##  Red: ---------------------------------------------------------------------------------------------------------
    ##  Version 6: 
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-l7li.csv", # Red (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-qi5x.csv", # Red (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-pifd.csv", # Red (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-uf3l.csv", # Red (pics+word - order 2)
    ##  Version 7: 
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-l7li.csv", # Red (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-qi5x.csv", # Red (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-pifd.csv", # Red (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-uf3l.csv", # Red (pics+word - order 2)          ##  Version 10: 
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-l7li.csv", # Red (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-qi5x.csv", # Red (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-pifd.csv", # Red (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-uf3l.csv", # Red (pics+word - order 2)     ##  Orange: ------------------------------------------------------------------------------------------------------
    ##  Version 6:         
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-a3o7.csv", # Orange (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-1u76.csv", # Orange (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-3n1f.csv", # Orange (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-5ium.csv", # Orange (pics+word - order 2)
    ##  Version 7:         
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-a3o7.csv", # Orange (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-1u76.csv", # Orange (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-3n1f.csv", # Orange (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-5ium.csv", # Orange (pics+word - order 2)       ##  Version 10:         
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-a3o7.csv", # Orange (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-1u76.csv", # Orange (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-3n1f.csv", # Orange (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-5ium.csv", # Orange (pics+word - order 2)  ##  Yellow: ------------------------------------------------------------------------------------------------------
    ##  Version 6:        
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-a9z8.csv", # Yellow (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-bqu6.csv", # Yellow (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-1v1k.csv", # Yellow (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-ggr4.csv", # Yellow (pics+word - order 2)
    ##  Version 7:        
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-a9z8.csv", # Yellow (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-bqu6.csv", # Yellow (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-1v1k.csv", # Yellow (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-ggr4.csv", # Yellow (pics+word - order 2)
    ##  Version 10:        
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-a9z8.csv", # Yellow (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-bqu6.csv", # Yellow (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-1v1k.csv", # Yellow (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-ggr4.csv", # Yellow (pics+word - order 2)
##  Green: ------------------------------------------------------------------------------------------------------
    ##  Version 6:
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-okc7.csv", # Green (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-wmoz.csv", # Green (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-jlur.csv", # Green (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-vbdo.csv", # Green (pics+word - order 2)
    ##  Version 7:
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-okc7.csv", # Green (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-wmoz.csv", # Green (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-jlur.csv", # Green (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-vbdo.csv", # Green (pics+word - order 2)
    ##  Version 10:
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-okc7.csv", # Green (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-wmoz.csv", # Green (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-jlur.csv", # Green (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-vbdo.csv", # Green (pics+word - order 2)
##  Teal: ------------------------------------------------------------------------------------------------------
    ##  Version 6:
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-pygj.csv", # Teal (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-qza1.csv", # Teal (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-eia6.csv", # Teal (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-7wwc.csv", # Teal (pics+word - order 2)
    ##  Version 7:
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-pygj.csv", # Teal (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-qza1.csv", # Teal (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-eia6.csv", # Teal (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-7wwc.csv", # Teal (pics+word - order 2)
    ##  Version 10:
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-pygj.csv", # Teal (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-qza1.csv", # Teal (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-eia6.csv", # Teal (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-7wwc.csv", # Teal (pics+word - order 2)
##  Blue: ------------------------------------------------------------------------------------------------------
    ##  Version 6:
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-j5hw.csv", # Blue (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-g65w.csv", # Blue (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-jayr.csv", # Blue (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-l2v4.csv", # Blue (pics+word - order 2)
    ##  Version 7:
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-j5hw.csv", # Blue (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-g65w.csv", # Blue (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-jayr.csv", # Blue (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-l2v4.csv", # Blue (pics+word - order 2)
    ##  Version 10:
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-j5hw.csv", # Blue (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-g65w.csv", # Blue (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-jayr.csv", # Blue (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-l2v4.csv", # Blue (pics+word - order 2)
##  Purple: ------------------------------------------------------------------------------------------------------
    ##  Version 6:
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-s7up.csv", # Purple (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-qvph.csv", # Purple (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-9x5b.csv", # Purple (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-1nqy.csv", # Purple (pics+word - order 2)
    ##  Version 7:
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-s7up.csv", # Purple (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-qvph.csv", # Purple (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-9x5b.csv", # Purple (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-1nqy.csv", # Purple (pics+word - order 2)
    ##  Version 10:
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-s7up.csv", # Purple (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-qvph.csv", # Purple (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-9x5b.csv", # Purple (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-1nqy.csv", # Purple (pics+word - order 2)
##  Pink: ------------------------------------------------------------------------------------------------------
    ##  Version 6:
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-jsz4.csv", # Pink (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-2j1b.csv", # Pink (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-od4o.csv", # Pink (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-qacf.csv", # Pink (pics+word - order 2)
    ##  Version 7:
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-jsz4.csv", # Pink (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-2j1b.csv", # Pink (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-od4o.csv", # Pink (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-qacf.csv", # Pink (pics+word - order 2)
    ##  Version 10:
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-jsz4.csv", # Pink (pics-only - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-2j1b.csv", # Pink (pics-only - order 2)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-od4o.csv", # Pink (pics+word - order 1)
      "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-qacf.csv") # Pink (pics+word - order 2)
...
##  Add 'exp6__study_totals' to 'exp6__participant.info' df:
    exp6__participant.info <- exp6__participant.info %>%
        ...
        mutate(test_list = ifelse(
          str_detect(test_list, "red") | str_detect(test_list, "teal"), "red.teal", 
          ifelse(str_detect(test_list, "orange") | str_detect(test_list, "blue"), "orange.blue", 
          ifelse(str_detect(test_list, "yellow") | str_detect(test_list, "purple"), "yellow.purple", 
          ifelse(str_detect(test_list, "green") | str_detect(test_list, "pink"), "green.pink", NA))))) %>%
        arrange(study_list, test_list) %>%
        select(id, prolific_id, everything())
...
##  Import test data:
    exp6__test_files <- c(
        ...
        "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-eo3t.csv",
        "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-eo3t.csv",

        "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-hnun.csv",   ##  Orange + Blue:
        "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-hnun.csv",
        "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-hnun.csv",
        
        "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-tmr7.csv",   ##  Yellow + Purple:
        "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-tmr7.csv",
        "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-tmr7.csv",

        "./resources/experiment_6/experiment_6_data/data_exp_47727-v6_task-cdc5.csv",   ##  Green + Pink:
        "./resources/experiment_6/experiment_6_data/data_exp_47727-v7_task-cdc5.csv",
        "./resources/experiment_6/experiment_6_data/data_exp_47727-v10_task-cdc5.csv")
...
#################################################################################################################
    ##  Drawings only:
        mutate(prop.FA__drawing.only = FA__drawing.only / (FA__drawing.only + CR__drawing.only)) %>%
            mutate(prop.FA__drawing.only = replace_na(prop.FA__drawing.only, 0)) %>%
        mutate(prop.CR__drawing.only = CR__drawing.only / (FA__drawing.only + CR__drawing.only)) %>%
            mutate(prop.CR__drawing.only = replace_na(prop.CR__drawing.only, 0)) %>%
        mutate(prop.FA.R__drawing.only = FA.R__drawing.only / FA__drawing.only) %>% 
            mutate(prop.FA.R__drawing.only = replace_na(prop.FA.R__drawing.only, 0)) %>%
        mutate(prop.FA.F__drawing.only = FA.F__drawing.only / FA__drawing.only) %>% 
            mutate(prop.FA.F__drawing.only = replace_na(prop.FA.F__drawing.only, 0)) %>%
        mutate(prop.FA.G__drawing.only = FA.G__drawing.only / FA__drawing.only) %>% 
            mutate(prop.FA.G__drawing.only = replace_na(prop.FA.G__drawing.only, 0)) %>%
    ##  Photos only:
        mutate(prop.FA__photo.only = FA__photo.only / (FA__photo.only + CR__photo.only)) %>%
            mutate(prop.FA__photo.only = replace_na(prop.FA__photo.only, 0)) %>%
        mutate(prop.CR__photo.only = CR__photo.only / (FA__photo.only + CR__photo.only)) %>%
            mutate(prop.CR__photo.only = replace_na(prop.CR__photo.only, 0)) %>%
        mutate(prop.FA.R__photo.only = FA.R__photo.only / FA__photo.only) %>% 
            mutate(prop.FA.R__photo.only = replace_na(prop.FA.R__photo.only, 0)) %>%
        mutate(prop.FA.F__photo.only = FA.F__photo.only / FA__photo.only) %>% 
            mutate(prop.FA.F__photo.only = replace_na(prop.FA.F__photo.only, 0)) %>%
        mutate(prop.FA.G__photo.only = FA.G__photo.only / FA__photo.only) %>% 
            mutate(prop.FA.G__photo.only = replace_na(prop.FA.G__photo.only, 0)) %>%     
    ##  Drawings plus words:
        mutate(prop.FA__drawing.plus.word = FA__drawing.plus.word / 
              (FA__drawing.plus.word + CR__drawing.plus.word)) %>%
            mutate(prop.FA__drawing.plus.word = replace_na(prop.FA__drawing.plus.word, 0)) %>%
        mutate(prop.CR__drawing.plus.word = CR__drawing.plus.word / 
              (FA__drawing.plus.word + CR__drawing.plus.word)) %>%
            mutate(prop.CR__drawing.plus.word = replace_na(prop.CR__drawing.plus.word, 0)) %>%
        mutate(prop.FA.R__drawing.plus.word = FA.R__drawing.plus.word / FA__drawing.plus.word) %>% 
            mutate(prop.FA.R__drawing.plus.word = replace_na(prop.FA.R__drawing.plus.word, 0)) %>%
        mutate(prop.FA.F__drawing.plus.word = FA.F__drawing.plus.word / FA__drawing.plus.word) %>% 
            mutate(prop.FA.F__drawing.plus.word = replace_na(prop.FA.F__drawing.plus.word, 0)) %>%
        mutate(prop.FA.G__drawing.plus.word = FA.G__drawing.plus.word / FA__drawing.plus.word) %>% 
            mutate(prop.FA.G__drawing.plus.word = replace_na(prop.FA.G__drawing.plus.word, 0)) %>%
    ##  Photos plus words:
        mutate(prop.FA__photo.plus.word = FA__photo.plus.word / 
              (FA__photo.plus.word + CR__photo.plus.word)) %>%
            mutate(prop.FA__photo.plus.word = replace_na(prop.FA__photo.plus.word, 0)) %>%
        mutate(prop.CR__photo.plus.word = CR__photo.plus.word / 
              (FA__photo.plus.word + CR__photo.plus.word)) %>%
            mutate(prop.CR__photo.plus.word = replace_na(prop.CR__photo.plus.word, 0)) %>%
        mutate(prop.FA.R__photo.plus.word = FA.R__photo.plus.word / FA__photo.plus.word) %>% 
            mutate(prop.FA.R__photo.plus.word = replace_na(prop.FA.R__photo.plus.word, 0)) %>%
        mutate(prop.FA.F__photo.plus.word = FA.F__photo.plus.word / FA__photo.plus.word) %>% 
            mutate(prop.FA.F__photo.plus.word = replace_na(prop.FA.F__photo.plus.word, 0)) %>%
        mutate(prop.FA.G__photo.plus.word = FA.G__photo.plus.word / FA__photo.plus.word) %>% 
            mutate(prop.FA.G__photo.plus.word = replace_na(prop.FA.G__photo.plus.word, 0))
#################################################################################################################
##    Calculate d-prime:                                                                                       ##
#################################################################################################################
##  Drawings only:      
##  Overall:
    exp6__drawing.only__d.prime <- exp6__test.stats %>%
        select(id, prop.hit__drawing.only, prop.FA__drawing.only) %>%
        mutate(prop.hit__drawing.only = replace(prop.hit__drawing.only, prop.hit__drawing.only == 1, .99)) %>%
        mutate(prop.hit__drawing.only = replace(prop.hit__drawing.only, prop.hit__drawing.only == 0, .01)) %>%
        mutate(prop.FA__drawing.only = replace(prop.FA__drawing.only, prop.FA__drawing.only == 1, .99)) %>%
        mutate(prop.FA__drawing.only = replace(prop.FA__drawing.only, prop.FA__drawing.only == 0, .01)) %>%
        mutate(d.prime__drawing.only = qnorm(prop.hit__drawing.only) - qnorm(prop.FA__drawing.only)) %>%
        select(id, d.prime__drawing.only)
##  Rec:
    exp6__drawing.only__d.prime.rec <- exp6__test.stats %>%
        select(id, prop.hit.R__drawing.only, prop.FA.R__drawing.only) %>%
        mutate(prop.hit.R__drawing.only = 
                 replace(prop.hit.R__drawing.only, prop.hit.R__drawing.only == 1, .99)) %>%
        mutate(prop.hit.R__drawing.only = 
                 replace(prop.hit.R__drawing.only, prop.hit.R__drawing.only == 0, .01)) %>%
        mutate(prop.FA.R__drawing.only = 
                 replace(prop.FA.R__drawing.only, prop.FA.R__drawing.only == 1, .99)) %>%
        mutate(prop.FA.R__drawing.only = 
                 replace(prop.FA.R__drawing.only, prop.FA.R__drawing.only == 0, .01)) %>%
        mutate(d.prime__drawing.only.rec = qnorm(prop.hit.R__drawing.only) - qnorm(prop.FA.R__drawing.only)) %>%
        select(id, d.prime__drawing.only.rec)
##  Fam:
    exp6__drawing.only__d.prime.fam <- exp6__test.stats %>%
        select(id, prop.hit.F__drawing.only, prop.FA.F__drawing.only) %>%
        mutate(prop.hit.F__drawing.only = 
                 replace(prop.hit.F__drawing.only, prop.hit.F__drawing.only == 1, .99)) %>%
        mutate(prop.hit.F__drawing.only = 
                 replace(prop.hit.F__drawing.only, prop.hit.F__drawing.only == 0, .01)) %>%
        mutate(prop.FA.F__drawing.only = 
                 replace(prop.FA.F__drawing.only, prop.FA.F__drawing.only == 1, .99)) %>%
        mutate(prop.FA.F__drawing.only = 
                 replace(prop.FA.F__drawing.only, prop.FA.F__drawing.only == 0, .01)) %>%
        mutate(d.prime__drawing.only.fam = qnorm(prop.hit.F__drawing.only) - qnorm(prop.FA.F__drawing.only)) %>%
        select(id, d.prime__drawing.only.fam)
##  Guess:
    exp6__drawing.only__d.prime.guess <- exp6__test.stats %>%
        select(id, prop.hit.G__drawing.only, prop.FA.G__drawing.only) %>%
        mutate(prop.hit.G__drawing.only = 
                 replace(prop.hit.G__drawing.only, prop.hit.G__drawing.only == 1, .99)) %>%
        mutate(prop.hit.G__drawing.only = 
                 replace(prop.hit.G__drawing.only, prop.hit.G__drawing.only == 0, .01)) %>%
        mutate(prop.FA.G__drawing.only = 
                 replace(prop.FA.G__drawing.only, prop.FA.G__drawing.only == 1, .99)) %>%
        mutate(prop.FA.G__drawing.only = 
                 replace(prop.FA.G__drawing.only, prop.FA.G__drawing.only == 0, .01)) %>%
        mutate(d.prime__drawing.only.guess = qnorm(prop.hit.G__drawing.only) - qnorm(prop.FA.G__drawing.only)) %>%
        select(id, d.prime__drawing.only.guess)
#---------------------------------------------------------------------------------------------------------------- ##  Photos only:      
##  Overall:
    exp6__photo.only__d.prime <- exp6__test.stats %>%
        select(id, prop.hit__photo.only, prop.FA__photo.only) %>%
        mutate(prop.hit__photo.only = replace(prop.hit__photo.only, prop.hit__photo.only == 1, .99)) %>%
        mutate(prop.hit__photo.only = replace(prop.hit__photo.only, prop.hit__photo.only == 0, .01)) %>%
        mutate(prop.FA__photo.only = replace(prop.FA__photo.only, prop.FA__photo.only == 1, .99)) %>%
        mutate(prop.FA__photo.only = replace(prop.FA__photo.only, prop.FA__photo.only == 0, .01)) %>%
        mutate(d.prime__photo.only = qnorm(prop.hit__photo.only) - qnorm(prop.FA__photo.only)) %>%
        select(id, d.prime__photo.only)
##  Rec:
    exp6__photo.only__d.prime.rec <- exp6__test.stats %>%
        select(id, prop.hit.R__photo.only, prop.FA.R__photo.only) %>%
        mutate(prop.hit.R__photo.only = 
                 replace(prop.hit.R__photo.only, prop.hit.R__photo.only == 1, .99)) %>%
        mutate(prop.hit.R__photo.only = 
                 replace(prop.hit.R__photo.only, prop.hit.R__photo.only == 0, .01)) %>%
        mutate(prop.FA.R__photo.only = 
                 replace(prop.FA.R__photo.only, prop.FA.R__photo.only == 1, .99)) %>%
        mutate(prop.FA.R__photo.only = 
                 replace(prop.FA.R__photo.only, prop.FA.R__photo.only == 0, .01)) %>%
        mutate(d.prime__photo.only.rec = qnorm(prop.hit.R__photo.only) - qnorm(prop.FA.R__photo.only)) %>%
        select(id, d.prime__photo.only.rec)
##  Fam:
    exp6__photo.only__d.prime.fam <- exp6__test.stats %>%
        select(id, prop.hit.F__photo.only, prop.FA.F__photo.only) %>%
        mutate(prop.hit.F__photo.only = 
                 replace(prop.hit.F__photo.only, prop.hit.F__photo.only == 1, .99)) %>%
        mutate(prop.hit.F__photo.only = 
                 replace(prop.hit.F__photo.only, prop.hit.F__photo.only == 0, .01)) %>%
        mutate(prop.FA.F__photo.only = 
                 replace(prop.FA.F__photo.only, prop.FA.F__photo.only == 1, .99)) %>%
        mutate(prop.FA.F__photo.only = 
                 replace(prop.FA.F__photo.only, prop.FA.F__photo.only == 0, .01)) %>%
        mutate(d.prime__photo.only.fam = qnorm(prop.hit.F__photo.only) - qnorm(prop.FA.F__photo.only)) %>%
        select(id, d.prime__photo.only.fam)
##  Guess:
    exp6__photo.only__d.prime.guess <- exp6__test.stats %>%
        select(id, prop.hit.G__photo.only, prop.FA.G__photo.only) %>%
        mutate(prop.hit.G__photo.only = 
                 replace(prop.hit.G__photo.only, prop.hit.G__photo.only == 1, .99)) %>%
        mutate(prop.hit.G__photo.only = 
                 replace(prop.hit.G__photo.only, prop.hit.G__photo.only == 0, .01)) %>%
        mutate(prop.FA.G__photo.only = 
                 replace(prop.FA.G__photo.only, prop.FA.G__photo.only == 1, .99)) %>%
        mutate(prop.FA.G__photo.only = 
                 replace(prop.FA.G__photo.only, prop.FA.G__photo.only == 0, .01)) %>%
        mutate(d.prime__photo.only.guess = qnorm(prop.hit.G__photo.only) - qnorm(prop.FA.G__photo.only)) %>%
        select(id, d.prime__photo.only.guess)
#----------------------------------------------------------------------------------------------------------------  ##  Drawing plus word:      
##  Overall:
    exp6__drawing.plus.word__d.prime <- exp6__test.stats %>%
        select(id, prop.hit__drawing.plus.word, prop.FA__drawing.plus.word) %>%
        mutate(prop.hit__drawing.plus.word = 
                 replace(prop.hit__drawing.plus.word, prop.hit__drawing.plus.word == 1, .99)) %>%
        mutate(prop.hit__drawing.plus.word = 
                 replace(prop.hit__drawing.plus.word, prop.hit__drawing.plus.word == 0, .01)) %>%
        mutate(prop.FA__drawing.plus.word = 
                 replace(prop.FA__drawing.plus.word, prop.FA__drawing.plus.word == 1, .99)) %>%
        mutate(prop.FA__drawing.plus.word = 
                 replace(prop.FA__drawing.plus.word, prop.FA__drawing.plus.word == 0, .01)) %>%
        mutate(d.prime__drawing.plus.word = 
                 qnorm(prop.hit__drawing.plus.word) - qnorm(prop.FA__drawing.plus.word)) %>%
        select(id, d.prime__drawing.plus.word)
##  Rec:
    exp6__drawing.plus.word__d.prime.rec <- exp6__test.stats %>%
        select(id, prop.hit.R__drawing.plus.word, prop.FA.R__drawing.plus.word) %>%
        mutate(prop.hit.R__drawing.plus.word = 
                 replace(prop.hit.R__drawing.plus.word, prop.hit.R__drawing.plus.word == 1, .99)) %>%
        mutate(prop.hit.R__drawing.plus.word = 
                 replace(prop.hit.R__drawing.plus.word, prop.hit.R__drawing.plus.word == 0, .01)) %>%
        mutate(prop.FA.R__drawing.plus.word = 
                 replace(prop.FA.R__drawing.plus.word, prop.FA.R__drawing.plus.word == 1, .99)) %>%
        mutate(prop.FA.R__drawing.plus.word = 
                 replace(prop.FA.R__drawing.plus.word, prop.FA.R__drawing.plus.word == 0, .01)) %>%
        mutate(d.prime__drawing.plus.word.rec = 
                 qnorm(prop.hit.R__drawing.plus.word) - qnorm(prop.FA.R__drawing.plus.word)) %>%
        select(id, d.prime__drawing.plus.word.rec)
##  Fam:
    exp6__drawing.plus.word__d.prime.fam <- exp6__test.stats %>%
        select(id, prop.hit.F__drawing.plus.word, prop.FA.F__drawing.plus.word) %>%
        mutate(prop.hit.F__drawing.plus.word = 
                 replace(prop.hit.F__drawing.plus.word, prop.hit.F__drawing.plus.word == 1, .99)) %>%
        mutate(prop.hit.F__drawing.plus.word = 
                 replace(prop.hit.F__drawing.plus.word, prop.hit.F__drawing.plus.word == 0, .01)) %>%
        mutate(prop.FA.F__drawing.plus.word = 
                 replace(prop.FA.F__drawing.plus.word, prop.FA.F__drawing.plus.word == 1, .99)) %>%
        mutate(prop.FA.F__drawing.plus.word = 
                 replace(prop.FA.F__drawing.plus.word, prop.FA.F__drawing.plus.word == 0, .01)) %>%
        mutate(d.prime__drawing.plus.word.fam = 
                 qnorm(prop.hit.F__drawing.plus.word) - qnorm(prop.FA.F__drawing.plus.word)) %>%
        select(id, d.prime__drawing.plus.word.fam)
##  Guess:
    exp6__drawing.plus.word__d.prime.guess <- exp6__test.stats %>%
        select(id, prop.hit.G__drawing.plus.word, prop.FA.G__drawing.plus.word) %>%
        mutate(prop.hit.G__drawing.plus.word = 
                 replace(prop.hit.G__drawing.plus.word, prop.hit.G__drawing.plus.word == 1, .99)) %>%
        mutate(prop.hit.G__drawing.plus.word = 
                 replace(prop.hit.G__drawing.plus.word, prop.hit.G__drawing.plus.word == 0, .01)) %>%
        mutate(prop.FA.G__drawing.plus.word = 
                 replace(prop.FA.G__drawing.plus.word, prop.FA.G__drawing.plus.word == 1, .99)) %>%
        mutate(prop.FA.G__drawing.plus.word = 
                 replace(prop.FA.G__drawing.plus.word, prop.FA.G__drawing.plus.word == 0, .01)) %>%
        mutate(d.prime__drawing.plus.word.guess = 
                 qnorm(prop.hit.G__drawing.plus.word) - qnorm(prop.FA.G__drawing.plus.word)) %>%
        select(id, d.prime__drawing.plus.word.guess)
#----------------------------------------------------------------------------------------------------------------  ##  Photo plus word:      
##  Overall:
    exp6__photo.plus.word__d.prime <- exp6__test.stats %>%
        select(id, prop.hit__photo.plus.word, prop.FA__photo.plus.word) %>%
        mutate(prop.hit__photo.plus.word = 
                 replace(prop.hit__photo.plus.word, prop.hit__photo.plus.word == 1, .99)) %>%
        mutate(prop.hit__photo.plus.word = 
                 replace(prop.hit__photo.plus.word, prop.hit__photo.plus.word == 0, .01)) %>%
        mutate(prop.FA__photo.plus.word = 
                 replace(prop.FA__photo.plus.word, prop.FA__photo.plus.word == 1, .99)) %>%
        mutate(prop.FA__photo.plus.word = 
                 replace(prop.FA__photo.plus.word, prop.FA__photo.plus.word == 0, .01)) %>%
        mutate(d.prime__photo.plus.word = 
                 qnorm(prop.hit__photo.plus.word) - qnorm(prop.FA__photo.plus.word)) %>%
        select(id, d.prime__photo.plus.word)
##  Rec:
    exp6__photo.plus.word__d.prime.rec <- exp6__test.stats %>%
        select(id, prop.hit.R__photo.plus.word, prop.FA.R__photo.plus.word) %>%
        mutate(prop.hit.R__photo.plus.word = 
                 replace(prop.hit.R__photo.plus.word, prop.hit.R__photo.plus.word == 1, .99)) %>%
        mutate(prop.hit.R__photo.plus.word = 
                 replace(prop.hit.R__photo.plus.word, prop.hit.R__photo.plus.word == 0, .01)) %>%
        mutate(prop.FA.R__photo.plus.word = 
                 replace(prop.FA.R__photo.plus.word, prop.FA.R__photo.plus.word == 1, .99)) %>%
        mutate(prop.FA.R__photo.plus.word = 
                 replace(prop.FA.R__photo.plus.word, prop.FA.R__photo.plus.word == 0, .01)) %>%
        mutate(d.prime__photo.plus.word.rec = 
                 qnorm(prop.hit.R__photo.plus.word) - qnorm(prop.FA.R__photo.plus.word)) %>%
        select(id, d.prime__photo.plus.word.rec)
##  Fam:
    exp6__photo.plus.word__d.prime.fam <- exp6__test.stats %>%
        select(id, prop.hit.F__photo.plus.word, prop.FA.F__photo.plus.word) %>%
        mutate(prop.hit.F__photo.plus.word = 
                 replace(prop.hit.F__photo.plus.word, prop.hit.F__photo.plus.word == 1, .99)) %>%
        mutate(prop.hit.F__photo.plus.word = 
                 replace(prop.hit.F__photo.plus.word, prop.hit.F__photo.plus.word == 0, .01)) %>%
        mutate(prop.FA.F__photo.plus.word = 
                 replace(prop.FA.F__photo.plus.word, prop.FA.F__photo.plus.word == 1, .99)) %>%
        mutate(prop.FA.F__photo.plus.word = 
                 replace(prop.FA.F__photo.plus.word, prop.FA.F__photo.plus.word == 0, .01)) %>%
        mutate(d.prime__photo.plus.word.fam = 
                 qnorm(prop.hit.F__photo.plus.word) - qnorm(prop.FA.F__photo.plus.word)) %>%
        select(id, d.prime__photo.plus.word.fam)
##  Guess:
    exp6__photo.plus.word__d.prime.guess <- exp6__test.stats %>%
        select(id, prop.hit.G__photo.plus.word, prop.FA.G__photo.plus.word) %>%
        mutate(prop.hit.G__photo.plus.word = 
                 replace(prop.hit.G__photo.plus.word, prop.hit.G__photo.plus.word == 1, .99)) %>%
        mutate(prop.hit.G__photo.plus.word = 
                 replace(prop.hit.G__photo.plus.word, prop.hit.G__photo.plus.word == 0, .01)) %>%
        mutate(prop.FA.G__photo.plus.word = 
                 replace(prop.FA.G__photo.plus.word, prop.FA.G__photo.plus.word == 1, .99)) %>%
        mutate(prop.FA.G__photo.plus.word = 
                 replace(prop.FA.G__photo.plus.word, prop.FA.G__photo.plus.word == 0, .01)) %>%
        mutate(d.prime__photo.plus.word.guess = 
                 qnorm(prop.hit.G__photo.plus.word) - qnorm(prop.FA.G__photo.plus.word)) %>%
        select(id, d.prime__photo.plus.word.guess)
#---------------------------------------------------------------------------------------------------------------- 
##  Add new d'prime calculations to 'exp6__test.stats' df: 
    test <- exp6__test.stats %>%
        right_join(exp6__drawing.only__d.prime) %>% right_join(exp6__photo.only__d.prime) %>%
        right_join(exp6__drawing.plus.word__d.prime)%>% right_join(exp6__photo.plus.word__d.prime) %>%
        right_join(exp6__drawing.only__d.prime.rec) %>% right_join(exp6__photo.only__d.prime.rec) %>%
        right_join(exp6__drawing.plus.word__d.prime.rec) %>% right_join(exp6__photo.plus.word__d.prime.rec) %>% 
        right_join(exp6__drawing.only__d.prime.fam) %>% right_join(exp6__photo.only__d.prime.fam) %>%
        right_join(exp6__drawing.plus.word__d.prime.fam) %>% right_join(exp6__photo.plus.word__d.prime.fam) %>%           right_join(exp6__drawing.only__d.prime.guess) %>% right_join(exp6__photo.only__d.prime.guess) %>%
        right_join(exp6__drawing.plus.word__d.prime.guess) %>% right_join(exp6__photo.plus.word__d.prime.guess)  
##  Merge 'exp6__participant.info' and 'exp6__test.stats' to create final tidy df: 
    exp6__final.data <- exp6__participant.info %>%
        right_join(exp6__test.stats, by = c("id", "prolific_id", "study_list", "test_list")) %>%
        arrange(prolific_id, study_list, test_list)
...
##################################################################################################################
##  Anyone with less than 90% toward encoding questions:
    exp6__exclude1 <- exp6__final.data %>%
        filter(study_correct_percent < 90)
##  Anyone + or - 3.0 z-score for total hits:
    exp6__exclude2 <- exp6__final.data %>%
        mutate("z.hit" = qnorm((hit__total + 0.5) / (hit__total + miss__total + 1))) %>%
        filter(z.hit >= 3.0 | z.hit <= -3.0)
##  Anyone + or - 3.0 z-score for total FAs:
    exp6__exclude3 <- exp6__final.data %>%
        mutate("z.FA" = qnorm((FA__total + 0.5) / (FA__total + CR__total + 1))) %>%
        filter(z.FA >= 3.0 | z.FA <= -3.0)
##  Anyone + or - 3.0 z-score for total recognition:
    exp6__exclude4 <- exp6__final.data %>%
        mutate("prop.hit__total" = hit__total / (hit__total + miss__total)) %>%
        mutate("prop.FA__total" = FA__total / (FA__total + CR__total)) %>%
        mutate("recog__total" = prop.hit__total - prop.FA__total) %>%
        mutate("z.recog" = (recog__total - mean(recog__total)) / sd(recog__total)) %>%
        filter(z.recog >= 3.0 | z.recog <= -3.0)
##  Join all to-be-excluded participants into single df:
    exp6__all_exclude <- exp6__exclude1 %>%
        bind_rows(exp6__exclude2, exp6__exclude3, exp6__exclude4)
##  Just get the id's for the to-be-excluded participants:
    exp6__p_to_exclude <- exp6__all_exclude$id
##  Remove these participants from 'exp6__final.data':
    exp6__final.data <- exp6__final.data %>%
        filter(!id %in% exp6__p_to_exclude) %>%
##  Programming error (these P didn't see non-word items, so we remove them):    
        filter(prolific_id != 14637 & prolific_id != 14020) %>%
        arrange(test_list, study_list)
    
  #write.csv(exp6__final.data,'~/Desktop/exp6__final.data.csv', row.names = FALSE)

complete_datasets <- exp6__final.data %>%
    select(id, prolific_id, study_list) %>%
    count(study_list)
write.csv(complete_datasets,'~/Desktop/complete_datasets.csv', row.names = FALSE)
write.csv(exp6__participant.info,'~/Desktop/exp6__participant.info.csv', row.names = FALSE)
```



2 participants were removed due to a programming error, whereby pictures without labels were not presented. 
...
```{r echo=FALSE, warning = FALSE, message=FALSE, cache=FALSE}
    exp2__spelling_corrections <- read_csv("./resources/experiment_2/spelling_corrections.csv") %>%
        replace(is.na(.), "") %>%
        rename("Item:" = "Item1", "Item: " = "Item2", "Item:  " = "Item3",
               "Response:" = "Response1", "Response " = "Response2", "Response  " = "Response3",
               "Corrected to:" = "Correction1", "Corrected to: " = "Correction2", 
               "Corrected to:  " = "Correction3")

    exp2__spelling_corrections %>%
        kable("latex", longtable = T, booktabs = T, linesep = "") %>%
        kable_styling(bootstrap_options = c("striped","condensed")) %>%
        kable_styling(latex_options = c("repeat_header"), font_size = 10) %>%
        row_spec(0, bold = T) %>%
        column_spec(1, width = "1.8cm", bold = T) %>%
        column_spec(2, width = "1.8cm") %>%
        column_spec(3, width = "2.2cm", border_right = T) %>%
        column_spec(4, width = "1.8cm", bold = T) %>%
        column_spec(5, width = "1.8cm") %>%
        column_spec(6, width = "2.2cm", border_right = T) %>%
        column_spec(7, width = "1.8cm", bold = T) %>%
...
```{r echo=FALSE, cache=TRUE, warnings=FALSE}
    exp2__norms <- exp2__raw_data %>%
        ...
        mutate(familiarity_mean = round(familiarity_mean, 2)) %>%
        mutate(familiarity_sd = round(familiarity_sd, 2)) %>%
        mutate(visualcomplexity_mean = round(visualcomplexity_mean, 2)) %>%
        mutate(visualcomplexity_sd = round(visualcomplexity_sd, 2)) %>%
        mutate(colourdiagnosticity_mean = round(colourdiagnosticity_mean, 2)) %>%
        mutate(colourdiagnosticity_sd = round(colourdiagnosticity_sd, 2)) %>%
        mutate(mentalimageryagreement_mean = round(mentalimageryagreement_mean, 2)) %>%
        mutate(mentalimageryagreement_sd = round(mentalimageryagreement_sd, 2)) %>%
        ...
        mutate(Fam_Mean = round(Fam_Mean, 2)) %>%
        mutate_all(as.character) %>%
    ...
    exp2__norms %>%
        kable("latex", longtable = T, booktabs = T, align = "lcccc") %>%
        kable_styling(bootstrap_options = c("striped","condensed")) %>%
        kable_styling(latex_options = c("repeat_header"), font_size = 10) %>%
