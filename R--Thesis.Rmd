...
#     Ensor data:
#----------------------------------------------------------------------------------------------------------------
      exp5__ensor.data <- read_csv("./resources/experiment_5/ensor-exp1-raw-data.csv") %>%
...
##      d-prime (calculated by R package):
##      List unique participant IDs to loop through:
        ensor__participant_IDs <- unique(exp5__ensor.data$id)
##    d-prime (grey word):
      ensor__dprime__grey.word <- tibble()
      for (i in ensor__participant_IDs) {
          exp5__temp <- exp5__ensor.data %>%
              filter(id == i) %>%     
              select(id, hit__grey.word, FA__grey.word, miss__grey.word, CR__grey.word)
          ...
          exp5__temp_d_prime <- (as_tibble(dprime(n_hit = exp5__temp$hit__grey.word,
              ...
              mutate("id" = i)
          exp5__temp <- exp5__temp %>% inner_join(exp5__temp_d_prime, by = "id")
          ensor__dprime__grey.word <- bind_rows(ensor__dprime__grey.word, exp5__temp) }

        ensor__dprime__grey.word <- ensor__dprime__grey.word %>%
            select(id, dprime, beta, aprime, bppd, c) %>%
            rename(ensor__dprime__grey.word = "dprime", beta__grey.word = "beta",
                   aprime__grey.word = "aprime", bppd__grey.word = "bppd",
                   c__grey.word = "c")
##    d-prime (col word):
      ensor__dprime__col.word <- tibble()
      for (i in ensor__participant_IDs) {
          exp5__temp <- exp5__ensor.data %>%
              filter(id == i) %>%     
          ...
          exp5__temp_d_prime <- (as_tibble(dprime(n_hit = exp5__temp$hit__col.word,
              ...
              mutate("id" = i)
          exp5__temp <- exp5__temp %>% inner_join(exp5__temp_d_prime, by = "id")
          ensor__dprime__col.word <- bind_rows(ensor__dprime__col.word, exp5__temp) }

        ensor__dprime__col.word <- ensor__dprime__col.word %>%
            select(id, dprime, beta, aprime, bppd, c) %>%
            rename(ensor__dprime__col.word = "dprime", beta__col.word = "beta",
                   aprime__col.word = "aprime", bppd__col.word = "bppd",
                   c__col.word = "c")
##    d-prime (grey drawing):
      ensor__dprime__grey.drawing <- tibble()
      for (i in ensor__participant_IDs) {
          exp5__temp <- exp5__ensor.data %>%
              filter(id == i) %>%     
              select(id, hit__grey.drawing, FA__grey.drawing, miss__grey.drawing, CR__grey.drawing)
          ...
          exp5__temp_d_prime <- (as_tibble(dprime(n_hit = exp5__temp$hit__grey.drawing,
              ...
              mutate("id" = i)
          exp5__temp <- exp5__temp %>% inner_join(exp5__temp_d_prime, by = "id")
          ensor__dprime__grey.drawing <- bind_rows(ensor__dprime__grey.drawing, exp5__temp) }

        ensor__dprime__grey.drawing <- ensor__dprime__grey.drawing %>%
            select(id, dprime, beta, aprime, bppd, c) %>%
            rename(ensor__dprime__grey.drawing = "dprime", beta__grey.drawing = "beta",
                   aprime__grey.drawing = "aprime", bppd__grey.drawing = "bppd",
                   c__grey.drawing = "c")
##    d-prime (col drawing):
      ensor__dprime__col.drawing <- tibble()
      for (i in ensor__participant_IDs) {
          exp5__temp <- exp5__ensor.data %>%
              filter(id == i) %>%     
              select(id, hit__col.drawing, FA__col.drawing, miss__col.drawing, CR__col.drawing)
          ...
          exp5__temp_d_prime <- (as_tibble(dprime(n_hit = exp5__temp$hit__col.drawing,
              ...
              mutate("id" = i)
          exp5__temp <- exp5__temp %>% inner_join(exp5__temp_d_prime, by = "id")
          ensor__dprime__col.drawing <- bind_rows(ensor__dprime__col.drawing, exp5__temp) }

      ensor__dprime__col.drawing <- ensor__dprime__col.drawing %>%
          select(id, dprime, beta, aprime, bppd, c) %>%
          rename(ensor__dprime__col.drawing = "dprime", beta__col.drawing = "beta", aprime__col.drawing = "aprime",
                 bppd__col.drawing = "bppd", c__col.drawing = "c")

##    Add d'prime calculations to 'exp5__ensor.data' df: 
      exp5__ensor.data <- exp5__ensor.data %>%
          right_join(ensor__dprime__grey.word) %>%
          right_join(ensor__dprime__col.word) %>%
          right_join(ensor__dprime__grey.drawing) %>%
          right_join(ensor__dprime__col.drawing)
        
  write.csv(exp5__ensor.data,'~/Desktop/exp5__ensor.data.csv', row.names = FALSE)   
  
      exp5__ensor.d.prime <- exp5__ensor.data %>%
          select(id, ensor__dprime__grey.word, ensor__dprime__col.word, ensor__dprime__grey.drawing,
                 ensor__dprime__col.drawing) %>%
          gather(key = "stim", value = "d.prime", ensor__dprime__grey.word, ensor__dprime__col.word,
                 ensor__dprime__grey.drawing, ensor__dprime__col.drawing) %>%
          mutate(stim = str_remove(stim, "dprime__"))
      exp5__ensor.c <- exp5__ensor.data %>%
          select(id, c__grey.word, c__col.word, c__grey.drawing, c__col.drawing) %>%
          gather(key = "stim", value = "c", c__grey.word, c__col.word, c__grey.drawing, c__col.drawing) %>%
          mutate(stim = str_remove(stim, "c__"))
      exp5__ensor.prop.hit <- exp5__ensor.data %>%
          select(id, prop.hit__grey.word, prop.hit__col.word, prop.hit__grey.drawing, prop.hit__col.drawing) %>%
          gather(key = "stim", value = "prop.hit", prop.hit__grey.word, prop.hit__col.word, prop.hit__grey.drawing,
                 prop.hit__col.drawing) %>%
          mutate(stim = str_remove(stim, "prop.hit__"))
      exp5__ensor.prop.FA <- exp5__ensor.data %>%
          select(id, prop.FA__grey.word, prop.FA__col.word, prop.FA__grey.drawing, prop.FA__col.drawing) %>%
          gather(key = "stim", value = "prop.FA", prop.FA__grey.word, prop.FA__col.word, prop.FA__grey.drawing,
                 prop.FA__col.drawing) %>%
          mutate(stim = str_remove(stim, "prop.FA__"))
      exp5__ensor.recog <- exp5__ensor.data %>%
          select(id, recog__grey.word, recog__col.word, recog__grey.drawing, recog__col.drawing) %>%
          gather(key = "stim", value = "recog", recog__grey.word, recog__col.word, recog__grey.drawing,
                 recog__col.drawing) %>%
          mutate(stim = str_remove(stim, "recog__"))
...
##    Prep data for Ensor vs. current study table:  
      exp5__table__ensor.vs.current <- 
          tribble(~"var", ~"Dist", ~"Stim", ~"Current experiment", ~"Ensor, et al. (2019)",
                  "Hits", "Low distinctiveness", "Words", 
                        round(exp5__prop.hit %>% filter(stim == "grey.word") %>% pull(prop.hit) %>% mean(),2),
                        round(mean(exp5__ensor.data$prop.hit__grey.word),2),
                  "Hits", "Low distinctiveness", "Drawings",
                        round(exp5__prop.hit %>% filter(stim == "grey.drawing") %>% pull(prop.hit) %>% mean(),2),
                        round(mean(exp5__ensor.data$prop.hit__grey.drawing),2),
                  "Hits", "High distinctiveness", "Words",
                        round(exp5__prop.hit %>% filter(stim == "col.word") %>% pull(prop.hit) %>% mean(),2),
                        round(mean(exp5__ensor.data$prop.hit__col.word),2),
                  "Hits", "High distinctiveness", "Drawings",
                        round(exp5__prop.hit %>% filter(stim == "col.drawing") %>% pull(prop.hit) %>% mean(),2),
                        round(mean(exp5__ensor.data$prop.hit__col.drawing),2),
                  "","","", NA, NA,
                  "FAs", "Low distinctiveness", "Words", 
                        round(exp5__prop.FA %>% filter(stim == "grey.word") %>% pull(prop.FA) %>% mean(),2),
                        round(mean(exp5__ensor.data$prop.FA__grey.word),2),
                  "FAs", "Low distinctiveness", "Drawings",
                        round(exp5__prop.FA %>% filter(stim == "grey.drawing") %>% pull(prop.FA) %>% mean(),2),
                        round(mean(exp5__ensor.data$prop.FA__grey.drawing),2),
                  "FAs", "High distinctiveness", "Words",
                        round(exp5__prop.FA %>% filter(stim == "col.word") %>% pull(prop.FA) %>% mean(),2),
                        round(mean(exp5__ensor.data$prop.FA__col.word),2),
                  "FAs", "High distinctiveness", "Drawings",
                        round(exp5__prop.FA %>% filter(stim == "col.drawing") %>% pull(prop.FA) %>% mean(),2),
                        round(mean(exp5__ensor.data$prop.FA__col.drawing),2),
                  "","","", NA, NA,
                  "d'", "Low distinctiveness", "Words", 
                        round(exp5__d.prime %>% filter(stim == "grey.word") %>% pull(d.prime) %>% mean(),2),
                        round(mean(exp5__ensor.data$d.prime__grey.word),2),
                  "d'", "Low distinctiveness", "Drawings",
                        round(exp5__d.prime %>% filter(stim == "grey.drawing") %>% pull(d.prime) %>% mean(),2),
                        round(mean(exp5__ensor.data$d.prime__grey.drawing),2),
                  "d'", "High distinctiveness", "Words",
                        round(exp5__d.prime %>% filter(stim == "col.word") %>% pull(d.prime) %>% mean(),2),
                        round(mean(exp5__ensor.data$d.prime__col.word),2),
                  "d'", "High distinctiveness", "Drawings",
                        round(exp5__d.prime %>% filter(stim == "col.drawing") %>% pull(d.prime) %>% mean(),2),
                        round(mean(exp5__ensor.data$d.prime__col.drawing),2),
                  "","","", NA, NA,
                  "c", "Low distinctiveness", "Words", 
                        round(exp5__c %>% filter(stim == "grey.word") %>% pull(c) %>% mean(),2),
                        round(mean(exp5__ensor.data$c__grey.word),2),
                  "c", "Low distinctiveness", "Drawings",
                        round(exp5__c %>% filter(stim == "grey.drawing") %>% pull(c) %>% mean(),2),
                        round(mean(exp5__ensor.data$c__grey.drawing),2),
                  "c", "High distinctiveness", "Words",
                        round(exp5__c %>% filter(stim == "col.word") %>% pull(c) %>% mean(),2),
                        round(mean(exp5__ensor.data$c__col.word),2),
                  "c", "High distinctiveness", "Drawings",
                        round(exp5__c %>% filter(stim == "col.drawing") %>% pull(c) %>% mean(),2),
                        round(mean(exp5__ensor.data$c__col.drawing),2)) %>%
                  rename(" " = "var", "  " = "Dist", "   " = "Stim") %>%
                  mutate_all(as.character) %>%
...
```{r, echo=FALSE, results = FALSE, cache=FALSE}
    table_numbering(name = "exp5__table__ensor.vs.current", 
                     caption = "Mean proportion of hits ")
```

(see `r table_numbering("exp5__table__ensor.vs.current", display = "cite")`

`r table_numbering("exp5__table__ensor.vs.current")`
```{r, echo=FALSE, cache=FALSE}
      exp5__table__ensor.vs.current %>%
          kable("latex", booktabs = T, align = "lllcc") %>%
          kable_styling(font_size = 7) #%>%
          #kable_styling(latex_options = c("hold_position", "scale_down", "condensed")) %>%
...
In the ANOVA on d' scores, there was a significant main effect of stim format `r exp5__d.prime.aov__main.effect`. Distinctive words (`r exp5__d.prime.means__col.word`) producing significantly better discrimination between hits and FAs than regular words (`r exp5__d.prime.means__grey.word`), `r exp5__d.prime.contrasts__grey.word.col.word`.
...
In the ANOVA on the proportion of hits, there was a significant main effect of stim format `r exp5__prop.hit.aov__main.effect`. However, the distinctive words (`r exp5__prop.hit.means__col.word`) did not produce any more hits than regular words (`r exp5__prop.hit.means__grey.word`), `r exp5__prop.hit.contrasts__grey.word.col.word`.
...
In the ANOVA on the proportion of FAs, there was a significant main effect of stim format `r exp5__prop.FA.aov__main.effect`. The distinctive words (`r exp5__prop.FA.means__col.word`) produced significantly fewer FAs than regular words (`r exp5__prop.FA.means__grey.word`), `r exp5__prop.FA.contrasts__grey.word.col.word`.
