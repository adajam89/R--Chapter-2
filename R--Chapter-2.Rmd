...
Despite their widespread use, line drawings have been criticised for their relative simplicity and lack of realism (@viggiano2004), and many researchers have instead begun to utilise photographs as experimental stimuli [@embree2012; @pitarque2016; @troyer2012; @troyer2016; @wang2013a]. Photographs are of great value in
...
While such stimuli are undoubtedly useful in a range of cognitive domains, they do not allow direct performance comparisons with other formats as the concepts depicted are unique to the set they derive from. In order to make such comparisons, and ensure any differences in performance (e.g. recognition memory ability) are indeed attributable to stimuli format, the objects/concepts depicted must be consistent across stimuli formats. Throughout a number of planned recognition experiments, we aim to systematically compare different ‘levels’ of stimuli format on measures of recognition; to our knowledge, no study has systematically matched items across a number of formats in an effort to determine the point at which stimuli distinctiveness impacts recognition. 

The current study presents a new set of photographic stimuli that act as an extension to the set of drawings/words provided by @rossion2004, wherein the concepts depicted have been carefully matched across formats [@rossion2004]. These new stimuli will be utilised 

It is unclear whether the magnitude of picture superiority effects are mediated by the format pictures are presented in, and the use of different formats across studies make it difficult to reconcile the effects of drawing vs. photograph. 

The selected formats present the same common, everyday objects in either: i) word form (written in simple, black ink); ii) drawing form (line-drawn illustrations); or iii) photograph form (detail rich exemplars of the real world object. Two of the stimuli formats were obtained from a pool

The chosen set of photograph stimuli will be taken forward for use in subsequent memory experiments. Normative data will be obtained that help ensure they accurately depict the everyday objects in a similar fashion as those presented in line drawings by Rossion & Pourtois (2004). 

the selected photographs will be supported by
...
The inherent subjectivity involved in the process of matching new photographs with existing drawings may lead to images that are not a reliable ‘match’ to the objects they were selected to depict. For example, the photograph chosen to depict the concept “bottle” may inadvertently provoke the majority of participants to provide the label "wine", thus indicating that this particular photograph has failed to accurately depict the concept it should. To address this issue, and ensure all photographs objectively depict the same objects as their line-drawn counterparts, three potential photograph variations were selected for each item.

; in an effort to reduce researcher bias, we will rely on the newly-acquired normative data to determine which photograph variation best ‘matches’ the items from the set of line drawings (Rossion & Pourtois, 2004).
...
For each item (e.g. shoe), we selected 
...
```{r define-stim-examples--caption, echo=FALSE, results = FALSE}
    figure_numbering(name = "stim-examples--caption",
                     caption = "Example stimuli from ")
```
