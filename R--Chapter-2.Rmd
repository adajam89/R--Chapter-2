...
# List required packages:    
    required_packages <- c("tidyverse", "dplyr","emmeans", "captioner", "ggplot2", "devtools", "tinytex",
                           "kableExtra", "janitor", "fs","irr", "formattable")
...
##  the variation we think is the best to be taken forward to the final stim set. 
    inter_rater <- selected_stimuli %>%
        ...
        distinct(stim)
    write.csv(inter_rater,'~/Desktop/inter_rater.csv', row.names = FALSE)         
...
##  For all the items where there was disagreement, we need to just select one of the options at random:
    disagreements <- merge(rater_1, rater_2) %>%
        ...
        mutate(stim = str_remove(disagreed, "[1-3]")) %>%
        group_by(stim)
...
##  Since we wrote our original randomised selection to .csv, we import that to get the same results.
    ...
    inter_rater_items <- bind_rows(agreements, randomised_disagree_items) %>%
        arrange() 
...
write.csv(final_stim_list,'~/Desktop/final_stim_list.csv', row.names = FALSE)
...
##   H stat:                                                                                                      ##
#################################################################################################################### 
##    Calculate H statistic (naming agreement):
h_calculation <- final_stim %>% 
  group_by(Stim) %>%
  filter(!Resp == "no") %>%                                                     # Drop any "no" response rows - these cannot be included in H calculations.
  arrange (Stim, desc(Resp_count)) %>%                                          # Arrange by "Stim" first, then by "Resp_count" (descending). This ensures that, for each stimuli, the most frequent name is at the top.
  mutate(One_dividedby_Pi = 1 / Resp_proportion) %>%                            # 1 divided by the proportion of each name.
  mutate(Log_two = log2(One_dividedby_Pi)) %>%                                  # Log of base 2 of 1 / prop.
  mutate(Multiplied = Resp_proportion*Log_two) %>%                              # Resp_proportion multiplied by Log of base 2 of 1 / prop.
  mutate(H = sum(Multiplied))                                                   # Sum of all multiplied values = H. 
h_summarised <- h_calculation %>%
  select(Stim, Resp, Resp_proportion, H) %>%
  distinct(Stim, .keep_all = TRUE) %>%
  rename(Most_freq_name = Resp) %>%
  mutate_at(c("Resp_proportion", "H"), round, 2)
write.csv(h_summarised,'~/Desktop/h_summarised.csv', row.names = FALSE)
##    Summarised H stats for grey items: 
h_summarised_grey <- h_summarised %>%
  filter(str_detect(Stim, "grey")) %>%
  rename("Grey Freq Name" = Most_freq_name, "Grey Resp Proportion" = Resp_proportion, "Grey H" = H) %>%
  ungroup(Stim) %>%
  mutate(Stim = str_remove_all(Stim, "_photo_grey")) %>%
  group_by(Stim)
##    Summarised H stats for colour items: 
h_summarised_colour <- h_summarised %>%
  filter(str_detect(Stim, "colour")) %>%
  rename("Colour Freq Name" = Most_freq_name, "Colour Resp Proportion" = Resp_proportion, "Colour H" = H) %>%
  ungroup(Stim) %>%
  mutate(Stim = str_remove_all(Stim, "_photo_colour")) %>%
  group_by(Stim)
##    Combined summarised H stats:
h_summarised_combined <- right_join(h_summarised_colour, h_summarised_grey, by = "Stim") %>%
  select("Stim", "Grey Resp Proportion", "Grey H", "Colour Resp Proportion", "Colour H")
#########################################################################################################################################################################################################################################           
##
## KRIPPENDORFF'S ALPHA:
##
#########################################################################################################################################################################################################################################
# IRR stat. Had to be run for each object. Still say how many items we disagreed on, here's some examples (they were v similar). 
...
kripp_jamie_ratings <- jamie_ratings %>%
  mutate(rater = "jamie") %>%                             # Remove numbers from the Stim coloumn, leaving us with just the item names. This allows us to merge with Rater 2 in a short while.  
  rename(rating = jamie_rating) %>%
  spread(Stim, rating)
kripp_helen_ratings <- helen_ratings %>%
  mutate(rater = "helen") %>%                             # Remove numbers from the Stim coloumn, leaving us with just the item names. This allows us to merge with Rater 2 in a short while.  
  rename(rating = helen_rating) %>%
  spread(Stim, rating)
kripp_ratings <- rbind(kripp_jamie_ratings, kripp_helen_ratings)
##    Create list of all objects that were rated to loop through:
rated_stim_list <- jamie_ratings %>%
  select(Stim) %>%
  mutate(Stim = str_remove_all(Stim, "[1-3]")) %>%                             # Remove numbers from the Stim coloumn, leaving us with just the item names. This allows us to merge with Rater 2 in a short while.  
  unique()
rated_stim_list <- pull(rated_stim_list)
##    Create empty df to store results of for loop: 
kripps_alpha <- tibble()                                                        
##    Loop that runs through each of the rated objects and individually calculates kripps alpha: 
for (i in rated_stim_list) {
  kripp_calc <- kripp_ratings %>%
    select(-rater) %>%                                    # Remove "rater" column (as it cannot be input into kripp.alpha function?).
    select(contains(i)) %>%                               # Select only the ratings for the current iteration (i).
    as.matrix() %>%                                       # Convert to matrix, so "kripp.alpha" can run. 
    kripp.alpha("ordinal")                                # Run Krippendorff's Alpha calculation for this item. 
  kripp_calc <- as.tibble(kripp_calc[c(2,3,5)])             # Convert result from "irrlist" to tibble. 
  kripp_calc <- kripp_calc %>%                              # Adjust "kripp_calc" df...
    mutate(object = i)                                      # To re-insert the object name. 
  kripps_alpha <- bind_rows(kripps_alpha, kripp_calc) }     # Add the completed row to our new "kripps_alpha" df, and repeat loop. 
##    Final edits: 
kripps_alpha <- kripps_alpha %>%
  mutate_at(c("value"), round, 2) %>%
  select(object, subjects, raters, value) %>%
  rename("Krippendorff's alpha (α)" = value, "Image Versions" = subjects, "Number of Raters" = raters, "Concept" = object)
write.csv(kripps_alpha,'~/Desktop/4 - kripps_alpha.csv', row.names = FALSE)
#########################################################################################################################################################################################################################################           
##
## NORMS FOR COLOUR ITEMS:
##
#########################################################################################################################################################################################################################################
col_final_stim <- final_stim_list %>%           # Create new list from "final_stim_list"...
  str_replace("$", "_photo_colour")             # Where we add the 'full' file name back in, so we can go back to the unfiltered fam/viscom etc. data and now filter by our final chosen items. 
##    Familiarity:
col_tidy_fam <- familiarity_data %>%
  select(col_final_stim, -Participant_ID, -Resp_Count) %>%                      # Filter by our final chosen items. Also drop two columns, as we don't need to match responses back to participants (only stim).  
  gather(key = Stim, value = Resp) %>%                                          # Gather all responses from wide into long format.
  filter(!is.na(Resp)) %>%                                                      # Filter out any NA rows, leaving us with just responses.
  group_by(Stim) %>%                                                            # Group by Stim. 
  arrange (Stim, Resp) %>%                                                      # Arrange by Stim name first, then by Resp. 
  mutate(Resp = str_remove_all(Resp, "Fam_")) %>%                               # Remove "Fam_" bit from responses. 
  mutate(Resp = as.numeric(Resp)) %>%                                           # Change Resp col to numeric. 
  summarise_all(funs(col_fam_mean = mean(Resp),                             # Summarise and calculate the mean; 
                     col_fam_sd = sd(Resp),                                 # Standard deviation;
                     col_fam_median = median(Resp),                         # Median
                     col_fam_min = min(Resp),                               # Minimum value; 
                     col_fam_max = max(Resp),                               # Maximum value; 
                     col_fam_range = (col_fam_max - col_fam_min)))  # Range.
##    Visual Complexity:
col_tidy_viscom <- viscom_data %>%
  select(col_final_stim, -Participant_ID, -Resp_Count) %>%                      # Filter by our final chosen items. Also drop two columns, as we don't need to match responses back to participants (only stim).  
  gather(key = Stim, value = Resp) %>%                                          # Gather all responses from wide into long format.
  filter(!is.na(Resp)) %>%                                                      # Filter out any NA rows, leaving us with just responses.
  group_by(Stim) %>%                                                            # Group by Stim. 
  arrange (Stim, Resp) %>%                                                      # Arrange by Stim name first, then by Resp. 
  mutate(Resp = str_remove_all(Resp, "VisCom_")) %>%                            # Remove "VisCom_" bit from responses. 
  mutate(Resp = as.numeric(Resp)) %>%                                           # Change Resp col to numeric. 
  summarise_all(funs(col_viscom_mean = mean(Resp),                              # Summarise and calculate the mean; 
                     col_viscom_sd = sd(Resp),                                  # Standard deviation;
                     col_viscom_median = median(Resp),                          # Median
                     col_viscom_min = min(Resp),                                # Minimum value; 
                     col_viscom_max = max(Resp),                                # Maximum value; 
                     col_viscom_range = (col_viscom_max - col_viscom_min)))     # Range.                     
##    Mental Imagery:
col_tidy_menima <- menima_data %>%
  select(col_final_stim, -Participant_ID, -Resp_Count) %>%                      # Filter by our final chosen items. Also drop two columns, as we don't need to match responses back to participants (only stim).  
  gather(key = Stim, value = Resp) %>%                                          # Gather all responses from wide into long format.
  filter(!is.na(Resp)) %>%                                                      # Filter out any NA rows, leaving us with just responses.
  group_by(Stim) %>%                                                            # Group by Stim. 
  arrange (Stim, Resp) %>%                                                      # Arrange by Stim name first, then by Resp. 
  mutate(Resp = str_remove_all(Resp, "MenIma_")) %>%                            # Remove "MenIma_" bit from responses. 
  mutate(Resp = as.numeric(Resp)) %>%                                           # Change Resp col to numeric. 
  summarise_all(funs(col_menima_mean = mean(Resp),                                  # Summarise and calculate the mean; 
                     col_menima_sd = sd(Resp),                                      # Standard deviation;
                     col_menima_median = median(Resp),                              # Median
                     col_menima_min = min(Resp),                                    # Minimum value; 
                     col_menima_max = max(Resp),                                    # Maximum value; 
                     col_menima_range = (col_menima_max - col_menima_min)))                 # Range.
##    Combined:
colour_final <- col_tidy_fam %>%
  right_join(col_tidy_viscom, by = "Stim") %>%
  right_join(col_tidy_menima, by = "Stim") %>%
  mutate(Stim = str_remove_all(Stim, "_photo_colour")) %>%
  rename(PhotoVersion = Stim) %>%
  mutate(Concept = str_remove_all(PhotoVersion, "[1-3]")) %>%       # Create new column "Concept", aka the object name without version number.   
  mutate(PhotoVersion = str_extract(PhotoVersion, "[1-3]")) %>%       # Create new column "Concept", aka the object name without version number.   
  select(Concept, everything()) %>%                                     # Change column order so Concept is first. 
  mutate_at(c("col_fam_mean", "col_fam_sd", "col_viscom_mean", "col_viscom_sd", "col_menima_mean", "col_menima_sd"), round, 2) %>%
  mutate_at(c("col_fam_median", "col_viscom_median", "col_menima_median"), round, 1)
write.csv(colour_final,'~/Desktop/colour_final.csv', row.names = FALSE)
#########################################################################################################################################################################################################################################           
##
## NORMS FOR GREY ITEMS:
##
#########################################################################################################################################################################################################################################
grey_final_stim <- final_stim_list %>%           # Create new list from "final_stim_list"...
  str_replace("$", "_photo_grey")             # Where we add the 'full' file name back in, so we can go back to the unfiltered fam/viscom etc. data and now filter by our final chosen items. 
##    Familiarity:
grey_tidy_fam <- familiarity_data %>%
  select(grey_final_stim, -Participant_ID, -Resp_Count) %>%                     # Filter by our final chosen items. Also drop two columns, as we don't need to match responses back to participants (only stim).  
  gather(key = Stim, value = Resp) %>%                                          # Gather all responses from wide into long format.
  filter(!is.na(Resp)) %>%                                                      # Filter out any NA rows, leaving us with just responses.
  group_by(Stim) %>%                                                            # Group by Stim. 
  arrange (Stim, Resp) %>%                                                      # Arrange by Stim name first, then by Resp. 
  mutate(Resp = str_remove_all(Resp, "Fam_")) %>%                               # Remove "Fam_" bit from responses. 
  mutate(Resp = as.numeric(Resp)) %>%                                           # Change Resp col to numeric. 
  summarise_all(funs(grey_fam_mean = mean(Resp),                             # Summarise and calculate the mean; 
                     grey_fam_sd = sd(Resp),                                 # Standard deviation;
                     grey_fam_median = median(Resp),                         # Median
                     grey_fam_min = min(Resp),                               # Minimum value; 
                     grey_fam_max = max(Resp),                               # Maximum value; 
                     grey_fam_range = (grey_fam_max - grey_fam_min)))  # Range.                     
##    Visual Complexity:
grey_tidy_viscom <- viscom_data %>%
  select(grey_final_stim, -Participant_ID, -Resp_Count) %>%                     # Filter by our final chosen items. Also drop two columns, as we don't need to match responses back to participants (only stim).  
  gather(key = Stim, value = Resp) %>%                                          # Gather all responses from wide into long format.
  filter(!is.na(Resp)) %>%                                                      # Filter out any NA rows, leaving us with just responses.
  group_by(Stim) %>%                                                            # Group by Stim. 
  ...
  mutate(Resp = str_remove_all(Resp, "VisCom_")) %>%                            # Remove "VisCom_" bit from responses. 
  mutate(Resp = as.numeric(Resp)) %>%                                           # Change Resp col to numeric. 
  summarise_all(funs(grey_viscom_mean = mean(Resp),                                  # Summarise and calculate the mean; 
                     grey_viscom_sd = sd(Resp),                                      # Standard deviation;
                     grey_viscom_median = median(Resp),                              # Median
                     grey_viscom_min = min(Resp),                                    # Minimum value; 
                     grey_viscom_max = max(Resp),                                    # Maximum value; 
                     grey_viscom_range = (grey_viscom_max - grey_viscom_min)))                 # Range.                     
##    Mental Imagery:
grey_tidy_menima <- menima_data %>%
  select(grey_final_stim, -Participant_ID, -Resp_Count) %>%                     # Filter by our final chosen items. Also drop two columns, as we don't need to match responses back to participants (only stim).  
  gather(key = Stim, value = Resp) %>%                                          # Gather all responses from wide into long format.
  filter(!is.na(Resp)) %>%                                                      # Filter out any NA rows, leaving us with just responses.
  group_by(Stim) %>%                                                            # Group by Stim. 
  ...
  mutate(Resp = str_remove_all(Resp, "MenIma_")) %>%                            # Remove "MenIma_" bit from responses. 
  mutate(Resp = as.numeric(Resp)) %>%                                           # Change Resp col to numeric. 
  summarise_all(funs(grey_menima_mean = mean(Resp),                                  # Summarise and calculate the mean; 
                     grey_menima_sd = sd(Resp),                                      # Standard deviation;
                     grey_menima_median = median(Resp),                              # Median
                     grey_menima_min = min(Resp),                                    # Minimum value; 
                     grey_menima_max = max(Resp),                                    # Maximum value; 
                     menima_range = (grey_menima_max - grey_menima_min)))                 # Range.      
##    Combined:
grey_final <- grey_tidy_fam %>%
  right_join(grey_tidy_viscom, by = "Stim") %>%
  right_join(grey_tidy_menima, by = "Stim") %>%
  mutate(Stim = str_remove_all(Stim, "_photo_grey")) %>%
  rename(PhotoVersion = Stim) %>%
  mutate(Concept = str_remove_all(PhotoVersion, "[1-3]")) %>%       # Create new column "Concept", aka the object name without version number.   
  mutate(PhotoVersion = str_extract(PhotoVersion, "[1-3]")) %>%       # Create new column "Concept", aka the object name without version number.   
  select(Concept, everything()) %>%                                     # Change column order so Concept is first. 
  mutate_at(c("grey_fam_mean", "grey_fam_sd", "grey_viscom_mean", "grey_viscom_sd", "grey_menima_mean", "grey_menima_sd"), round, 2) %>%
  mutate_at(c("grey_fam_median", "grey_viscom_median", "grey_menima_median"), round, 1)
write.csv(grey_final,'~/Desktop/grey_final.csv', row.names = FALSE)
...
## Participants
```{r, echo=FALSE}
participant_summary$Age <- as.double(participant_summary$Age)
```

A total of `r nrow(participant_summary)` participants completed the experiment: `r formatC(length(participant_summary$Gender[participant_summary$Gender == "female"]))` females (*M* age = `r formatC(mean(participant_summary$Age[participant_summary$Gender == "female"]))` years, *SD* = `r formatC(sd(participant_summary$Age))`) and `r formatC(length(participant_summary$Gender[participant_summary$Gender == "male"]))` males (*M* = `r formatC(mean(participant_summary$Age[participant_summary$Gender == "male"]))` years, *SD* = `r formatC(sd(participant_summary$Age))`). Subjects were recruited from voluntary participation websites (e.g. \href{https://www.prolific.co/}{Prolific Academic}) and received either payment or research participation credit for taking part. 

## Materials

In Experiment 1, we used a random sample of line-drawn, everyday object stimuli [@rossion2004], along with their written-word counterparts. These items were brought forward into the current study to be matched with photographs, with the exception of a select few depicting shapes that could not easily be translated into photographic format. Such items included arrow and star shapes (without any context), which could not be represented in a photograph in a similar manner. 

The matching process was completed by searching open-source, copyright-freZe image websites (e.g. \href{https://unsplash.com/}{Unsplash}; \href{https://www.pexels.com/}{Pexels} for photographs that depicted the everyday objects in a similiar fashion as the line-drawings (see Appendix B for full list of photograph sources). Three unique photographs were selected for each object, with a general aim of choosing two that closely resembled the line-drawn version, and one that was somewhat different / more modern than the original. 

Adobe Photoshop CC (20.0.04 Release) was used to remove the background from photographs and isolate the object of interest from other potentially distracting visual details. This was completed manually using the Magnetic Lasso and Polygonal Lasso tools. Edges were either not feathered, or feathered by 1px. Once objects had been "cut out" from their background, their orientation was also adjusted to ensure they matched as closely as possible with the depictions of the line drawings (for example, a boot shown with the toe facing left and heel facing right was matched); this sometimes entailed flipping or mirroring the object to 'correct' its' direction. 

In rare instances, the Clone Stamp and Content Aware Fill tools were used to eliminate irrelevant and potentially distracting items that were not removed by the initial crop around the edge of the object (see Appendix ???? for some examples). Any obvious text (e.g. words / letters / brand names) were also removed in a similiar manner; this was done in an effort to ensure photographs were not made additionally memorable by inadvertedly combining more than one stimuli format (photograph + words). Our ultimate goal was to independantly examine recognition toward words, line-drawings, and photographic stimuli, and so a photograph that also contained text may not be directly comparable to one that does not). Numbers were often removed too on the same basis - apart from a few exceptions where numbers were integral to the depiction of the object (e.g. the numbers on a ruler, or a labelled clock).

Cropped photos were exported in both colour and greyscale from Photoshop in “.png” format. 
Other minor edits were made in Adobe Lightroom Classic CC (8.2 Release):
exposure (brightness) - too light / too dark contrast 
Highlights - decreased if some areas of the image were too bright.
Shadows - raised if some areas of the image were too dark. 
Noise reduction - as some objects were not the main focus of the original image, and so the increased crop factor made noise more visible.


The unique changes made to each image were applied to both greyscale and colour versions (e.g. if the colour version of shoe1 had an exposure increase of .010 and shadows raised by +20, these same values were aplied to the greyscale version of shoe1). There were some colour-specific edits though:the white balance of the cropped .png colour photos were normalised using……, and Common image artifacts, such as chromatic aberration (purple fringing) were corrected. 

To normalise the size between all stimuli, the colour corrected and greyscale .pngs were finally imported back into Photoshop CC (20.0.04 Release) to each be placed on a 600x600 pixel white background, and again exported as .pngs. 

MENTION: left/right orientation of images. See [@moreno-martinez2012]. Also - better explanation by [@brodeur2010].

<!-- This needs to be more precise. Does each image roughly take up the same amount of pixels or something? -->
...
Some of the photos don’t really change between colour and greyscale (e.g. sock 3). Examine if responses are the same between these. Maybe remove these items from analyses when looking at the effects of colour?

## Data preperation

Item exclusion:
Items with a “don’t know” score over 20% were excluded:

Krippendorff's alpha (α)
Measure of inter-rater reliability. An α of 1 indicates perfect agreement. 

Naming agreement: H-statistic
Measure of agreement between naming responses (takes into account the number of different names given):
...
## Appendix A
```{r, echo=FALSE}
replaced_items <- read_csv("resources/replaced_items.csv", col_types = cols(.default = "c"))
kable(replaced_items)
```

\newpage
## Appendix B
```{r, echo=FALSE}
original_image_urls <- read_csv("resources/original_image_urls.csv", col_types = cols(.default = "c"))
original_image_urls %>%
  kable(longtable = T, caption = "Original photograph sources") %>%
