...
```{r load-packages, include=FALSE, cache=TRUE}
rm(list=ls())  
...
\newpage
## Method
```{r exp2--age-gender--tablecaption, echo=FALSE, results = FALSE, cache=TRUE}
...
```{r exp2--stim-examples--caption, echo=FALSE, results = FALSE, cache=TRUE}
...
```{r define-photo-manipulation-caption, echo=FALSE, results = FALSE, cache=TRUE}
...
```{r define-men-ima-procedure-caption, echo=FALSE, results = FALSE, cache=TRUE}
...
# Create norms table (all stim):
    ...
    colvsgreynorms_table <- colvsgreynorms %>%
        ...
        rename(" " = "Variable", "  " = "grey_sd", "   " = "blank", "    " = "col_sd")
...
## Results
```{r define-exp2--colvsgreynorms--tablecaption, echo=FALSE, results = FALSE, cache=TRUE}
...
```
To compare normative data between the grey and colour photographs, a series of independant samples t-tests were run.
A series of 

### General analyses
Naming accuracy was very high across all items (*M* = `r mean_naming_acc`), indicating that overall, the selected photographs accurately depicted the intended concepts. Accuracy was the same between the grey (*M* = `r mean_grey_naming_acc`) and colour (*M* = `r mean_col_naming_acc`) versions of the photographs [`r apa__naming_acc__ttest$statistic`]. Overall *H* values were low (*M* = `r mean_H_stat`), similarly indicating a good level of naming agreement across all items. While @rossion2004 observed significantly better naming agreement for their colour items in comparison to their grey items, the same pattern was not replicated when comparing the colour photographs (*M* = `r mean_col_H_stat`) to the grey photographs (*M* = `r mean_grey_H_stat`) in the current study [`r apa__H_stat__ttest$statistic`]. 

Scores of mental imagery agreement (*M* = `r mean_men_ima`),
...
```{r, echo=FALSE, cache=TRUE}
  colvsgreynorms_table %>%
      kable("latex", booktabs = T, align = "lrlcrl", #col.names = NULL,
      ...
      column_spec(3, italic = TRUE, width = "0.8cm") %>%
      column_spec(4, width = "0.8cm") %>%
      column_spec(5, width = "0.8cm") %>%
      column_spec(6, italic = TRUE, width = "0.8cm")
```
...
## Method
```{r exp3--age-gender--tablecaption, echo=FALSE, results = FALSE, cache=TRUE}
...
```{r exp3--prop-hits-and-FAs--tablecaption, echo=FALSE, results = FALSE, cache=TRUE}
...
```{r exp3--dprime-interaction--figurecaption, echo=FALSE, results = FALSE, cache=TRUE}
...
```{r exp3--prop-rec-fam-guess-hits--figurecaption, echo=FALSE, results = FALSE, cache=TRUE}
...
```{r exp3--prop-rec-hits--figurecaption, echo=FALSE, results = FALSE, cache=TRUE}
...
```{r exp3--prop-fam-hits--figurecaption, echo=FALSE, results = FALSE, cache=TRUE}
...
```{r exp3--prop-rec-fam-guess-FA--figurecaption, echo=FALSE, results = FALSE, cache=TRUE}
...
```{r exp3--prop-fam-FA--figurecaption, echo=FALSE, results = FALSE, cache=TRUE}
...
# Appendices
## Appendix A
```{r exp2--spelling-corrections--tablecaption, echo=FALSE, results = FALSE, cache=TRUE}
...
\newpage
## Appendix B
```{r norms-table, include=FALSE, warning = FALSE, message=FALSE, cache=TRUE}
    norms <- raw_data %>%
        ungroup() %>% 
        select(stim, familiarity, visualcomplexity, colourdiagnosticity, mentalimageryagreement) %>%
        mutate(stim = str_replace(stim, "colour1", "colour")) %>%
