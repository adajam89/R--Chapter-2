...
# Chapter 2
Development of standardised photographic stimuli; an extension of Rossion and Pourtois' (2004) object pictorial set

In an effort to understand how recognition memory abilities can deteriorate with age, studies typically make comparisons between the recollection and familiarity abilities of young adults (YAs), healthy older adults (OAs), and those presenting early isolated memory impairments (amnestic Mild Cognitive Impairment; aMCI). Findings thus far have been inconsistent, with some aMCI groups exhibiting impairment to both recollection and familiarity processes, whilst others show only familiarity impairments. Recent reviews have proposed methodological differences across studies could account for the inconsistent findings. For example, there is very little consistency across experiments in regard to the type of stimuli participants engage with during recognition tasks (such as: written words, line-drawn pictures, or photographs) - despite robust evidence to show stimuli format impacts later recognition (namely the Picture Superiority Effect, where pictures (compared to words) produce increased recollection in healthy populations. The level at which stimuli distinctiveness impacts successful recognition in aMCI is currently unclear. 

Throughout a number of planned recognition experiments, we aim to systematically compare the impact different stimuli formats (written words, line-drawings, and photographs) have on the recollection and familiarity processes of YAs, OAs, and those with aMCI. In order to make such comparisons, and ensure any differences in memory test performance are indeed attributable to stimuli format, we must ensure the same to-be-remembered concepts are similarly depicted across the three stimuli formats. We have already selected a set of line-drawings for use in these experiments: pictures of common, everyday objects (e.g. table), obtained from Rossion and Pourtois (2004). The written-word counterparts of these objects will also serve as our word stimuli. Finally, corresponding photographs were obtained online with the aim of depicting the everyday objects in a similar manner to the line-drawings; due to the inherent subjectivity of this latter task, three different photographs were found for each everyday object, with the aim of taking the best ‘match’ forward. 
...
Does the type of picture matter?
i.e. how does stimuli distinctiveness affect recognition response patterns? 
Line-drawings, photographs, etc. 

No study has matched items to examine at what point stimuli-distinctiveness impacts recognition.
Also discuss how different picture types fit in with the Dual-coding hypothesis.

To examine the effect of stimuli distinctiveness, we need different ‘levels’ of the same objects stimuli. The new level of ‘photos’ was created for the same objects.

establish a new set of stimuli. 
...
My research into recognition memory (approved, REC Project Reference: PS-19001) will utilise different  stimuli formats (words, line-drawings, and photographs) to examine the potential role of each format on  subsequent memory test performance. All stimuli are innocuous, and depict common, everyday objects;  
line-drawings (and their written word counterparts) were sourced from Rossion & Pourtois (2004), whilst  potential photographs depicting the objects in a similar manner were manually sourced (from online image  databases). To ensure any differences in memory test performance are indeed attributable to stimuli  format, it is important that all objects are clearly and similarly depicted across words, line-drawings, and  photographs. 
Whilst every effort has been made to source photographs that depict the everyday objects in a similar  fashion to the line-drawings, the inherent subjectivity of this process may lead to images that are not an  accurate ‘match’ of the objects they were chosen to depict; for example, the photograph chosen to convey  “bottle” may inaccurately provoke the label “wine” in the majority of participants. To address this issue - and ensure all photographs objectively depict the same objects as their line-drawn counterparts when  included in the planned memory experiments - I am seeking to collect pilot data that will inform the  selection of photograph stimuli.  
...
```{r raw-data, include=FALSE, warning = FALSE, message=FALSE, cache=FALSE}
####################################################################################################################
##                                                                                                                ##
##    EXPERIMENT 2:                                                                                               ##
##                                                                                                                ##
...
## Materials
A pool of 136 line-drawings [@rossion2004] - depicting common, everyday objects - were brought forward from the previous experiment. These items (along with their written-word labels) formed two unique stimuli formats that would be used in future experiments, but in this study, they were used as a reference in the photograph matching process. Some items were substituted due to unique restrictions that meant they could not easily be translated into photographic format (for example, the shapes "arrow" and "star" can not be represented similarly as photographs). Photo stimuli were obtained by searching open-source, copyright-free image websites (e.g. \href{https://unsplash.com/}{Unsplash}; \href{https://www.pexels.com/}{Pexels}) for photographs that depicted the same everyday objects as the line-drawings  (the full list of image references can be found in Appendix A). 
...
```{r define-photo-manipulation-caption, echo=FALSE, results = FALSE}
    figure_numbering(name = "photo-manipulation-exmaples",
                     caption = "Example manipulations of the photograph stimuli.", )
```

Despite isolating objects from their background, a small number of photographs still contained irrelevant and potentially distracting details (e.g. in one photograph of a 'piano', there was a sign atop the object that may have impacted how the item is encoded when ultimately used in recognition tasks). Such details were removed using the clone stamp and content-aware fill tools Any obvious text (e.g. brand names) and numbers were also removed from photographs using the same method (see `r figure_numbering("photo-manipulation-exmaples", display = "cite")`). The primary aim of the current study was to obtain photographic stimuli that could be clearly separated from words and line drawings; if some of our photograph stimuli contained text, whilst others did not, it is conceivable that they may not be directly comparable in terms of recognition. There were a couple of exceptions to this rule, when such details happened to be integral to the depiction of an object (e.g. the numbers found on a ruler or clock).

&nbsp;
&nbsp;
```{r photo-manipulation-exmaples-image, echo=FALSE, out.width = '100%', cache=TRUE}
knitr::include_graphics("./resources/images/photo-manipulation-exmaples.png")
```

`r figure_numbering("photo-manipulation-exmaples")`  
&nbsp;
&nbsp;
