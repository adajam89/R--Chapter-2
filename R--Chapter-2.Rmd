...
##  An error in the experiment meant 'Order' column was not created for first 20p (all of whom did 'A'): 
        mutate(order = ifelse(id == "5cd3a546fe890a00014dedd1" | id == "5ba2de5c962418000113e3c8" |
                              ...
                              id == "59d65ad8e2762800015eb51b" | id == "5c03b6681a20110001e996a5", "A", 
                              order)) %>%
...
##  Sensible word responses, but based on the picture P were responding to, were probably typos: 
        ...
        mutate(naming_response = str_replace(naming_response,  "^glass$", "glasses")) %>%s
...
####################################################################################################################
    results_norms <- raw_data %>%
        ...
        select(stim, familiarity, visualcomplexity, colourdiagnosticity, mentalimageryagreement,
               fam_rt, viscom_rt, coldia_rt, menima_rt) %>%
        ...
        mutate_at(vars(familiarity, visualcomplexity,
                       colourdiagnosticity, mentalimageryagreement,
                       colourdiagnosticity, fam_rt, viscom_rt,
                       coldia_rt, menima_rt), as.numeric) %>%
        group_by(stim) %>%
...
##  Create norms table (all stim):
    colvsgreynorms <- tribble(
        ~"Variable", ~"Grey", ~"grey_sd", ~"blank", ~"Colour", ~"col_sd",
        "Naming accuracy",
        ...
        "Mental imagery agreement", 
            grey_norms %>% select(grey_menima_mean) %>% pull() %>% round (2),     # Grey MenIma mean;
            grey_norms %>% select(grey_menima_sd) %>% pull() %>% round (2),       # Grey MenIma sd;
            "",
            col_norms %>% select(col_menima_mean) %>% pull() %>% round (2),       # Col MenIma mean;
            col_norms %>% select(col_menima_sd) %>% pull() %>% round (2),         # Col MenIma sd;
        "Familiarity",
            grey_norms %>% select(grey_fam_mean) %>% pull() %>% round (2),        # Grey Fam mean;
            grey_norms %>% select(grey_fam_sd) %>% pull() %>% round (2),          # Grey Fam sd;
            "",
            col_norms %>% select(col_fam_mean) %>% pull() %>% round (2),          # Col Fam mean;
            col_norms %>% select(col_fam_sd) %>% pull() %>% round (2),            # Col Fam sd;
        "Visual complexity",
...
##   Mental imagery agreement:                                                                                 ##
#################################################################################################################
    men_ima <- results_norms %>%
        select(stim, mentalimageryagreement_mean) %>%
        ungroup()
    ...
    col_men_ima <- men_ima %>%
        ...
        select(-stim)
    men_ima__ttest <- t.test(grey_men_ima, col_men_ima)
    apa__men_ima__ttest <- papaja:::apa_print(men_ima__ttest)
    mean_men_ima <- men_ima %>% pull() %>% mean() %>% round(2) 
    mean_grey_men_ima <- grey_men_ima %>% pull() %>% mean() %>% round(2) 
    mean_col_men_ima <- col_men_ima %>% pull() %>% mean() %>% round(2)
####################################################################################################################
##   Mental imagery agreement RT t-test:                                                                          ##
####################################################################################################################
    men_ima_rt <- results_norms %>%
        select(stim, mentalimageryagreement_mean) %>%
        ungroup()
    grey_men_ima <- men_ima %>%
        filter(str_detect(stim, "grey")) %>%
...
####################################################################################################################
    ...
    col_fam <- fam %>%
        ...
        select(-stim)
    fam__ttest <- t.test(grey_fam, col_fam)
    apa__fam__ttest <- papaja:::apa_print(fam__ttest)
    mean_fam <- fam %>% pull() %>% mean() %>% round(2)
    mean_grey_fam <- grey_fam %>% pull() %>% mean() %>% round(2) 
    mean_col_fam <- col_fam %>% pull() %>% mean() %>% round(2)
####################################################################################################################
##   Visual complexity t-test:                                                                                    ##
####################################################################################################################
    vis_com <- results_norms %>%
        select(stim, visualcomplexity_mean) %>%
        ungroup()
    grey_vis_com <- vis_com %>%
        filter(str_detect(stim, "grey")) %>%
        mutate(stim = str_remove_all(stim, "[-](.*)")) %>%
    ...
    col_vis_com <- vis_com %>%
        ...
        select(-stim)
    vis_com__ttest <- t.test(grey_vis_com, col_vis_com)
    apa__vis_com__ttest <- papaja:::apa_print(vis_com__ttest)
    mean_vis_com <- vis_com %>% pull() %>% mean() %>% round(2)
    mean_grey_vis_com <- grey_vis_com %>% pull() %>% mean() %>% round(2)
    mean_col_vis_com <- col_vis_com %>% pull() %>% mean() %>% round(2)
####################################################################################################################
##   Colour diagnosticity:                                                                                        ##
####################################################################################################################
    col_dia <- results_norms %>%
        select(stim, colourdiagnosticity_mean) %>%
        ungroup() %>%
        drop_na(colourdiagnosticity_mean)
    mean_col_dia <- col_dia %>% pull() %>% mean() %>% round(2)
```
