...
Naming accuracy was high in both the grey (*M* = ) and colour (*M* = ) photographs, with no significant difference between them [] 

`r apa__naming_acc__ttest$full_result`


## Results
```{r define-exp2--colvsgreynorms--tablecaption, echo=FALSE, results = FALSE}
    table_numbering(name = "exp2--colvsgreynorms--tablecaption",
                     caption = "Mean (*SD*) scores for each variable by colour and greyscale photographs.")
```
To compare normative data between the grey and colour photographs, a series of independant samples t-tests were run.
A series of 

### General analyses
Naming accuracy was high for both grey (94%) and colour (95%) photographs, with no significant difference between the two []
