...
```{r define-exp2--colvsgreynorms--tablecaption, echo=FALSE, results = FALSE, cache=FALSE}
    table_numbering(name = "exp2--colvsgreynorms--tablecaption",
                     caption = "Summary statistics for each of the measured variables. Mean values are presented in bold (SDs are shown in parentheses).")
```
...
### *The role of colour*
No naming differences were observed between the grey and colour photographs. For accuracy and *H* values, this result was expected. The addition/absence of colour should not alter how participants label the items overall, apart from rare instances whereby a lack of colour may lead to the misidentification of an object (e.g. incorrectly labelling a greyscale photograph of an orange as 'grapefruit'). However, the absence of reaction time differences was not expected; it is reasonable to assume that colour photographs - with an additional layer of contextual information - would be identified (and thus named) quicker than grey items (e.g. a colour photograph of an orange should not be subject to the potential ambiguity that might accompany a greyscale depiction). Indeed, @rossion2004 found significant differences between the naming RTs of their colour and greyscale drawings, 

In the current study, significant differences were observed between the grey and colour items on scores of mental imagery agreement, with colour photographs offering a better match to subjects imagined presentation of the objects than grey photographs. 
...
The specific methodologies utilised in recognition memory research are rarely consistent across studies. An often overlooked factor is the format to-be-remembered stimuli are presented in, with many studies utilising written or spoken words (???), line drawings or illustrations (???), or real world photographs (???).
...
##  Prolific - grey (v131):
                     ...
                     "./resources/raw_data/experiment_3_and_4/data_exp_10169-v131_task-6t6c.csv", # Study_6_Grey ##  Hanover - grey (v4):
...
### Design
The current study utilised a mixed design, with a 3-level within-subjects factor of stimuli format (words, drawings, photographs), and a 2-level between-subjects factor of response option (RFG, RFBG). Subjects passed through 2 levels of blocked randomization (equally sized, predetermined blocks). First, participants were randomly assigned one of six possible study lists (each of equal length, and containing an even number of word, drawing, and photograph items); second, subjects were either assigned a recognition test with three possible response options (RFG: “Recollection”, “Familiarity”,“Guessing”), or four possible response options (RFBG: “Recollection”, “Familiarity”, “Guessing”, “Both”). These randomisation processes were completed automatically by the experiment software using balanced randomisation methods.

### Procedure
Data was collected online using \href{https://gorilla.sc/}{Gorilla} - a platform for the builidng and hosting of online experiments. 
...
\newpage
## Results
A series of 3x2 repeated measures ANOVAs were conducted on each of the DVs using a within-subjects factor of stimuli format (photographs / drawings / words) and a between-subjects factor of response option (RFG / RFBG). Significant main effects and interaction effects were followed-up with pairwise comparisons.


Follow-up comparisons for main effects of group employed a Bonferroni-adjusted alpha level of .017 for 3 comparisons, whilst comparisons for significant interactions employed a Bonferroni-adjusted alpha level of .005 for 9 comparisons.
...
##  Format ANOVA main effects for use in report:  
    apa__grey__anova__RECOG <- papaja:::apa_print.afex_aov(grey__anova__RECOG) 
        ...
        apa__grey__anova__RECOG__resp.option.main.effect <- apa__grey__anova__RECOG$full$resp_option %>%
            str_extract("^(?:[^\\,]*\\,){3}([^\\,]*)")
        apa__grey__anova__RECOG__interaction.main.effect <- apa__grey__anova__RECOG$full$resp_option_stim_format %>%
            str_extract("^(?:[^\\,]*\\,){3}([^\\,]*)")
#-----------------------------------------------------------------------------------------------------------------#
#  Run post-hoc contrasts                                                                                         
#-----------------------------------------------------------------------------------------------------------------#
