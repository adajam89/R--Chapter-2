...
##  Import rating data: 
    rater_1 <- read_csv("./resources/rater_1.csv", col_types = list(col_character(), scol_double())) %>%
...
##  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    apa__fam__ttest <- papaja:::apa_print(fam_rt__ttest)
################################################################################################################
##   Visual Complexity:                                                                                       ##
################################################################################################################
...
##  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    viscom_rt__ttest <- t.test(grey_vis_com$grey_viscom_rt, col_vis_com$col_viscom_rt)
    apa__vis_com__ttest <- papaja:::apa_print(viscom_rt__ttest)    
...
##  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    apa__men_ima__ttest <- papaja:::apa_print(men_ima_rt__ttest)
```
...
##  Create norms table:
    colvsgreynorms <- tribble( ~"Variable", ~"Grey", ~"grey_sd", ~"blank", ~"Colour", ~"col_sd",
        "Naming accuracy",
            grey_norms %>% select(grey_naming_acc_mean) %>% pull() %>% round (2), ##  Grey naming accuracy mean;
            grey_norms %>% select(grey_naming_acc_sd) %>% pull() %>% round (2),"",##  Grey naming accuracy sd;
            col_norms %>% select(col_naming_acc_mean) %>% pull() %>% round (2),   ##  Col naming accuracy mean;
            col_norms %>% select(col_naming_acc_sd) %>% pull() %>% round (2),     ##  Col naming accuracy sd;
        "Naming agreement (H)", 
            grey_norms %>% select(grey_H_mean) %>% pull() %>% round (2),          ##  Grey H mean;
            grey_norms %>% select(grey_H_sd) %>% pull() %>% round (2),"",         ##  Grey H sd;
            col_norms %>% select(col_H_mean) %>% pull() %>% round (2),            ##  Col H mean;
            col_norms %>% select(col_H_sd) %>% pull() %>% round (2),              ##  Col H sd;
        "Mental imagery agreement", 
            grey_men_ima_means$grey_menima_mean, grey_men_ima_means$grey_menima_sd, "",
            col_men_ima_means$col_menima_mean,  col_men_ima_means$col_menima_sd,
        "Familiarity",
            grey_fam_means$grey_fam_mean, grey_fam_means$grey_fam_sd, "",
            col_fam_means$col_fam_mean,  col_fam_means$col_fam_sd,
        "Visual complexity",
            grey_norms %>% select(grey_viscom_mean) %>% pull() %>% round (2),     ##  Grey VisCom mean;
            grey_norms %>% select(grey_viscom_sd) %>% pull() %>% round (2),"",    ##  Grey VisCom sd;
            col_norms %>% select(col_viscom_mean) %>% pull() %>% round (2),       ##  Col VisCom mean;
            col_norms %>% select(col_viscom_sd) %>% pull() %>% round (2),         ##  Col VisCom sd;
        "Colour diagnosticity",
            NA,                                                                   ##  Grey ColDia mean;
            NA,"",                                                                ##  Grey ColDia sd;
            col_norms %>% select(col_coldia_mean) %>% pull() %>% round (2),       ##  Col ColDia mean;
            col_norms %>% select(col_coldia_sd) %>% pull() %>% round (2))         ##  Col ColDia sd. 
    
    colvsgreynorms_table <- colvsgreynorms %>%
        mutate(grey_sd = str_c("(", grey_sd, ")")) %>%
        mutate(col_sd = str_c("(", col_sd, ")")) %>%
        mutate_all(as.character) %>%
        mutate(Grey = replace_na(Grey, "-")) %>%
        mutate(grey_sd = replace_na(grey_sd, "")) %>%
        unite("Grey", Grey:grey_sd, sep = " ", remove = TRUE) %>%
        unite("Colour", Colour:col_sd, sep = " ", remove = TRUE) %>%
        rename(" " = blank)
```
...
```{r define-exp2--colvsgreynorms--tablecaption, echo=FALSE, results = FALSE, cache=FALSE}
    table_numbering(name = "exp2--colvsgreynorms--tablecaption",
                     caption = "Summary statistics (mean and *SD*) for each of the measured variables, by colour and greyscale items.")
```

## Results
Mean scores for each of the variables of interest, separated between colour and greyscale items, are shown in `r table_numbering("exp2--colvsgreynorms--tablecaption", display = "cite")` (mean scores for each unique photograph are presented in Appendix B). To compare normative data across the grey and colour versions of the photographs, a series of independent samples t-tests were run on each variable, as well as their corresponding reaction times (excluding scores of colour diagnosticity, which only apply to items presented in colour). 

### *Naming*
Naming accuracy was very high for all of the photographs (*M* = `r mean_naming_acc`), indicating that overall, the selected items closely depicted the intended concepts. Accuracy rates did not differ between the grey (*M* = `r mean_grey_naming_acc`) and colour (*M* = `r mean_col_naming_acc`) versions of the photographs [`r apa__naming_acc__ttest$statistic`]. *H* values were also low across all items (*M* = `r mean_H_stat`), showing that subjects generally agreed on how the item should be named. While @rossion2004 observed significantly better naming agreement for their colour - rather than greyscale - items, this pattern was not replicated with the current set of photographs; *H* values did not differ between the grey (*M* = `r mean_grey_H_stat`) and colour (*M* = `r mean_col_H_stat`) photographs [`r apa__H_stat__ttest$statistic`]. A mean reaction time (RT) of (*M* = `r mean_naming_rt`s) was observed for naming responses, and while this this tells us little without any sort of baseline (the current methods of collecting naming responses were slightly different than that of @rossion2004), they did allow comparisons to be made between grey and colour items (though no difference was observed [grey *M* = `r mean_grey_naming_rt`s, colour *M* = `r mean_col_naming_rt`s, `r apa__naming_rt__ttest$statistic`]). Overall, these analyses suggest that the current photographs closely resemble the line drawings they were designed to match, with high levels of naming accuracy and agreement among subjects. The absence of any colour differences indicates there were no naming advantages when photographs were made even more distinctive through the addition of colour. 
...
```{r, echo=FALSE, cache=TRUE}
  colvsgreynorms_table %>%
      kable("latex", booktabs = T, align = "lccc", #col.names = NULL,
            linesep = c("", "\\addlinespace", "\\addlinespace", "\\addlinespace", "\\addlinespace")) %>%
      kable_styling(latex_options = "hold_position") %>%
      add_header_above(c(" ", "Photographs" = 2)) %>%
...
### *Mental imagery agreement*
Scores of mental imagery agreement were moderate across all items (*M* = `r mean_men_ima`), and presented data consistent with previous scores obtained in response to drawing stimuli (@rossion2004). Unlike previous studies, however, the items in the current study exhibited colour differences; colour photographs (*M* = `r mean_col_men_ima`) showed significantly higher mental imagery agreement scores than grey photographs (*M* = `r mean_grey_men_ima`), [`r apa__men_ima__ttest$statistic`].

### *Familiarity*
Scores of familiarity (*M* = `r mean_fam`),
grey (*M* = `r mean_grey_fam`),
colour (*M* = `r mean_col_fam`),
[`r apa__fam__ttest$statistic`]

### *Visual complexity*
Scores of visual complexity (*M* = `r mean_vis_com`),
grey (*M* = `r mean_grey_vis_com`),
colour (*M* = `r mean_col_vis_com`),
[`r apa__vis_com__ttest$statistic`]
