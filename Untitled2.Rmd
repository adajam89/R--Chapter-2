...
##################################################################################################################
    ...
    ##  Convert total number of correct responses at study as proportion:
          mutate(study.total__percent.correct = study.total__n.correct*(100/study.resp__total)) %>%
...
##    Tidy test data:
    ...
    ##  Replace stim code with actual item name:
          ...
          mutate(question.number = str_extract(question.number, ".{12}$")) %>%
          mutate(question.number = ifelse(question.number == str_detect(., "recollection"), "recollection",
                 "NOOOOOOOOOOOO"))
                                            
      write.csv(exp1__test.stats,'~/Desktop/exp1__test.stats.csv')
                   ...
                   str_extract(question.number, ".{12}$")) %>%
      ...
      %>%
          mutate(question = str_remove(question, ".{4}$")) %>%
          mutate(resp_option = case_when(str_detect(question.number, "q3") ~ "rfg",
                                         ...
                                         str_detect(question.number, "recollection") ~ "rf_ratings",
                                         str_detect(question.number, "familiarity") ~ "rf_ratings"))
    ...
    ##  Split the 'stim' column up, so item name and stimuli format are separate:
      ...
      %>%
    ##  Clearly show which lists were studied as words and pics: 
          mutate(test_words = ifelse(shown_format == "word", study_list, NA)) %>%
          mutate(test_pics = ifelse(shown_format == "picture", study_list, NA))
      ...
      %>%
          ...
          mutate(rfr_resp_option_type = question) %>%
                  ...
                  mutate(rfr_resp_option_type = 
                   ifelse(response == "recollection" | response == "familiarity",
      ...
      %>%
##    Reshape to put RTs on the same row as resp_options):
          mutate(temp_index = as.double(resp_option)) %>%
          mutate(index = ifelse((is.na(temp_index) & resp_option == "rfg") & 
                                  (resp_option == "old" | resp_option == "new"), 'rfg_old_new','rt')) %>%
          mutate(index = ifelse((is.na(temp_index) & resp_option == "rfbg") & 
                                  (resp_option == "old" | resp_option == "new"), 'rfbg_old_new', index)) %>%
          mutate(index = ifelse((is.na(temp_index) & resp_option == "rf_ratings") & 
                                  (resp_option == "old" | resp_option == "new"), 'rf_ratings_old_new', index)) %>%
          mutate(index = ifelse((is.na(temp_index) & resp_option == "rfg") & 
                                  (resp_option != "old" & resp_option != "new"), 'rfg', index)) %>%
          mutate(index = ifelse((is.na(temp_index) & resp_option == "rfbg") & 
                                  (resp_option != "old" & resp_option != "new"), 'rfbg', index)) %>%
          mutate(index = ifelse((is.na(temp_index) & 
                                   resp_option == "rf_ratings" & rfr_resp_option_type == "recollection") & 
                                  (resp_option != "old" & resp_option != "new"), 'rec_rating', index)) %>%
          mutate(index = ifelse((is.na(temp_index) & 
                                   resp_option == "rf_ratings" & rfr_resp_option_type == "familiarity") & 
                                  (resp_option != "old" & resp_option != "new"), 'fam_rating', index)) %>%
...
#------------------------------------------------------------------------------------------------------------------
      exp1__test.stats <- reshape(exp1__test.stats, idvar=c('test_id','question'), timevar='index', 
          ...
          unite("old_new", resp_option.rfg, resp_option.rfbg, 
                resp_option.rf_ratings_old_new, remove = TRUE, na.rm = TRUE) %>%
          select(-resp_option.rfg_old_new, -resp_option.rfg, -resp_option.rf_ratings_old_new, 
                 ...
                 -resp_option.rfbg) %>%
##    Recode RF-Ratings resp_options to better show rec/fam:
          mutate(resp_option.rec_rating = str_remove_all(resp_option.rec_rating, " ")) %>%
          mutate(resp_option.rec_rating = str_remove_all(resp_option.rec_rating, "(?<=.{1}).+")) %>%
          mutate(resp_option.rec_rating = str_c("rec_", resp_option.rec_rating)) %>%
          mutate(resp_option.fam_rating = str_remove_all(resp_option.fam_rating, " ")) %>%
          mutate(resp_option.fam_rating = str_remove_all(resp_option.fam_rating, "(?<=.{1}).+")) %>%
          mutate(resp_option.fam_rating = str_c("fam_", resp_option.fam_rating)) %>%
##    Unite columns of the same resp_option type:
          unite("recfam_1", resp_option.rfg, resp_option.rfbg, resp_option.rec_rating, remove = TRUE, na.rm = TRUE) %>%
          mutate(recfam_1 = na_if(recfam_1, "")) %>%
          rename(recfam_2 = "resp_option.fam_rating")
##    Merge with 'exp1__image_urls' to get proper item names:
      ...
      exp1__test.stats <- exp1__test.stats %>%
          ...
          rename(old_new_rt = "resp_option.rt", resp_option = "resp_option.rt", 
...
##    Calculate mean word reaction time (all resp_options):
          mutate(mean_test_rt_word = mean(old_new_rt[stim == "word"])) %>%
##    Calculate mean pic reaction time (all resp_options):
...
##  Add new d'prime calculations to 'exp3__test.stats' df:  ------------------------------------------------------ 
    exp1__test.stats <- exp1__test.stats %>%
        ...
        mutate_at(vars(age, study.resp__total, study.word__n.resps, study.pic__n.resps,
