...
##    Tidy test data:
    ...
    ##  Replace stim code with actual item name:
          ...
          select(test_id, study_list, resp_option, stim, response, question.number) %>%
      ##  Remove filler items:
          filter(!str_detect(study_list, 'filler')) %>%
      ##  Split the 'stim' column up, so item name and stimuli format are separate:
          separate(stim, c("item_name", "shown_format")) %>%
      ##  Clearly show which lists were presented as words and pics at test: 
          mutate(words.at.test = ifelse(shown_format == "word", study_list, NA)) %>%
          mutate(pics.at.test = ifelse(shown_format == "picture", study_list, NA)) %>%
      ##  Recode R/F responses for clarity:
          ...
          select(-study_list, -question.number) %>%
...
##    Add justifications back in: 
      exp1__test.stats <- exp1__test.stats %>%
          ...
          select(test_id, ip, start_date, rec_justification, fam_justification, both_justification,
                 ...
                 old_new, old_new_rt, recfam_1, recfam_2, test.total__n.resps, test.word__n.resps, 
                 test.pic__n.resps, mean_old_new_rt_total, mean_test_rt_word, mean_test_rt_pic) %>%
...
##    Remove Ps with incomplete test data:
      exp1__incomplete_exp1__test.stats <- exp1__test.stats %>%
          select(test_id, test.total__n.resps) %>%
